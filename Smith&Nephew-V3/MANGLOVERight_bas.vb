    ...
    Sub PR_CalculateExtension(ByRef xyWristUlnar As XY, ByRef xyWristRadial As XY, ByRef nInsert As Double,
        ...
        If g_RightExtendTo = RIGHT_GLOVE_ELBOW Or g_RightExtendTo = RIGHT_GLOVE_AXILLA Then
            ...
            For ii = iStartTape To iLastTape
                ...
                If ii = iStartTape + 1 And g_nRightPleats(1) > 0 Then nSpacing = ARMEDDIA1.fnDisplayToInches(g_nRightPleats(1))
                If ii = iStartTape + 2 And g_nRightPleats(2) > 0 Then nSpacing = ARMEDDIA1.fnDisplayToInches(g_nRightPleats(2))
                'XXXXXXXXXX
                'XXXX Careful now! this won't work if only 3 or less tapes given
                'XXXXXXXXXX
                'Axilla
                If ii = iLastTape - 1 And g_nRightPleats(3) <> 0 Then nSpacing = ARMEDDIA1.fnDisplayToInches(g_nRightPleats(3))
                If ii = iLastTape And g_nRightPleats(4) <> 0 Then nSpacing = ARMEDDIA1.fnDisplayToInches(g_nRightPleats(4))
