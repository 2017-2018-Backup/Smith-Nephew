...
Friend Class cadglved
    ...
    Public g_MATERIAL As Project1.ARMEDDIA1.fabric
	...
	Private Sub Form_LinkClose()
		...
		If g_sSide = "Left" Then
			Me.Text = "CAD GLOVE Edit - Left"
			g_Direction = -1
		End If
		If g_sSide = "Right" Then
			Me.Text = "CAD GLOVE Edit - Right"
			g_Direction = 1
		End If
		...
		EditMMs(g_iFirstTape).BackColor = System.Drawing.ColorTranslator.FromOle(&HFFFFFF)
	...
	Private Sub cadglved_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		...
		If g_OffFold Then
			...
			For ii = 0 To 17
				If g_iChanged(ii) < 0 Or g_iChanged(ii) > 0 Then
					...
					nFiguredValue = ((((ARMEDDIA1.fnDisplayToInches(g_nLengths(ii)) * (System.Math.Abs(g_iRed(ii) - 100) / 100)) + 0.375) - (2 * g_nInsertSize)) / 2)
			...
			If g_GloveType = GLOVE_ELBOW And g_iLastTape = ELBOW_TAPE And ((g_iChanged(g_iLastTape) < 0 Or g_iChanged(g_iLastTape) > 0) Or (g_iChanged(g_iLastTape - 1) < 0 Or g_iChanged(g_iLastTape - 1) > 0)) Then
				...
				nFiguredValue = ((((ARMEDDIA1.fnDisplayToInches(g_nLengths(g_iLastTape - 1)) * (System.Math.Abs(g_iRed(g_iLastTape - 1) - 100) / 100)) + 0.375) - (2 * g_nInsertSize)) / 2)
				xyPt.y = xyProfileETS(iProfileVertex - 1).y
				xyPt.X = xyOtemplate.X - ((nFiguredValue / 2) * g_Direction)
				
				iProfileVertex = g_iVertexETSMap(g_iLastTape)
				nFiguredValue = ((((ARMEDDIA1.fnDisplayToInches(g_nLengths(g_iLastTape)) * (System.Math.Abs(g_iRed(g_iLastTape) - 100) / 100)) + 0.375) - (2 * g_nInsertSize)) / 2)
	...
	Private Sub PR_DrawProfileEdits()
		...
		If g_OffFold Then
			If g_iChanged(g_iFirstTape) > 0 Or g_iChanged(g_iFirstTape + 1) > 0 Then
				...
				nFiguredValue = ((((ARMEDDIA1.fnDisplayToInches(g_nLengths(g_iFirstTape)) * (System.Math.Abs(g_iRed(g_iFirstTape) - 100) / 100)) + 0.375) - (2 * g_nInsertSize)) / 2)
				xyPALMER.X = xyPALM6.X + (nFiguredValue * g_Direction)
				
				nFiguredValue = ((((ARMEDDIA1.fnDisplayToInches(g_nLengths(g_iFirstTape + 1)) * (System.Math.Abs(g_iRed(g_iFirstTape + 1) - 100) / 100)) + 0.375) - (2 * g_nInsertSize)) / 2)
				iProfileVertex = g_iVertexLFSMap(g_iFirstTape + 1)
				xyPt1.y = xyProfileLFS(iProfileVertex - 1).y
				xyPt1.X = xyOtemplate.X + ((nFiguredValue / 2) * g_Direction)
				CADGLEXT.PR_CalcWristBlendLFS(xyArcCenter, aArcSweep, xyArcStart, xyPALMER, xyPt1, g_LFSBlendProfile)
				For ii = 1 To 8
					iProfileVertex = g_iVertexLFSBlendMap(ii)
					'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
					PrintLine(fNum, "SetVertex(hCurvLFS," & iProfileVertex & "," & g_LFSBlendProfile.X(ii + 1) & "," & g_LFSBlendProfile.y(ii + 1) & ");")
		...
		If g_ReDrawn <> True Then
			...
			'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(fNum, "hDDE = Open (" & QQ & "dde" & QCQ & "cadglved" & QCQ & "cadglved" & QQ & ");")
			'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
	...
	Private Sub PR_GetProfileFromFile(ByRef sFileName As String)
		...
		Input(fFileNum, xyArcCenter.X)
		Input(fFileNum, xyArcCenter.y)
		Input(fFileNum, nArcRadius)
		Input(fFileNum, aArcStart)
		Input(fFileNum, aArcSweep)
		If g_sSide = "Right" Then
			ARMDIA1.PR_CalcPolar(xyArcCenter, aArcStart + aArcSweep, nArcRadius, xyArcStart)
			aArcSweep = -aArcSweep
		Else
			ARMDIA1.PR_CalcPolar(xyArcCenter, aArcStart, nArcRadius, xyArcStart)
		End If
		Input(fFileNum, xyCLStart.X)
		Input(fFileNum, xyCLStart.y)
		Input(fFileNum, xyCLEnd.X)
		Input(fFileNum, xyCLEnd.y)
