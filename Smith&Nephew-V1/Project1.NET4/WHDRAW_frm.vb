...
Friend Class whdraw
    ...
    Private Function FN_ChapOpen(ByRef sDrafixFile As String) As Short
        ...
        'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        PrintLine(fNum, "//Patient - " & Project1.WHDRAW1.g_sPatient & CC & " " & g_sFileNo & CC & " SIDE - " & g_sSide)
        'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
    ...
    Private Function FN_WHOpen(ByRef sDrafixFile As String, ByRef sName As Object, ByRef sPatientFile As Object, ByRef sLeftorRight As Object) As Short
        ...
        Project1.WHDRAW1.g_sPatient = sName
    ...
    Private Sub Form_LinkClose()
        ...
        If txtWorkOrder.Text = "" Then
            Project1.WHDRAW1.g_sWorkOrder = "-"
        Else
            Project1.WHDRAW1.g_sWorkOrder = txtWorkOrder.Text
        End If
    ...
    Private Sub PR_StandardDetails()
        ...
        BDYUTILS.PR_PutStringAssign("sWorkOrder", Project1.WHDRAW1.g_sWorkOrder)
    ...
    Private Sub PR_WH_2ndLeg()
        ...
        'Open Macro file (fNum is declared as Global)
        sFile = "@C:\ACAD2018_SMITH-NEPHEW\Lookup Tables\JOBST\DRAW.D"
        'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        fNum = FN_WHOpen(sFile, txtPatientName, txtFileNo, sSide)
    ...
    Private Sub PR_WH_ChapBody(ByRef sSide As String)
        ...
        Dim xyStartBotOFF, xyStartCL, xyPt2, xyPt, xyPt1, xyPt3, xyStartOFF, xyInt As WHUTILS.xy
        Dim xyWaistBandProx, xyEOSCL, xyChapDatumOFF, xyWaistBandDist As WHUTILS.xy
        Dim xyVelcroOverLapProx, xyVelcroOverLapDist As WHUTILS.xy
        Dim iDistCase, iProxCase As Short
        Dim xyChapBotDatumOFF As XY
        Dim xyCenTop, xyCenTopProx, xyCenTopDist, xyArcBottomPt As WHUTILS.xy
        ...
        Dim TmpBotlegProfile As WHDRAW1.curve
        ...
        If FileLen("@C:\ACAD2018_SMITH-NEPHEW\Lookup Tables\JOBST\LEGCURVE.DAT") = 0 Then
            MsgBox("@C:\ACAD2018_SMITH-NEPHEW\Lookup Tables\JOBST\LEGCURVE.DAT" & "Not found", 48, g_sDialogueID)
            Exit Sub
        End If

        fFile = FreeFile()
        FileOpen(fFile, "@C:\ACAD2018_SMITH-NEPHEW\Lookup Tables\JOBST\LEGCURVE.DAT", OpenMode.Input)

        'Get control points
        Input(fFile, Project1.WHDRAW1.xyOtemplate.X)
        Input(fFile, Project1.WHDRAW1.xyOtemplate.y)
        ...
        xyChapDatum.y = Project1.WHDRAW1.xyOtemplate.y
        ...
        Project1.WHDRAW1.xyOrigin = Project1.WHDRAW1.xyOtemplate
        WHUTILS.PR_MakeXY(xyStartCL, xyChapDatum.X - 2.5, xyChapDatum.y)
        WHUTILS.PR_MakeXY(xyStartOFF, xyStartCL.X, xyChapDatum.y + 100)

        WHUTILS.PR_MakeXY(xyPt1, g_TopLegProfile.X(iStart), g_TopLegProfile.y(iStart))

        Sucess = False
        If g_TopLegProfile.n = 3 Then iStart = 0 'NB 2 tape leg
        For ii = iStart + 1 To g_TopLegProfile.n
            WHUTILS.PR_MakeXY(xyPt2, g_TopLegProfile.X(ii), g_TopLegProfile.y(ii))
            'Lingam
            'If WHUTILS.FN_LinLinInt(xyStartCL, xyStartOFF, xyPt1, xyPt2, xyStartOFF) Then
            '    Sucess = True
            '    Exit For
            'End If
        ...
        If Not Sucess Then
            WHUTILS.PR_MakeXY(xyStartOFF, xyStartCL.X, g_TopLegProfile.y(1))
        ...
        Sucess = False
        WHUTILS.PR_MakeXY(xyChapDatumOFF, xyChapDatum.X, xyChapDatum.y + 100)
        'Lingam
        'If WHUTILS.FN_LinLinInt(xyChapDatum, xyChapDatumOFF, xyProfileDistal, xyProfileProximal, xyChapDatumOFF) Then
        '    Sucess = True
        'End If
        ...
        WHUTILS.PR_MakeXY(xyEOSCL, xyChapDatum.X + g_nChapLenFigured, xyChapDatum.y)
        WHUTILS.PR_MakeXY(xyVelcroOverLapProx, xyEOSCL.X, xyChapDatum.y + nEighthOfWaistCir)
        WHUTILS.PR_MakeXY(xyVelcroOverLapDist, xyEOSCL.X - 1, xyChapDatum.y + nEighthOfWaistCir)

        WHUTILS.PR_MakeXY(xyWaistBandProx, xyVelcroOverLapProx.X, xyVelcroOverLapProx.Y - g_nChapCirFigured)
        WHUTILS.PR_MakeXY(xyWaistBandDist, xyVelcroOverLapDist.X, xyVelcroOverLapDist.Y - g_nChapCirFigured)


        'Draw CHAP
        'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        fNum = FN_ChapOpen("C:\ACAD2018_SMITH-NEPHEW\Lookup Tables\JOBST\DRAW.D")

        WHUTILS.PR_SetLayer("Template" & sSide)
        WHUTILS.PR_DrawLine(xyWaistBandDist, xyWaistBandProx)
        WHUTILS.PR_DrawLine(xyWaistBandProx, xyVelcroOverLapProx)

        'Draw velcro OVERLAP
        'If two legs and we are drawing the Left then don't draw the
        'velcro overlap
        If g_nLegStyle = 7 And sSide = "Left" Then
            WHUTILS.PR_DrawLine(xyVelcroOverLapProx, xyVelcroOverLapDist)
        Else
            'Label overlap so it can be deleted by the Zipper programme
            WHUTILS.PR_MakeXY(xyPt1, xyVelcroOverLapProx.X, xyVelcroOverLapProx.Y + 2.5)
            WHUTILS.PR_MakeXY(xyPt2, xyVelcroOverLapDist.X, xyVelcroOverLapDist.Y + 2.5)
            WHUTILS.PR_DrawLine(xyVelcroOverLapProx, xyPt1)
            WHUTILS.PR_AddDBValueToLast("Zipper", "VelcroOverlap")
            WHUTILS.PR_DrawLine(xyPt1, xyPt2)
            WHUTILS.PR_AddDBValueToLast("Zipper", "VelcroOverlap")
            WHUTILS.PR_DrawLine(xyVelcroOverLapDist, xyPt2)
            WHUTILS.PR_AddDBValueToLast("Zipper", "VelcroOverlap")

            WHUTILS.PR_SetLayer("Notes")
            WHUTILS.PR_DrawLine(xyVelcroOverLapProx, xyVelcroOverLapDist)
            WHUTILS.PR_AddDBValueToLast("Zipper", "VelcroOverlap")
            'Lingam
            'WHUTILS.PR_CalcMidPoint(xyPt1, xyVelcroOverLapDist, xyPt3)
            WHUTILS.PR_InsertSymbol("TextAsSymbol", xyPt3, 1, 90)
            WHUTILS.PR_AddDBValueToLast("Data", "2-1/2\"" VELCRO")
            WHUTILS.PR_AddDBValueToLast("Zipper", "VelcroOverlap")
            WHUTILS.PR_SetLayer("Template" & sSide)
        ...
        If g_nLegStyle = 7 Then
            WHUTILS.PR_SetLayer("Notes")
            WHUTILS.PR_SetTextData(WHUTILS.HORIZ_CENTER, WHUTILS.VERT_CENTER, 0.125, WHUTILS.CURRENT, WHUTILS.CURRENT)
            ...
            g_nCurrTextAngle = 90
            'Lingam
            'WHUTILS.PR_CalcMidPoint(xyEOSCL, xyVelcroOverLapProx, xyPt3)
            xyPt3.X = xyPt3.X - 0.5
            WHUTILS.PR_DrawText("ONE WAIST BAND", xyPt3, 0.125)
            'UPGRADE_WARNING: Couldn't resolve default property of object g_nCurrTextAngle. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
            g_nCurrTextAngle = 0
        ...
        WHUTILS.PR_MakeXY(xyCenTop, xyChapDatum.X + nTopRadius, xyChapDatum.y + nTopRadius)

        'Adjust to fit if the center of the arc is above the velcro line
        'or the end of the profile
        WHUTILS.PR_MakeXY(xyArcBottomPt, xyCenTop.X, xyChapDatum.y)
        If xyCenTop.y > xyVelcroOverLapDist.Y Then
            'Revise center of arc and Raduis
            WHUTILS.PR_MakeXY(xyArcBottomPt, xyCenTop.X, xyChapDatum.y)
            'Lingam
            'nLength = WHUTILS.FN_CalcLength(xyArcBottomPt, xyVelcroOverLapDist) / 2
            aAngle = 90 - WHUTILS.FN_CalcAngle(xyArcBottomPt, xyVelcroOverLapDist)
            nTopRaduisProx = nLength / System.Math.Cos(aAngle * (PI / 180))
            WHUTILS.PR_MakeXY(xyCenTopProx, xyArcBottomPt.X, xyArcBottomPt.Y + nTopRaduisProx)
            WHUTILS.PR_DrawArc(xyCenTopProx, xyArcBottomPt, xyVelcroOverLapDist)
        Else
            WHUTILS.PR_MakeXY(xyPt1, xyCenTop.X + nTopRadius, xyCenTop.Y)
            WHUTILS.PR_DrawLine(xyVelcroOverLapDist, xyPt1)
            WHUTILS.PR_DrawArc(xyCenTop, xyArcBottomPt, xyPt1)
        End If

        If xyCenTop.y > xyChapDatumOFF.Y Then
            'Revise center of arc and Raduis
            nLength = WHUTILS.FN_CalcLength(xyArcBottomPt, xyChapDatumOFF) / 2
            aAngle = WHUTILS.FN_CalcAngle(xyArcBottomPt, xyChapDatumOFF) - 90
            nTopRaduisDist = nLength / System.Math.Cos(aAngle * (PI / 180))
            WHUTILS.PR_MakeXY(xyCenTopDist, xyArcBottomPt.X, xyArcBottomPt.Y + nTopRaduisDist)
            WHUTILS.PR_DrawArc(xyCenTopDist, xyChapDatumOFF, xyArcBottomPt)
        Else
            WHUTILS.PR_MakeXY(xyPt1, xyCenTop.X - nTopRadius, xyCenTop.Y)
            WHUTILS.PR_DrawLine(xyChapDatumOFF, xyPt1)
            WHUTILS.PR_DrawArc(xyCenTop, xyPt1, xyArcBottomPt)
        End If
    ...
    Private Sub PR_WH_CutOut()
        ...
        If nLegStyle = 2 Then 'Brief
            ...
            If txtCrotchStyle.Text = "Open Crotch" And nOpenOff = 0.75 Then nBriefOff = 2.0#
