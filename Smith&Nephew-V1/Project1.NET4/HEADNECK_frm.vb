...
Friend Class HeadNeck
    ...
    Private Sub cmdDraw_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDraw.Click
        ...
        txtfNum.Text = CStr(FN_Open("c:\jobst\draw.d", txtPatientName, txtFileNo))
    ...
    Private Function FN_CirLinInt(ByRef xyStart As HEADNECK1.xy, ByRef xyEnd As HEADNECK1.xy, ByRef xyCen As HEADNECK1.xy, ByRef nRad As Double, ByRef xyInt As HEADNECK1.xy) As Short
        ...
        If nSlope = 0 Or nSlope = 180 Then
            ...
            While nSign > -2
                ...
                If nRoot >= BDYUTILS.min(xyStart.X, xyEnd.X) And nRoot <= max(xyStart.X, xyEnd.X) Then
                    xyInt.X = nRoot
                    xyInt.Y = xyStart.Y
                    FN_CirLinInt = True
                    Exit Function
        ...
        If nSlope = 90 Or nSlope = 270 Then
            ...
            While nSign > -2
                ...
                If nRoot >= BDYUTILS.min(xyStart.Y, xyEnd.Y) And nRoot <= max(xyStart.Y, xyEnd.Y) Then
                    xyInt.Y = nRoot
                    xyInt.X = xyStart.X
                    FN_CirLinInt = True
                    Exit Function
    ...
    Private Function FN_LinLinInt(ByRef xyLine1Start As HEADNECK1.xy, ByRef xyLine1End As HEADNECK1.xy, ByRef xyLine2Start As HEADNECK1.xy, ByRef xyLine2End As HEADNECK1.xy, ByRef xyInt As HEADNECK1.xy) As Short
        'Function:
        ...
        If (nX < BDYUTILS.min(xyLine1Start.X, xyLine1End.X) Or nX > max(xyLine1Start.X, xyLine1End.X)) Then Exit Function
        ...
        If (nY < BDYUTILS.min(xyLine1Start.Y, xyLine1End.Y) Or nY > max(xyLine1Start.Y, xyLine1End.Y)) Then Exit Function
        ...
        If (nX < BDYUTILS.min(xyLine2Start.X, xyLine2End.X) Or nX > max(xyLine2Start.X, xyLine2End.X)) Then Exit Function
        ...
        If (nY < BDYUTILS.min(xyLine2Start.Y, xyLine2End.Y) Or nY > max(xyLine2Start.Y, xyLine2End.Y)) Then Exit Function
    ...
    Private Sub HeadNeck_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        ...
        ' Load Fabric File
        filenum = FreeFile()
        FileOpen(filenum, HEADNECK1.g_sPathJOBST & "\fabric.dat", VB.OpenMode.Input)
    ...
    Private Sub PR_DrawChin()
        ...
        ''To Draw Polyline
        Dim PolyStart(11), PolyEnd(11) As Double
        PolyStart(1) = xyChinStart.X
        PolyEnd(1) = xyChinStart.Y
        For ii = 1 To 10
            PolyStart(ii + 1) = ChinProfileX(ii)
            PolyEnd(ii + 1) = ChinProfileY(ii)
        Next
        PR_DrawPoly(PolyStart, PolyEnd, 11)
        PR_AddEntityID("Chin")
    ...
    Private Sub PR_DrawCSBackCurve(ByRef xyBacklineBot As HEADNECK1.xy, ByRef xyFacelineTop As HEADNECK1.xy, ByRef xyChinStrapTopFront As HEADNECK1.xy, ByRef xyNeckTopVelcro As HEADNECK1.xy)
        ...
        PR_DrawPoly(ChinStrapBackX, ChinStrapBackY, 17)
    ...
    Private Sub PR_DrawCSChin(ByRef xyMouthFront As HEADNECK1.xy, ByRef xyMouthBack As HEADNECK1.xy, ByRef xyNeckBottomFront As HEADNECK1.xy, ByRef xyNeckBottomVelcro As HEADNECK1.xy)
        ...
        If optChinStrap.Checked = True Or optModifiedChinStrap.Checked = True Then
            ...
            Next ii
            PrintLine(CInt(txtfNum.Text), ");")
            PR_DrawPoly(OffsetChinX, OffsetChinY, 12)
            ...
            PR_DrawPoly(ChinStrapChinX, ChinStrapChinY, 12)
    ...
    Private Sub PR_DrawCSFrontCurve(ByRef xyLipStrapTop As HEADNECK1.xy, ByRef xyLipStrapBottom As HEADNECK1.xy, ByRef xyMouthFront As HEADNECK1.xy, ByRef xyMouthBack As HEADNECK1.xy, ByRef xyFacelineTop As HEADNECK1.xy, ByRef xyChinStrapTopFront As HEADNECK1.xy)
        ...
        PrintLine(CInt(txtfNum.Text), ");")
        PR_AddEntityID("ChinStrapFront")
        PR_DrawPoly(ChinStrapFrontX, ChinStrapFrontY, 8)
    ...
    Private Sub PR_DrawCSLipCovering()
        ...
        If chkLipCovering.CheckState = 1 Then
            ...
            If xy1.X > xyLipCoverBottomMid.X Then
                ...
                ii = BDYUTILS.FN_CirLinInt(xy1, xy2, xyLipCoveringBotCen, FN_CalcLength(xyLipCoverBottomRight, xyLipCoveringBotCen), xy2)
            Else
            ...
            PrintLine(CInt(txtfNum.Text), ");")
            PR_AddEntityID("LipStrapChinConstructLine")
            Dim StrapX(10), StrapY(10) As Double
            For i = 3 To 12
                StrapX(i - 2) = ChinStrapChinX(i)
                StrapY(i - 2) = ChinStrapChinY(i)
            Next i
            PR_DrawPoly(StrapX, StrapY, 10)
            PR_SetLayer("TemplateLeft")
    ...
    Private Sub PR_DrawCSModBackCurve(ByRef xyNeckTopVelcro As HEADNECK1.xy, ByRef xyChinStrapTopFront As HEADNECK1.xy)
        ...
        PrintLine(CInt(txtfNum.Text), ");")
        PR_AddEntityID("ChinStrapBack")
        PR_DrawPoly(ChinStrapBackX, ChinStrapBackY, 8)
    ...
    Private Sub PR_DrawLipCovering()
        ...
        If chkLipCovering.CheckState = 1 And optChinStrap.Checked = False And optModifiedChinStrap.Checked = False Then
            ...
            PR_MakeXY(xy2, xy1.X, xyLipCoveringBotCen.Y)
            ii = BDYUTILS.FN_CirLinInt(xy1, xy2, xyLipCoveringBotCen, FN_CalcLength(xyLipCoveringBotCen, xyFlapBottomRight), xy2)
            PR_DrawLine(xy1, xy2)
            length = FN_CalcLength(xy1, xy2) + 0.25
            'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
            VelcroText = FN_InchesToText(ARMEDDIA1.fnRoundInches(length))
    ...
    Private Sub PR_DrawNoseCovering()
        ...
        If chkNoseCovering.CheckState = 1 Then
            ...
            PrintLine(CInt(txtfNum.Text), ");")
            ''To Draw Polyline
            Dim xyStartPt(4), xyEndPt(4) As Double
            For ii = 0 To 3
                PR_CalcPolar(NostralPt1, aStartAngle - (aAngleInc * ii), nRad, NostralPt2)
                xyStartPt(ii + 1) = NostralPt2.X
                xyEndPt(ii + 1) = NostralPt2.Y
            Next
            PR_DrawPoly(xyStartPt, xyEndPt, 4)
