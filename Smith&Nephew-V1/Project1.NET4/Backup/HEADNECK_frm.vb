...
Friend Class HeadNeck
	...
	Dim xyNoseDiagTop, xyNoseDiagBot As xy 'Used DrawNose & DrawEyeFlap
	...
	Private Sub cmdClose_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdClose.Click
		...
		Else
			...
			If Response = 6 Then
				...
				txtfNum.Text = FN_OpenSave("c:\jobst\draw.d", txtPatientName, txtFileNo)
				PR_SaveDetails()
	...
	Private Sub cmdDraw_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdDraw.Click
		...
		txtfNum.Text = CStr(ARMDIA1.FN_Open("c:\jobst\draw.d", txtPatientName, txtFileNo))
	...
	Private Function FN_CalcCirCirInt(ByRef xyCen1 As xy, ByRef nRad1 As Double, ByRef xyCen2 As xy, ByRef nRad2 As Double, ByRef xyInt1 As xy, ByRef xyInt2 As xy) As Short
		...
		nLength = ARMDIA1.FN_CalcLength(xyCen1, xyCen2)
		...
		aAngle = ARMDIA1.FN_CalcAngle(xyCen1, xyCen2)
		...
		ARMDIA1.PR_CalcPolar(xyCen1, aAngle, nRad1, xyInt1)
		
		aAngle = aAngle + aTheta
		ARMDIA1.PR_CalcPolar(xyCen1, aAngle, nRad1, xyInt2)
	...
	Private Function FN_CmToInches(ByRef flag As Object, ByRef cm As Object) As Object
	...
	Private Function FN_LinLinInt(ByRef xyLine1Start As xy, ByRef xyLine1End As xy, ByRef xyLine2Start As xy, ByRef xyLine2End As xy, ByRef xyInt As xy) As Short
		'Function:
		...
		If (nX < BDYUTILS.min(xyLine1Start.X, xyLine1End.X) Or nX > ARMDIA1.max(xyLine1Start.X, xyLine1End.X)) Then Exit Function
		...
		If (nY < BDYUTILS.min(xyLine1Start.Y, xyLine1End.Y) Or nY > ARMDIA1.max(xyLine1Start.Y, xyLine1End.Y)) Then Exit Function
		...
		If (nX < BDYUTILS.min(xyLine2Start.X, xyLine2End.X) Or nX > ARMDIA1.max(xyLine2Start.X, xyLine2End.X)) Then Exit Function
		...
		If (nY < BDYUTILS.min(xyLine2Start.Y, xyLine2End.Y) Or nY > ARMDIA1.max(xyLine2Start.Y, xyLine2End.Y)) Then Exit Function
	...
	Private Function FN_Open(ByRef sDrafixFile As String, ByRef sName As Object, ByRef sPatientFile As Object) As Short
		...
		FileOpen(CInt(txtfNum.Text), sDrafixFile, OpenMode.Output)
		...
		PrintLine(CInt(txtfNum.Text), "//Patient - " & txtPatientName.Text & CC & " " & txtFileNo.Text & CC & " Head & Neck ")
		...
		PrintLine(CInt(txtfNum.Text), "XY     xyTitleBoxOrigin,xyTitleOrigin, xyStart,TitleOrigin, xyTitleScale, xyOrigin;")
		...
		PrintLine(CInt(txtfNum.Text), "Table(" & QQ & "add" & QCQ & "field" & QCQ & "ID" & QCQ & "string" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Table(" & QQ & "add" & QCQ & "field" & QCQ & "HeadNeck" & QCQ & "string" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Table(" & QQ & "add" & QCQ & "field" & QCQ & "units" & QCQ & "string" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Table(" & QQ & "add" & QCQ & "field" & QCQ & "Data" & QCQ & "string" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Table(" & QQ & "add" & QCQ & "field" & QCQ & "WorkOrder" & QCQ & "string" & QQ & ");")
		
		'Text data
		PrintLine(CInt(txtfNum.Text), "SetData(" & QQ & "TextHorzJust" & QC & txtCurrTextHorizJust.Text & ");")
		PrintLine(CInt(txtfNum.Text), "SetData(" & QQ & "TextVertJust" & QC & txtCurrTextVertJust.Text & ");")
		PrintLine(CInt(txtfNum.Text), "SetData(" & QQ & "TextHeight" & QC & txtCurrTextHt.Text & ");")
		PrintLine(CInt(txtfNum.Text), "SetData(" & QQ & "TextAspect" & QC & txtCurrTextAspect.Text & ");")
		PrintLine(CInt(txtfNum.Text), "SetData(" & QQ & "TextFont" & QC & txtCurrTextFont.Text & ");")
		
		'Get Start point
		PrintLine(CInt(txtfNum.Text), "GetUser (" & QQ & "xy" & QCQ & "Indicate Start Point" & QC & "&xyStart);")
		
		'Display Hour Glass symbol
		PrintLine(CInt(txtfNum.Text), "Display (" & QQ & "cursor" & QCQ & "wait" & QCQ & "Drawing" & QQ & ");")
		
		'Clear user selections etc
		PrintLine(CInt(txtfNum.Text), "UserSelection (" & QQ & "clear" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Execute (" & QQ & "menu" & QCQ & "SetStyle" & QC & "Table(" & QQ & "find" & QCQ & "style" & QCQ & "bylayer" & QQ & "));")
		
		'Place a marker at the start point later use
		ARMDIA1.PR_SetLayer("Construct")
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "marker" & QCQ & "cross" & QC & "xyStart" & CC & "0.125);")
		ARMDIA1.PR_AddEntityID("OriginMarker")
		' Update data base fields
		PrintLine(CInt(txtfNum.Text), "SetDBData(hEnt," & QQ & "Fabric" & QCQ & cboFabric.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hEnt," & QQ & "WorkOrder" & QCQ & txtWorkOrder.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hEnt," & QQ & "Data" & QCQ & txtData.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hEnt," & QQ & "units" & QCQ & txtUnits.Text & QQ & ");")
		
		'Set values for use futher on by other macros
		PrintLine(CInt(txtfNum.Text), "xyOrigin = xyStart" & ";")
		PrintLine(CInt(txtfNum.Text), "sFileNo = " & QQ & txtFileNo.Text & QQ & ";")
	...
	Private Function FN_OpenSave(ByRef sDrafixFile As String, ByRef sName As Object, ByRef sPatientFile As Object) As Object
		...
		'Clear user selections etc
		PrintLine(CInt(txtfNum.Text), "UserSelection (" & QQ & "clear" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "UserSelection (" & QQ & "update" & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "Execute (" & QQ & "menu" & QCQ & "SetStyle" & QC & "Table(" & QQ & "find" & QCQ & "style" & QCQ & "bylayer" & QQ & "));")
	...
	Private Sub HeadNeck_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
		...
		' Load Fabric File
		filenum = FreeFile
		FileOpen(filenum, g_sPathJOBST & "\fabric.dat", OpenMode.Input)
	...
	Private Sub PR_AddEntityID(ByRef sType As String)
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		'UPGRADE_WARNING: Couldn't resolve default property of object CC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		PrintLine(CInt(txtfNum.Text), "if (hEnt) SetDBData( hEnt," & QQ & "ID" & QQ & CC & QQ & sID & QQ & ");")
		'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		'UPGRADE_WARNING: Couldn't resolve default property of object CC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		PrintLine(CInt(txtfNum.Text), "if (hEnt) SetDBData( hEnt," & QQ & "Data" & QQ & CC & QQ & sData & QQ & ");")
		
	End Sub
	
	Private Sub PR_CalcPolar(ByRef xyStart As xy, ByVal nAngle As Double, ByRef nLength As Double, ByRef xyReturn As xy)
	...
	Private Sub PR_DrawArc(ByRef xyCen As xy, ByRef xyArcStart As xy, ByRef xyArcEnd As xy)
		...
		nEndAng = ARMDIA1.FN_CalcAngle(xyCen, xyArcEnd)
		'UPGRADE_WARNING: Couldn't resolve default property of object nStartAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		'UPGRADE_WARNING: Couldn't resolve default property of object nEndAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		...
		'UPGRADE_WARNING: Couldn't resolve default property of object nStartAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		'UPGRADE_WARNING: Couldn't resolve default property of object nRad. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCen.X) & CC & "xyStart.y +" & Str(xyCen.Y) & CC & Str(nRad) & CC & Str(nStartAng) & CC & Str(nDeltaAng) & ");")
	...
	Private Sub PR_DrawChin()
		...
		Dim xyMouthLeft, xyMouthRight As xy
		Dim xy4, xy2, xy1, xy3, xyTmp As xy
		Dim xyTopCen, xyBotCen, xyChinStart As xy
		...
		ARMDIA1.PR_MakeXY(xyMouthLeft, Tempx, TempY)
		
		Tempx = -(System.Math.Abs(LeftArcAdjOut(CInt(txtRadiusNo.Text))) - MouthWidth)
		ARMDIA1.PR_MakeXY(xyMouthRight, Tempx, TempY)
		...
		If (optOpenFaceMask.Checked = True) And chkEyes.CheckState = 1 Then
			ARMDIA1.PR_MakeXY(xyMouthRight, Tempx - 0.125, TempY)
		End If
		
		ARMDIA1.PR_DrawLine(xyMouthLeft, xyMouthRight)
		ARMDIA1.PR_AddEntityID("Mouth")
		...
		ARMDIA1.PR_MakeXY(xyChinStart, Val(txtChinLeftBotX.Text), Val(txtChinLeftBotY.Text))
		ARMDIA1.PR_MakeXY(xy1, xyChinStart.X, xyChinStart.Y + (nNeckOffset - 1))
		ARMDIA1.PR_MakeXY(xy2, xy1.X, xyChinStart.Y + nNeckOffset)
		ARMDIA1.PR_MakeXY(xy4, -System.Math.Abs(LeftArcAdjOut(CInt(txtRadiusNo.Text))) + 0.375, Val(txtMouthHeight.Text))
		ARMDIA1.PR_MakeXY(xy3, xy4.X, xy4.Y - 1)
		
		'Fit curve to three lines defined by the 4 points above
		n1 = ARMDIA1.FN_CalcLength(xy1, xy2)
		n2 = ARMDIA1.FN_CalcLength(xy2, xy3)
		n3 = ARMDIA1.FN_CalcLength(xy3, xy4)
		nValue = ARMDIA1.FN_CalcLength(xy1, xy3)
		nValue = ((n1 ^ 2 + n2 ^ 2) - nValue ^ 2) / (2 * n1 * n2)
		rAngle = BDYUTILS.Arccos(nValue) / 2
		...
		ARMDIA1.PR_CalcPolar(xy2, ARMDIA1.FN_CalcAngle(xy2, xy3), (2 * (n1 / 3)), xyTmp)
		ChinProfileX(5) = xyTmp.X
		ChinProfileY(5) = xyTmp.Y
		
		ARMDIA1.PR_MakeXY(xyTmp, ChinProfileX(2), ChinProfileY(2))
		ARMDIA1.PR_CalcPolar(xyTmp, 180, nBotRadius, xyBotCen)
		ARMDIA1.PR_MakeXY(xyTmp, ChinProfileX(5), ChinProfileY(5))
		aInc = ARMDIA1.FN_CalcAngle(xyBotCen, xyTmp) / 3
		
		For ii = 3 To 4
			ARMDIA1.PR_CalcPolar(xyBotCen, aInc, nBotRadius, xyTmp)
			aInc = aInc + aInc
			ChinProfileX(ii) = xyTmp.X
			ChinProfileY(ii) = xyTmp.Y
		Next ii
		
		ARMDIA1.PR_CalcPolar(xy3, ARMDIA1.FN_CalcAngle(xy3, xy2), (n3 / 2), xyTmp)
		ChinProfileX(6) = xyTmp.X
		ChinProfileY(6) = xyTmp.Y
		
		ARMDIA1.PR_CalcPolar(xyTmp, ARMDIA1.FN_CalcAngle(xy3, xy2) + 90, nTopRadius, xyTopCen)
		aAngle = ARMDIA1.FN_CalcAngle(xyTopCen, xyTmp)
		aInc = (aAngle - 180) / 3
		
		For ii = 7 To 9
			aAngle = aAngle - aInc
			ARMDIA1.PR_CalcPolar(xyTopCen, aAngle, nTopRadius, xyTmp)
			ChinProfileX(ii) = xyTmp.X
			ChinProfileY(ii) = xyTmp.Y
		Next ii
		...
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyChinStart.X) & CC & "xyStart.y+" & Str(xyChinStart.Y))
		For ii = 1 To 10
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinProfileX(ii)) & CC & "xyStart.y+" & Str(ChinProfileY(ii)))
		Next ii
		PrintLine(CInt(txtfNum.Text), ");")
		ARMDIA1.PR_AddEntityID("Chin")
	...
	Private Sub PR_DrawChinCollar()
		...
		Dim xyBotInnerLeft, xyBotInnerRight As xy
		Dim xyTopInnerLeft, xyTopInnerRight As xy
		Dim xyBotOuterLeft, xyBotOuterRight As xy
		Dim xyTopOuterLeft, xyTopOuterRight As xy
		Dim xyTopInnerContour, xyBotInnerContour As xy
		Dim xyTopOuterContour, xyBotOuterContour As xy
		...
		If optChinCollar.Checked = True Or optContouredChinCollar.Checked = True Then
			...
			ARMDIA1.PR_MakeXY(xyBotInnerLeft, 0, 0)
			ARMDIA1.PR_MakeXY(xyTopInnerLeft, 0, YDist)
			ARMDIA1.PR_MakeXY(xyTopOuterLeft, 0, 0)
			ARMDIA1.PR_MakeXY(xyBotOuterLeft, 0, -YDist)
			
			If optContouredChinCollar.Checked = True Then
				ARMDIA1.PR_MakeXY(xyBotInnerContour, InnerXContourDist, 0)
				ARMDIA1.PR_MakeXY(xyBotInnerRight, InnerXDist, YContourStart)
				ARMDIA1.PR_MakeXY(xyTopInnerRight, InnerXDist, YContourEnd)
				ARMDIA1.PR_MakeXY(xyTopInnerContour, InnerXContourDist, YDist)
				ARMDIA1.PR_MakeXY(xyTopOuterContour, OuterXContourDist, 0)
				ARMDIA1.PR_MakeXY(xyTopOuterRight, OuterXDist, -YContourStart)
				ARMDIA1.PR_MakeXY(xyBotOuterRight, OuterXDist, -YContourEnd)
				ARMDIA1.PR_MakeXY(xyBotOuterContour, OuterXContourDist, -YDist)
			Else
				ARMDIA1.PR_MakeXY(xyBotInnerRight, InnerXDist, 0)
				ARMDIA1.PR_MakeXY(xyTopInnerRight, InnerXDist, YDist)
				ARMDIA1.PR_MakeXY(xyTopOuterRight, OuterXDist, 0)
				ARMDIA1.PR_MakeXY(xyBotOuterRight, OuterXDist, -YDist)
			End If
			
			' Draw Inner Collar
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotInnerLeft.X) & CC & "xyStart.y+" & Str(xyBotInnerLeft.Y))
			If optContouredChinCollar.Checked = True Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotInnerContour.X) & CC & "xyStart.y+" & Str(xyBotInnerContour.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotInnerRight.X) & CC & "xyStart.y+" & Str(xyBotInnerRight.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopInnerRight.X) & CC & "xyStart.y+" & Str(xyTopInnerRight.Y))
			If optContouredChinCollar.Checked = True Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopInnerContour.X) & CC & "xyStart.y+" & Str(xyTopInnerContour.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopInnerLeft.X) & CC & "xyStart.y+" & Str(xyTopInnerLeft.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotInnerLeft.X) & CC & "xyStart.y+" & Str(xyBotInnerLeft.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("InnerChinCollar")
			
			' Draw Outer Collar
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopOuterLeft.X) & CC & "xyStart.y+" & Str(xyTopOuterLeft.Y))
			If optContouredChinCollar.Checked = True Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopOuterContour.X) & CC & "xyStart.y+" & Str(xyTopOuterContour.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopOuterRight.X) & CC & "xyStart.y+" & Str(xyTopOuterRight.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotOuterRight.X) & CC & "xyStart.y+" & Str(xyBotOuterRight.Y))
			If optContouredChinCollar.Checked = True Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotOuterContour.X) & CC & "xyStart.y+" & Str(xyBotOuterContour.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyBotOuterLeft.X) & CC & "xyStart.y+" & Str(xyBotOuterLeft.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyTopOuterLeft.X) & CC & "xyStart.y+" & Str(xyTopOuterLeft.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("OuterChinCollar")
			
			ARMDIA1.PR_MakeXY(xyTextPoint, xyTopInnerLeft.X + 1, xyTopInnerLeft.Y - 0.5)
			PR_TemplateDetails(xyTextPoint, "INNER Chin Collar")
			ARMDIA1.PR_MakeXY(xyTextPoint, xyBotInnerLeft.X + 0.35, xyBotInnerLeft.Y + 1.75)
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawText("Fold", xyTextPoint, 0.25, 90)
			
			ARMDIA1.PR_MakeXY(xyTextPoint, xyTopOuterLeft.X + 1, xyTopOuterLeft.Y - 0.5)
			PR_TemplateDetails(xyTextPoint, "OUTER Chin Collar")
			ARMDIA1.PR_MakeXY(xyTextPoint, xyBotOuterLeft.X + 0.35, xyBotOuterLeft.Y + 1.75)
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawText("Fold", xyTextPoint, 0.25, 90)
			ARMDIA1.PR_SetLayer("TemplateLeft")
	...
	Private Sub PR_DrawChinStrap()
		...
		Dim xyNeckTopFront, xyNeckTopBack As xy
		Dim xyNeckBottomFront, xyNeckBottomBack As xy
		Dim xyNeckTopVelcro, xyNeckBottomVelcro As xy
		Dim xyChinStrapTopFront, xyChinStrapTopBack As xy
		Dim xyFacelineTop, xyFacelineBot As xy
		Dim xyBacklineTop, xyBacklineBot As xy
		Dim xyMouthFront, xyMouthBack As xy
		Dim xyLipStrapTop, xyLipStrapBottom As xy
		Dim xyStrapText, xyTextPoint As xy
		...
		If optChinStrap.Checked = True Or optModifiedChinStrap.Checked = True Then
			...
			ARMDIA1.PR_MakeXY(xyNeckTopBack, XDistance, 0)
			If xyNeckTopBack.X > HeadBack Then
				XOffset = -(xyNeckTopBack.X - HeadBack)
				ARMDIA1.PR_MakeXY(xyNeckTopBack, xyNeckTopBack.X + XOffset, 0)
			End If
			
			ARMDIA1.PR_MakeXY(xyNeckTopFront, -XDistance + XOffset, 0)
			...
			ARMDIA1.PR_MakeXY(xyNeckBottomFront, xyNeckTopFront.X, -NeckDepth)
			ARMDIA1.PR_MakeXY(xyNeckBottomBack, xyNeckTopBack.X, -NeckDepth)
			ARMDIA1.PR_MakeXY(xyNeckTopVelcro, xyNeckTopBack.X + 1, xyNeckTopBack.Y)
			ARMDIA1.PR_MakeXY(xyNeckBottomVelcro, xyNeckBottomBack.X + 1 + XOffset, xyNeckBottomBack.Y)
			
			ARMDIA1.PR_DrawLine(xyNeckBottomVelcro, xyNeckTopVelcro)
			ARMDIA1.PR_AddEntityID("VelcroBack")
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawLine(xyNeckTopBack, xyNeckBottomBack)
			ARMDIA1.PR_SetLayer("TemplateLeft")
			ARMDIA1.PR_AddEntityID("NeckBack")
			
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_MakeXY(xyStrapText, xyNeckTopBack.X, xyNeckTopBack.Y - 0.25)
			...
			ARMDIA1.PR_DrawText(VelcroText, xyStrapText, 0.1, 0)
			
			ARMDIA1.PR_MakeXY(xyStrapText, xyNeckTopBack.X + 0.1, xyNeckTopBack.Y - 0.5)
			ARMDIA1.PR_DrawText(" Velcro", xyStrapText, 0.1, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
			
			' Set Top Front Point
			YDistance = Val(txtCSChinAngle.Text)
			ARMDIA1.PR_MakeXY(xyChinStrapTopFront, 0, YDistance)
			
			' Set FaceLine and Back Line
			XDistance = Val(txtCSChinAngle.Text) / 2
			ARMDIA1.PR_MakeXY(xyFacelineTop, xyChinStrapTopFront.X - XDistance, xyChinStrapTopFront.Y)
			ARMDIA1.PR_MakeXY(xyBacklineTop, xyChinStrapTopFront.X + XDistance, xyChinStrapTopFront.Y)
			
			ARMDIA1.PR_MakeXY(xyFacelineBot, xyFacelineTop.X, 0)
			ARMDIA1.PR_MakeXY(xyBacklineBot, xyBacklineTop.X, 0)
			
			' Set Mouth and lipstrap
			YDistance = Val(txtCSChintoMouth.Text)
			XDistance = Val(txtCSMouthWidth.Text)
			ARMDIA1.PR_MakeXY(xyMouthFront, xyFacelineBot.X, xyFacelineBot.Y + YDistance)
			ARMDIA1.PR_MakeXY(xyMouthBack, xyMouthFront.X + XDistance, xyMouthFront.Y)
			
			ARMDIA1.PR_MakeXY(xyLipStrapTop, xyMouthFront.X - 0.125, xyMouthFront.Y + 0.75)
			ARMDIA1.PR_MakeXY(xyLipStrapBottom, xyMouthFront.X - 0.125, xyMouthFront.Y)
			...
			' Draw Top
			If optChinStrap.Checked = True Then
				ARMDIA1.PR_MakeXY(xyChinStrapTopBack, xyChinStrapTopFront.X + 2, xyChinStrapTopFront.Y)
			ElseIf optModifiedChinStrap.Checked = True Then 
				ARMDIA1.PR_MakeXY(xyChinStrapTopBack, xyChinStrapTopFront.X + 2.5, xyChinStrapTopFront.Y)
			End If
			ARMDIA1.PR_DrawLine(xyChinStrapTopFront, xyChinStrapTopBack)
			ARMDIA1.PR_AddEntityID("ChinStrapTop")
			
			' Draw Velcro Top
			ARMDIA1.PR_DrawLine(xyNeckTopBack, xyNeckTopVelcro)
			ARMDIA1.PR_AddEntityID("VelcroTop")
			
			' Set and draw chin
			PR_DrawCSChin(xyMouthFront, xyMouthBack, xyNeckBottomFront, xyNeckBottomVelcro)
			
			' Calculate Strap Length
			TempLen = ARMEDDIA1.fnRoundInches(ARMDIA1.FN_CalcLength(xyChinStrapTopBack, xyNeckTopBack))
			...
			ARMDIA1.PR_MakeXY(xyStrapText, xyChinStrapTopFront.X + 0.1, xyChinStrapTopBack.Y - 0.5)
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawText("Strap Length: " & FN_InchesToText(StrapLength) & "\""", xyStrapText, 0.1, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
			...
			' Add Patient Details
			ARMDIA1.PR_MakeXY(xyTextPoint, -1.5, 0.75)
			PR_TemplateDetails(xyTextPoint, "Patient Details")
		End If
		
	End Sub
	
	Private Sub PR_DrawCSBackCurve(ByRef xyBacklineBot As xy, ByRef xyFacelineTop As xy, ByRef xyChinStrapTopFront As xy, ByRef xyNeckTopVelcro As xy)
		
		Dim XDistance, YDistance As Double
		Dim nRadius, aAngle, XStep, YStep, aInc, nOffSet As Double
		Dim ii As Short
		Dim xyArcCen, xyTopEar, xyBotEar, xyTmp As xy
		...
		ARMDIA1.PR_MakeXY(xyTopEar, 0, CDbl(txtCSEarTopHeight.Text))
		ARMDIA1.PR_MakeXY(xyBotEar, 0, CDbl(txtCSEarBotHeight.Text))
		...
		ARMDIA1.PR_MakeXY(xyTmp, ChinStrapBackX(5), ChinStrapBackY(5))
		aAngle = ARMDIA1.FN_CalcAngle(xyTopEar, xyTmp)
		ARMDIA1.PR_CalcPolar(xyTopEar, aAngle, nOffSet, xyTmp)
		
		aAngle = (aAngle + 90) / 2
		nRadius = nOffSet * System.Math.Tan(aAngle * (PI / 180))
		ARMDIA1.PR_MakeXY(xyArcCen, xyTopEar.X + nRadius, xyTopEar.Y - nOffSet)
		aAngle = ARMDIA1.FN_CalcAngle(xyArcCen, xyTmp)
		aInc = (180 - aAngle) / 3
		For ii = 6 To 9
			ARMDIA1.PR_CalcPolar(xyArcCen, aAngle, nRadius, xyTmp)
			aAngle = aAngle + aInc
		...
		ARMDIA1.PR_MakeXY(xyTmp, ChinStrapBackX(13), ChinStrapBackY(13))
		aAngle = ARMDIA1.FN_CalcAngle(xyBotEar, xyTmp)
		ARMDIA1.PR_CalcPolar(xyBotEar, aAngle, nOffSet, xyTmp)
		
		aAngle = ((360 - aAngle) + 90) / 2
		nRadius = nOffSet * System.Math.Tan(aAngle * (PI / 180))
		ARMDIA1.PR_MakeXY(xyArcCen, xyBotEar.X + nRadius, xyBotEar.Y + nOffSet)
		aAngle = ARMDIA1.FN_CalcAngle(xyArcCen, xyTmp)
		aInc = (aAngle - 180) / 3
		aAngle = 180
		For ii = 10 To 12
		...
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
		For ii = 1 To 17
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapBackX(ii)) & CC & "xyStart.y+" & Str(ChinStrapBackY(ii)))
		Next ii
		PrintLine(CInt(txtfNum.Text), ");")
		ARMDIA1.PR_AddEntityID("ChinStrapBack")
	...
	Private Sub PR_DrawCSChin(ByRef xyMouthFront As xy, ByRef xyMouthBack As xy, ByRef xyNeckBottomFront As xy, ByRef xyNeckBottomVelcro As xy)
		
		Dim xyChinTop, xyChinBottom As xy
		Dim xyBotCurve, xyTopCurve As xy
		Dim xyDiagEnd, xyDiagStart, xyTmp As xy
		...
		If optChinStrap.Checked = True Or optModifiedChinStrap.Checked = True Then
			...
			'Set and Draw Chin
			ARMDIA1.PR_MakeXY(xyChinTop, xyMouthFront.X + 0.25, xyMouthFront.Y)
			ARMDIA1.PR_MakeXY(xyChinBottom, xyNeckBottomFront.X + 0.25, xyNeckBottomFront.Y)
			...
			ARMDIA1.PR_MakeXY(xyBotCurve, ChinStrapChinX(3) - BotRadius, ChinStrapChinY(3))
			...
			ARMDIA1.PR_MakeXY(xyTopCurve, ChinStrapChinX(10) + TopRadius, ChinStrapChinY(10))
			...
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For ii = 1 To 12
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(OffsetChinX(ii)) & CC & "xyStart.y+" & Str(OffsetChinY(ii)))
			Next ii
			PrintLine(CInt(txtfNum.Text), ");")
			...
			ARMDIA1.PR_AddEntityID("Chin")
			ARMDIA1.PR_DrawLine(xyChinTop, xyMouthBack)
			ARMDIA1.PR_AddEntityID("ChinStrapMouth")
			ARMDIA1.PR_DrawLine(xyChinBottom, xyNeckBottomVelcro)
			ARMDIA1.PR_AddEntityID("ChinStrapBottom")
			
			If chkNeckElastic.CheckState = 1 Then
				ARMDIA1.PR_MakeXY(xyTmp, xyChinBottom.X + 0.4, xyChinBottom.Y + 0.5)
				ARMDIA1.PR_SetLayer("Notes")
				ARMDIA1.PR_DrawText("1\"" Elastic Sewn Underneath", xyTmp, 0.1, 0)
				ARMDIA1.PR_SetLayer("TemplateLeft")
			End If
			...
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For ii = 1 To 12
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapChinX(ii)) & CC & "xyStart.y+" & Str(ChinStrapChinY(ii)))
			Next ii
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LipStrapChinConstructLine")
	...
	Private Sub PR_DrawCSFrontCurve(ByRef xyLipStrapTop As xy, ByRef xyLipStrapBottom As xy, ByRef xyMouthFront As xy, ByRef xyMouthBack As xy, ByRef xyFacelineTop As xy, ByRef xyChinStrapTopFront As xy)
		...
		If chkLipStrap.CheckState = 1 Then
			...
			ARMDIA1.PR_DrawLine(xyLipStrapTop, xyLipStrapBottom)
			ARMDIA1.PR_AddEntityID("ChinStrapFront")
			ARMDIA1.PR_DrawLine(xyMouthBack, xyLipStrapBottom)
			ARMDIA1.PR_AddEntityID("Mouth")
		...
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
		For ii = 1 To 8
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapFrontX(ii)) & CC & "xyStart.y+" & Str(ChinStrapFrontY(ii)))
		Next ii
		PrintLine(CInt(txtfNum.Text), ");")
		ARMDIA1.PR_AddEntityID("ChinStrapFront")
	...
	Private Sub PR_DrawCSLipCovering()
		...
		'
		Dim xyLipCoverTopLeft, xyLipCoverTopRight As xy
		Dim xyLipCoverBottomRight, xyLipCoverBottomMid As xy
		Dim xy2, xy6, xyTmp, xy7, xy3, xy1 As xy
		...
		If chkLipCovering.CheckState = 1 Then
			...
			ARMDIA1.PR_MakeXY(xyLipCoverTopLeft, XPoint + 0.25, YPoint + 1.25)
			ARMDIA1.PR_MakeXY(xyLipCoverTopRight, -0.75, YPoint + 1.25)
			ARMDIA1.PR_MakeXY(xyLipCoverBottomRight, -0.75, 0.25)
			...
			'at xy6
			ARMDIA1.PR_MakeXY(xy6, ChinStrapChinX(6), ChinStrapChinY(6))
			ARMDIA1.PR_MakeXY(xy7, ChinStrapChinX(7), ChinStrapChinY(7))
			
			'Take intersection point at end of horizontal line
			DrawArcPart = True
			aStart = ARMDIA1.FN_CalcAngle(xyChinBotCen, xy6)
			aAngle = aStart - ((0.375 / (2 * PI * BotRadius)) * 360)
			ARMDIA1.PR_CalcPolar(xyChinBotCen, aAngle, BotRadius, xyLipCoverBottomMid)
			...
			For ii = 10 To 7 Step -1
				Select Case nn
					...
					Case 7
						nOffSet = 0.06
				End Select
				ARMDIA1.PR_MakeXY(xyTmp, ChinStrapChinX(ii) + 0.125, ChinStrapChinY(ii))
				ARMDIA1.PR_CalcPolar(xyChinTopCen, ARMDIA1.FN_CalcAngle(xyChinTopCen, xyTmp), ARMDIA1.FN_CalcLength(xyChinTopCen, xyTmp) - nOffSet, xyTmp)
			...
			If DrawArcPart Then
				...
				ARMDIA1.PR_CalcPolar(xyChinBotCen, aAngle + ((aStart - aAngle) / 3), BotRadius, xyTmp)
				ChinStrapLipX(10) = xyTmp.X + 0.125
				ChinStrapLipY(10) = xyTmp.Y
				
				ARMDIA1.PR_CalcPolar(xyChinBotCen, aAngle + (2 * ((aStart - aAngle) / 3)), BotRadius, xyTmp)
			...
			ARMDIA1.PR_MakeXY(Point1, xyLipCoverTopRight.X - 0.125, xyLipCoverTopRight.Y)
			ARMDIA1.PR_MakeXY(Point2, Point1.X - 0.125, Point1.Y)
			ARMDIA1.PR_DrawLine(Point1, Point2)
			ARMDIA1.PR_MakeXY(Point2, Point1.X, Point1.Y - 0.125)
			ARMDIA1.PR_DrawLine(Point1, Point2)
			
			ARMDIA1.PR_MakeXY(Point1, xyLipCoverBottomRight.X - 0.125, xyLipCoverBottomRight.Y)
			ARMDIA1.PR_MakeXY(Point2, Point1.X - 0.125, Point1.Y)
			ARMDIA1.PR_DrawLine(Point1, Point2)
			ARMDIA1.PR_MakeXY(Point2, Point1.X, Point1.Y + 0.125)
			ARMDIA1.PR_DrawLine(Point1, Point2)
			
			ARMDIA1.PR_MakeXY(Point1, xyLipCoverBottomMid.X - 0.125, xyLipCoverBottomMid.Y)
			ARMDIA1.PR_DrawLine(Point1, xyLipCoverBottomMid)
			
			ARMDIA1.PR_SetLayer("TemplateLeft")
			
			ARMDIA1.PR_MakeXY(xyLipCoverTopLeft, xyLipCoverTopLeft.X + XOffset, xyLipCoverTopLeft.Y)
			ARMDIA1.PR_MakeXY(xyLipCoverTopRight, xyLipCoverTopRight.X + XOffset, xyLipCoverTopRight.Y)
			ARMDIA1.PR_MakeXY(xyLipCoverBottomRight, xyLipCoverBottomRight.X + XOffset, xyLipCoverBottomRight.Y)
			ARMDIA1.PR_MakeXY(xyLipCoverBottomMid, xyLipCoverBottomMid.X + XOffset, xyLipCoverBottomMid.Y)
			
			nOffSet = ARMDIA1.FN_CalcLength(xyLipCoverBottomMid, xyLipCoverBottomRight) / 2
			aAngle = ARMDIA1.FN_CalcAngle(xyLipCoverBottomMid, xyLipCoverBottomRight)
			ARMDIA1.PR_CalcPolar(xyLipCoverBottomMid, aAngle, nOffSet, xyTmp)
			ARMDIA1.PR_CalcPolar(xyTmp, aAngle + 270, 9, xyLipCoveringBotCen)
			ARMDIA1.PR_DrawArc(xyLipCoveringBotCen, xyLipCoverBottomMid, xyLipCoverBottomRight)
			ARMDIA1.PR_AddEntityID("LipCoveringBot")
			...
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyLipCoverTopLeft.X) & CC & "xyStart.y+" & Str(xyLipCoverTopLeft.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyLipCoverTopRight.X) & CC & "xyStart.y+" & Str(xyLipCoverTopRight.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyLipCoverBottomRight.X) & CC & "xyStart.y+" & Str(xyLipCoverBottomRight.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LipCovering")
			
			'Draw chin area
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			If DrawArcPart Then
				For i = 1 To 11
					PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapLipX(i)) & CC & "xyStart.y+" & Str(ChinStrapLipY(i)))
				Next i
			Else
				For i = 1 To 8
					PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapLipX(i)) & CC & "xyStart.y+" & Str(ChinStrapLipY(i)))
			...
			End If
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("ChinStrapLipCoveringCurve")
			
			
			ARMDIA1.PR_MakeXY(TextPoint, (xyLipCoverTopLeft.X + 0.25), (xyLipCoverTopLeft.Y - 0.1))
			PR_TemplateDetails(TextPoint, "Chin Strap Lip Covering")
			
			' Add Velcro
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_MakeXY(xy1, xyLipCoverTopRight.X - 0.75, xyLipCoverTopRight.Y)
			
			If xy1.X > xyLipCoverBottomMid.X Then
				ARMDIA1.PR_MakeXY(xy2, xy1.X, xyLipCoveringBotCen.Y)
				ii = BDYUTILS.FN_CirLinInt(xy1, xy2, xyLipCoveringBotCen, ARMDIA1.FN_CalcLength(xyLipCoverBottomRight, xyLipCoveringBotCen), xy2)
			Else
				ARMDIA1.PR_MakeXY(xy2, xy1.X, xyChinBotCen.Y)
				ARMDIA1.PR_MakeXY(xyTmp, (xyChinBotCen.X + XOffset) + 0.125, xyChinBotCen.Y)
				ii = BDYUTILS.FN_CirLinInt(xy1, xy2, xyTmp, BotRadius, xy2)
				'PR_DrawMarker xyTmp
				'PR_DrawText "xyTmp", xyTmp, .05, 0
			End If
			
			ARMDIA1.PR_DrawLine(xy1, xy2)
			length = ARMDIA1.FN_CalcLength(xy1, xy2) + 0.25
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = FN_InchesToText(ARMEDDIA1.fnRoundInches(length))
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = VelcroText & "\""  Velcro"
			ARMDIA1.PR_MakeXY(TextPoint, xy1.X - 0.25, xy1.Y - 0.25)
			ARMDIA1.PR_DrawText(VelcroText, TextPoint, 0.1, -90)
			length = ARMDIA1.FN_CalcLength(xyLipCoverTopRight, xyLipCoverBottomRight) + 0.25
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = FN_InchesToText(ARMEDDIA1.fnRoundInches(length))
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = VelcroText & "\""  Velcro"
			ARMDIA1.PR_MakeXY(TextPoint, xyLipCoverTopRight.X - 0.25, xy1.Y - 0.25)
			ARMDIA1.PR_DrawText(VelcroText, TextPoint, 0.1, -90)
			...
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For i = 3 To 12
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinStrapChinX(i) + XOffset) & CC & "xyStart.y+" & Str(ChinStrapChinY(i)))
			Next i
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LipStrapChinConstructLine")
	...
	Private Sub PR_DrawCSModBackCurve(ByRef xyNeckTopVelcro As xy, ByRef xyChinStrapTopFront As xy)
	...
	Private Sub PR_DrawEarFlaps()
		...
		Dim xyFlapTopLeft, xyFlapTopRight As xy
		Dim xyFlapBottomLeft, xyFlapBottomRight As xy
		Dim FlapX, DartAng, FlapY As Double
		Dim Point2, Point1, Notes As xy
		Dim PI As Double
		Dim Y1, X1, X2, y2 As Double
		Dim RightXOffSet, LeftXOffSet, YOffSet As Double
		Dim Left3, Left1, Left2, Left4 As xy
		Dim Right3, Right1, Right2, Right4 As xy
		...
		If optModifiedChinStrap.Checked = True Or optFaceMask.Checked = True Or optOpenFaceMask.Checked = True Then
			...
			If optModifiedChinStrap.Checked = False Then
				...
				ARMDIA1.PR_MakeXY(Point1, X1, Y1)
				ARMDIA1.PR_MakeXY(Point2, X2, y2)
				DartAng = ARMDIA1.FN_CalcAngle(Point1, Point2)
				DartAng = DartAng * (PI / 180)
				FlapX = System.Math.Abs(CDbl(txtDartStartX.Text)) + 2
				FlapY = FlapX * System.Math.Tan(DartAng)
				ARMDIA1.PR_MakeXY(xyFlapBottomRight, Point1.X + FlapX, Point1.Y + FlapY + 0.25)
				ARMDIA1.PR_MakeXY(xyFlapBottomLeft, -0.625, xyFlapBottomRight.Y)
			Else
				ARMDIA1.PR_MakeXY(xyFlapBottomRight, 2.1875, 0.25)
				ARMDIA1.PR_MakeXY(xyFlapBottomLeft, -0.375, 0.25)
			End If
			' Set OffSets
			LeftXOffSet = -3
			RightXOffSet = 1.625
			...
			If chkLeftEarFlap.CheckState = 1 Then
				' Set Top Points
				If optModifiedChinStrap.Checked = False Then
					ARMDIA1.PR_MakeXY(xyFlapTopLeft, -0.625, CDbl(txtTopLeftEar.Text) + 0.125)
					ARMDIA1.PR_MakeXY(xyFlapTopRight, 2, CDbl(txtTopLeftEar.Text) + 0.125)
				Else
					ARMDIA1.PR_MakeXY(xyFlapTopLeft, -0.375, CDbl(txtTopLeftEar.Text) + 0.125)
					ARMDIA1.PR_MakeXY(xyFlapTopRight, 2.1875, CDbl(txtTopLeftEar.Text) + 0.125)
				End If
				
				'Indicate position on Pattern
				ARMDIA1.PR_SetLayer("Notes")
				ARMDIA1.PR_MakeXY(Point1, xyFlapTopLeft.X + 0.125, xyFlapTopLeft.Y)
				ARMDIA1.PR_MakeXY(Point2, xyFlapTopLeft.X, xyFlapTopLeft.Y - 0.125)
				ARMDIA1.PR_DrawLine(Point1, xyFlapTopLeft)
				ARMDIA1.PR_DrawLine(Point2, xyFlapTopLeft)
				
				ARMDIA1.PR_MakeXY(Point1, xyFlapTopRight.X - 0.125, xyFlapTopRight.Y)
				ARMDIA1.PR_MakeXY(Point2, xyFlapTopRight.X, xyFlapTopRight.Y - 0.125)
				ARMDIA1.PR_DrawLine(Point1, xyFlapTopRight)
				ARMDIA1.PR_DrawLine(Point2, xyFlapTopRight)
				
				ARMDIA1.PR_MakeXY(Point1, xyFlapBottomRight.X - 0.125, xyFlapBottomRight.Y)
				ARMDIA1.PR_MakeXY(Point2, xyFlapTopRight.X, xyFlapBottomRight.Y + 0.125)
				ARMDIA1.PR_DrawLine(Point1, xyFlapBottomRight)
				ARMDIA1.PR_DrawLine(Point2, xyFlapBottomRight)
				
				ARMDIA1.PR_MakeXY(Point1, xyFlapBottomLeft.X + 0.125, xyFlapBottomLeft.Y)
				ARMDIA1.PR_MakeXY(Point2, xyFlapBottomLeft.X, xyFlapBottomLeft.Y + 0.125)
				ARMDIA1.PR_DrawLine(Point1, xyFlapBottomLeft)
				ARMDIA1.PR_DrawLine(Point2, xyFlapBottomLeft)
				
				ARMDIA1.PR_MakeXY(Right1, xyFlapTopLeft.X + RightXOffSet, xyFlapTopLeft.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Right2, xyFlapTopRight.X + RightXOffSet, xyFlapTopRight.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Right3, xyFlapBottomRight.X + RightXOffSet, xyFlapBottomRight.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Right4, xyFlapBottomLeft.X + RightXOffSet, xyFlapBottomLeft.Y + YOffSet)
				
				ARMDIA1.PR_SetLayer("TemplateLeft")
				
				ARMDIA1.PR_MakeXY(Left1, xyFlapTopLeft.X + LeftXOffSet, xyFlapTopLeft.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Left2, xyFlapTopRight.X + LeftXOffSet, xyFlapTopRight.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Left3, xyFlapBottomRight.X + LeftXOffSet, xyFlapBottomRight.Y + YOffSet)
				ARMDIA1.PR_MakeXY(Left4, xyFlapBottomLeft.X + LeftXOffSet, xyFlapBottomLeft.Y + YOffSet)
				
				PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left1.X) & CC & "xyStart.y+" & Str(Left1.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left2.X) & CC & "xyStart.y+" & Str(Left2.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left3.X) & CC & "xyStart.y+" & Str(Left3.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left4.X) & CC & "xyStart.y+" & Str(Left4.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left1.X) & CC & "xyStart.y+" & Str(Left1.Y))
				PrintLine(CInt(txtfNum.Text), ");")
				ARMDIA1.PR_AddEntityID("LeftEarFlap")
			...
			If chkRightEarFlap.CheckState = 1 Then
				' Set Top Points
				If optModifiedChinStrap.Checked = False Then
					ARMDIA1.PR_MakeXY(xyFlapTopLeft, -0.625, CDbl(txtTopRightEar.Text) + 0.125)
					ARMDIA1.PR_MakeXY(xyFlapTopRight, 2, CDbl(txtTopRightEar.Text) + 0.125)
				Else
					ARMDIA1.PR_MakeXY(xyFlapTopLeft, -0.375, CDbl(txtTopRightEar.Text) + 0.125)
					ARMDIA1.PR_MakeXY(xyFlapTopRight, 2.1875, CDbl(txtTopRightEar.Text) + 0.125)
				End If
				'Indicate position on Pattern
				ARMDIA1.PR_SetLayer("Notes")
				ARMDIA1.PR_MakeXY(Point1, xyFlapTopLeft.X + 0.125, xyFlapTopLeft.Y)
				...
				PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right1.X) & CC & "xyStart.y+" & Str(Right1.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right2.X) & CC & "xyStart.y+" & Str(Right2.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right3.X) & CC & "xyStart.y+" & Str(Right3.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right4.X) & CC & "xyStart.y+" & Str(Right4.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right1.X) & CC & "xyStart.y+" & Str(Right1.Y))
				PrintLine(CInt(txtfNum.Text), ");")
				ARMDIA1.PR_AddEntityID("RightEarFlap")
	...
	Private Sub PR_DrawEarHole(ByRef EarX As Double, ByRef EarTopY As Double, ByRef EarBotY As Double, ByRef Template As String)
		...
		Dim xyTopOfEarRight, xyBotOfEarRight As xy
		Dim xyTopOfEarLeft, xyBotOfEarLeft As xy
		Dim xyCen As xy
		...
		ARMDIA1.PR_SetLayer(Template)
		
		' Set points and Draw Right of Ear
		ARMDIA1.PR_MakeXY(xyTopOfEarRight, EarX + HalfEarWidth, EarTopY)
		ARMDIA1.PR_MakeXY(xyBotOfEarRight, EarX + HalfEarWidth, EarBotY)
		ARMDIA1.PR_DrawLine(xyTopOfEarRight, xyBotOfEarRight)
		ARMDIA1.PR_AddEntityID("EarRight")
		
		' Set points and Draw Left of Ear
		ARMDIA1.PR_MakeXY(xyTopOfEarLeft, EarX - HalfEarWidth, EarTopY)
		ARMDIA1.PR_MakeXY(xyBotOfEarLeft, EarX - HalfEarWidth, EarBotY)
		ARMDIA1.PR_DrawLine(xyTopOfEarLeft, xyBotOfEarLeft)
		ARMDIA1.PR_AddEntityID("EarLeft")
		
		' Draw Top Curve of Ear
		ARMDIA1.PR_MakeXY(xyCen, EarX, EarTopY)
		ARMDIA1.PR_DrawArc(xyCen, xyTopOfEarLeft, xyTopOfEarRight)
		ARMDIA1.PR_AddEntityID("EarTop")
		
		'Draw Bottom Curve of Ear
		ARMDIA1.PR_MakeXY(xyCen, EarX, EarBotY)
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCen.X) & CC & "xyStart.y +" & Str(xyCen.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
		ARMDIA1.PR_AddEntityID("EarBottom")
		ARMDIA1.PR_SetLayer("TemplateLeft")
		...
		If optModifiedChinStrap.Checked = True Or optFaceMask.Checked = True Or optOpenFaceMask.Checked = True Then
			...
			If chkEarSize.CheckState = 0 Then
				...
				ElseIf chkLeftEarClosed.CheckState = 1 Then 
					ARMDIA1.PR_SetLayer("Notes")
					ARMDIA1.PR_MakeXY(xyTextPoint, 0.25, EarBotY)
					ARMDIA1.PR_DrawText("Left Ear Closed", xyTextPoint, 0.1, 0)
					ARMDIA1.PR_SetLayer("TemplateLeft")
				End If
				...
				ElseIf chkRightEarClosed.CheckState = 1 Then 
					ARMDIA1.PR_SetLayer("Notes")
					ARMDIA1.PR_MakeXY(xyTextPoint, 0.25, EarBotY + 0.25)
					ARMDIA1.PR_DrawText("Right Ear Closed", xyTextPoint, 0.1, 0)
					ARMDIA1.PR_SetLayer("TemplateLeft")
				End If
	...
	Private Sub PR_DrawEyeFlaps()
		...
		'
		Dim xyFlapTopLeft, xyFlapTopRight As xy
		Dim xySideLineInt, xyFlapBottomLeft, xyFlapBottomRight, xyBotLineInt As xy
		Dim FlapX, DartAng, FlapY As Double
		Dim PI As Double
		Dim Notes As xy
		...
		Dim Point1, Point2 As xy
		Dim RightXOffSet, LeftXOffSet, YOffSet As Double
		Dim Left5, Left3, Left1, Left2, Left4, Left6 As xy
		Dim Right5, Right3, Right1, Right2, Right4, Right6 As xy
		...
		' Set Top Points
		ARMDIA1.PR_MakeXY(xyFlapTopLeft, X1 + 0.25, Y1 + 1.125)
		ARMDIA1.PR_MakeXY(xyFlapTopRight, X1 + Dist + 0.25, Y1 + 1.125)
		
		' Calculate Bottom points
		ARMDIA1.PR_MakeXY(xyFlapBottomRight, xyFlapTopRight.X, Y1 - 0.875)
		ARMDIA1.PR_MakeXY(xyFlapBottomLeft, xyFlapTopLeft.X, Y1 - 0.875)
		...
		aAngle = ARMDIA1.FN_CalcAngle(xyNoseDiagBot, xyNoseDiagTop)
		ARMDIA1.PR_CalcPolar(xyNoseDiagTop, aAngle, 20, xyPt1)
		aAngle = ARMDIA1.FN_CalcAngle(xyNoseDiagTop, xyNoseDiagBot)
		ARMDIA1.PR_CalcPolar(xyNoseDiagBot, aAngle, 20, xyPt2)
		
		iSide = BDYUTILS.FN_LinLinInt(xyPt1, xyPt2, xyFlapBottomLeft, xyFlapTopLeft, xySideLineInt)
		iBot = BDYUTILS.FN_LinLinInt(xyPt1, xyPt2, xyFlapBottomLeft, xyFlapBottomRight, xyBotLineInt)
		...
		If chkLeftEyeFlap.CheckState = 1 Or chkRightEyeFlap.CheckState = 1 Then
			...
			If Intersection Then
				ARMDIA1.PR_MakeXY(Point1, xyBotLineInt.X + 0.125, xyBotLineInt.Y)
				ARMDIA1.PR_DrawLine(Point1, xyBotLineInt)
			Else
				ARMDIA1.PR_MakeXY(Point1, xyFlapBottomLeft.X + 0.125, xyFlapBottomLeft.Y)
				ARMDIA1.PR_MakeXY(Point2, xyFlapBottomLeft.X, xyFlapBottomLeft.Y + 0.125)
				ARMDIA1.PR_DrawLine(Point1, xyFlapBottomLeft)
		...
		If chkLeftEyeFlap.CheckState = 1 Then
			ARMDIA1.PR_SetLayer("TemplateLeft")
			ARMDIA1.PR_MakeXY(Left1, xyFlapTopLeft.X - XOffset, xyFlapTopLeft.Y + LeftYOffSet)
			ARMDIA1.PR_MakeXY(Left2, xyFlapTopRight.X - XOffset, xyFlapTopRight.Y + LeftYOffSet)
			ARMDIA1.PR_MakeXY(Left3, xyFlapBottomRight.X - XOffset, xyFlapBottomRight.Y + LeftYOffSet)
			ARMDIA1.PR_MakeXY(Left4, xyFlapBottomLeft.X - XOffset, xyFlapBottomLeft.Y + LeftYOffSet)
			If Intersection Then
				ARMDIA1.PR_MakeXY(Left5, xySideLineInt.X - XOffset, xySideLineInt.Y + LeftYOffSet)
				ARMDIA1.PR_MakeXY(Left6, xyBotLineInt.X - XOffset, xyBotLineInt.Y + LeftYOffSet)
			End If
			...
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left3.X) & CC & "xyStart.y+" & Str(Left3.Y))
			If Intersection Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left6.X) & CC & "xyStart.y+" & Str(Left6.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left5.X) & CC & "xyStart.y+" & Str(Left5.Y))
			Else
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left4.X) & CC & "xyStart.y+" & Str(Left4.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Left1.X) & CC & "xyStart.y+" & Str(Left1.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LeftEyeFlap")
		...
		If chkRightEyeFlap.CheckState = 1 Then
			ARMDIA1.PR_SetLayer("TemplateRight")
			ARMDIA1.PR_MakeXY(Right1, xyFlapTopLeft.X - XOffset, xyFlapTopLeft.Y + RightYOffSet)
			ARMDIA1.PR_MakeXY(Right2, xyFlapTopRight.X - XOffset, xyFlapTopRight.Y + RightYOffSet)
			ARMDIA1.PR_MakeXY(Right3, xyFlapBottomRight.X - XOffset, xyFlapBottomRight.Y + RightYOffSet)
			ARMDIA1.PR_MakeXY(Right4, xyFlapBottomLeft.X - XOffset, xyFlapBottomLeft.Y + RightYOffSet)
			If Intersection Then
				ARMDIA1.PR_MakeXY(Right5, xySideLineInt.X - XOffset, xySideLineInt.Y + RightYOffSet)
				ARMDIA1.PR_MakeXY(Right6, xyBotLineInt.X - XOffset, xyBotLineInt.Y + RightYOffSet)
			End If
			...
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right3.X) & CC & "xyStart.y+" & Str(Right3.Y))
			If Intersection Then
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right6.X) & CC & "xyStart.y+" & Str(Right6.Y))
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right5.X) & CC & "xyStart.y+" & Str(Right5.Y))
			Else
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right4.X) & CC & "xyStart.y+" & Str(Right4.Y))
			End If
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Right1.X) & CC & "xyStart.y+" & Str(Right1.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("RightEyeFlap")
	...
	Private Sub PR_DrawEyeOpening()
		...
		Dim xyTopOfEye, xyBotOfEye As xy
		Dim xyTopOfEyeStart, xyBotOfEyeStart As xy
		Dim xyTopOfEyeEnd, xyBotOfEyeEnd As xy
		Dim xyCen, xyCen2 As xy
		Dim i As Short
		...
		ARMDIA1.PR_MakeXY(xyTopOfEye, Tempx, -(System.Math.Abs(CDbl(txtMidToEyeTop.Text))))
		ARMDIA1.PR_MakeXY(xyBotOfEye, Tempx, -(System.Math.Abs(CDbl(txtMidToEyeTop.Text))) - 0.25)
		...
		ARMDIA1.PR_MakeXY(xyTopOfEyeStart, xyTopOfEye.X + 0.5, xyTopOfEye.Y)
		ARMDIA1.PR_MakeXY(xyTopOfEyeEnd, xyTopOfEye.X + EyeWidthFromFace - 0.125, xyTopOfEye.Y)
		
		' Set points for bottom of eye
		ARMDIA1.PR_MakeXY(xyBotOfEyeStart, xyBotOfEye.X + 0.5, xyBotOfEye.Y)
		ARMDIA1.PR_MakeXY(xyBotOfEyeEnd, xyBotOfEye.X + EyeWidthFromFace - 0.125, xyBotOfEye.Y)
		
		' Set points for Left Curve of eye
		ARMDIA1.PR_MakeXY(xyCen, xyTopOfEyeStart.X, xyTopOfEyeStart.Y - 0.125)
		
		' Set points for Right Curve of eye
		ARMDIA1.PR_MakeXY(xyCen2, xyTopOfEyeEnd.X, xyTopOfEyeEnd.Y - 0.125)
		'UPGRADE_WARNING: Couldn't resolve default property of object nStartAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		nStartAng = 0
		'UPGRADE_WARNING: Couldn't resolve default property of object nDeltaAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
		nDeltaAng = 90
		...
		If optFaceMask.Checked = True Or (optOpenFaceMask.Checked = True And chkEyes.CheckState = 1) Then
			...
			If chkNoseCovering.CheckState = 0 Then
				ARMDIA1.PR_DrawLine(xyTopOfEye, xyBotOfEye)
				ARMDIA1.PR_AddEntityID("NoseBridge")
			End If
			
			ARMDIA1.PR_DrawLine(xyTopOfEyeStart, xyTopOfEyeEnd)
			ARMDIA1.PR_AddEntityID("EyeTop")
			
			ARMDIA1.PR_DrawLine(xyBotOfEyeStart, xyBotOfEyeEnd)
			ARMDIA1.PR_AddEntityID("EyeBottom")
			
			ARMDIA1.PR_DrawArc(xyCen, xyTopOfEyeStart, xyBotOfEyeStart)
			ARMDIA1.PR_AddEntityID("EyeLeft")
			
			'UPGRADE_WARNING: Couldn't resolve default property of object nDeltaAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCen2.X) & CC & "xyStart.y +" & Str(xyCen.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
			'UPGRADE_WARNING: Couldn't resolve default property of object nDeltaAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			nDeltaAng = -90
			ARMDIA1.PR_AddEntityID("EyeRightTop")
			'UPGRADE_WARNING: Couldn't resolve default property of object nDeltaAng. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCen2.X) & CC & "xyStart.y +" & Str(xyCen.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
			ARMDIA1.PR_AddEntityID("EyeRightBottom")
	...
	Private Sub PR_DrawForeHead()
		...
		Dim xyForeHeadBot, xyForeHeadTop As xy
		Dim Tempx, TempY As Double
		
		If chkOpenHeadMask.CheckState = 0 Then
			Tempx = -LeftArcAdjOut(CInt(txtRadiusNo.Text))
			TempY = LeftArcOppOut(CInt(txtRadiusNo.Text))
			ARMDIA1.PR_MakeXY(xyForeHeadBot, Tempx, 0)
			ARMDIA1.PR_MakeXY(xyForeHeadTop, Tempx, TempY)
			
			ARMDIA1.PR_DrawLine(xyForeHeadBot, xyForeHeadTop)
			ARMDIA1.PR_AddEntityID("ForeHead")
		End If
	End Sub
	
	Private Sub PR_DrawHeadBand()
		Dim xyStartLeftArc, xyCentre, xyEndLeftArc As xy
		Dim xyStartRightArc, xyEndRightArc As xy
		Dim HeadBand As Double
		Dim RightArcEnd, RightArcStart, RightArcStep As Double
		Dim Across, Upwards As Double
		Dim xyTmp, xyTextPoint, xyNotes, xyTmp1 As xy
		...
		If optHeadBand.Checked = True Then
			...
			ARMDIA1.PR_MakeXY(xyStartLeftArc, -AcrossOut, UpwardsOut)
			...
			ARMDIA1.PR_MakeXY(xyCentre, 0, 0)
			
			nRadius = ARMDIA1.FN_CalcLength(xyStartLeftArc, xyCentre)
			ARMDIA1.PR_MakeXY(xyTmp, 0, Upwards)
			ARMDIA1.PR_MakeXY(xyTmp1, xyStartLeftArc.X - 10, Upwards)
			iError = BDYUTILS.FN_CirLinInt(xyTmp, xyTmp1, xyCentre, nRadius, xyEndLeftArc)
			
			ARMDIA1.PR_MakeXY(xyStartRightArc, RightArcStart + RightArcStep, 0)
			Across = System.Math.Sqrt((System.Math.Abs(xyStartRightArc.X) * System.Math.Abs(xyStartRightArc.X)) - (Upwards * Upwards))
			ARMDIA1.PR_MakeXY(xyEndRightArc, Across, Upwards)
			
			ARMDIA1.PR_DrawArc(xyCentre, xyStartLeftArc, xyEndLeftArc)
			ARMDIA1.PR_AddEntityID("HeadBandLeftArc")
			ARMDIA1.PR_DrawArc(xyCentre, xyStartRightArc, xyEndRightArc)
			ARMDIA1.PR_AddEntityID("HeadBandRightArc")
			ARMDIA1.PR_DrawLine(xyEndLeftArc, xyEndRightArc)
			ARMDIA1.PR_AddEntityID("HeadBandTop")
			
			ARMDIA1.PR_MakeXY(xyTmp, xyStartLeftArc.X, xyStartRightArc.Y)
			ARMDIA1.PR_DrawLine(xyTmp, xyStartRightArc)
			ARMDIA1.PR_AddEntityID("HeadBandBottom")
			ARMDIA1.PR_DrawLine(xyTmp, xyStartLeftArc)
			ARMDIA1.PR_AddEntityID("HeadBandLeftLine")
			
			'add closure
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_MakeXY(xyTextPoint, xyEndRightArc.X - 0.1, Upwards / 2)
			ARMDIA1.PR_DrawText("VELCRO", xyTextPoint, 0.1, ARMDIA1.FN_CalcAngle(xyEndRightArc, xyStartRightArc))
			
			' Add notes
			ARMDIA1.PR_MakeXY(xyTextPoint, xyEndLeftArc.X + 0.125, xyEndLeftArc.Y - 0.13)
	...
	Private Sub PR_DrawLeftArc()
		...
		Dim xyEndArc, xyStartArc, xyCentreArc As xy
		Dim AcrossIn, UpwardsIn As Double
		Dim AcrossOut, UpwardsOut As Double
		
		If chkOpenHeadMask.CheckState = 0 Then
			ARMDIA1.PR_MakeXY(xyCentreArc, 0, 0)
			...
			ARMDIA1.PR_MakeXY(xyStartArc, -AcrossIn, UpwardsIn)
			ARMDIA1.PR_MakeXY(xyEndArc, -AcrossOut, UpwardsOut)
			ARMDIA1.PR_DrawArc(xyCentreArc, xyStartArc, xyEndArc)
			ARMDIA1.PR_AddEntityID("LeftArc")
		End If
	End Sub
	...
	Private Sub PR_DrawLeftCutOut()
		...
		Dim LeftTip, Centre, RightTip As xy
		
		If chkOpenHeadMask.CheckState = 0 Then
			CentreX = -3.8 / 2.54
			CentreY = 3.8 / 2.54
			
			ARMDIA1.PR_MakeXY(Centre, CentreX, CentreY)
			...
			ARMDIA1.PR_MakeXY(LeftTip, -AcrossLeft, UpwardsLeft)
			ARMDIA1.PR_MakeXY(RightTip, -AcrossRight, UpwardsRight)
			
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(LeftTip.X) & CC & "xyStart.y+" & Str(LeftTip.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Centre.X) & CC & "xyStart.y+" & Str(Centre.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(RightTip.X) & CC & "xyStart.y+" & Str(RightTip.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LeftCutOut")
	...
	Private Sub PR_DrawLine(ByRef xyStart As xy, ByRef xyFinish As xy)
		...
		PrintLine(CInt(txtfNum.Text), QQ & "line" & QC)
		PrintLine(CInt(txtfNum.Text), "xyStart.x+" & Str(xyStart.X) & CC & "xyStart.y+" & Str(xyStart.Y) & CC)
		PrintLine(CInt(txtfNum.Text), "xyStart.x+" & Str(xyFinish.X) & CC & "xyStart.y+" & Str(xyFinish.Y))
		PrintLine(CInt(txtfNum.Text), ");")
	...
	Private Sub PR_DrawLipCovering()
		...
		Dim xyFlapTopRight, xyFlapTopLeft, xyFlapBottomMid As xy
		Dim xyFlapBottomRight, xyFlapBottomLeft, xyTextPoint As xy
		Dim xy2, Notes, xy1, xyTmp As xy
		Dim xy6, xyDartStart, xyDartEnd, xy5 As xy
		...
		If chkLipCovering.CheckState = 1 And optChinStrap.Checked = False And optModifiedChinStrap.Checked = False Then
			...
			ARMDIA1.PR_MakeXY(xyFlapTopLeft, -LeftArcAdjOut(CInt(txtRadiusNo.Text)) + 0.375, Y1 + 0.5)
			
			
			' Set Top Right Point
			ARMDIA1.PR_MakeXY(xyFlapTopRight, -0.875, xyFlapTopLeft.Y)
			...
			ARMDIA1.PR_MakeXY(xyDartStart, X1, Y1)
			ARMDIA1.PR_MakeXY(xyDartEnd, X3, Y3)
			
			If X1 >= X2 Then
				ARMDIA1.PR_MakeXY(xyFlapBottomRight, xyFlapTopRight.X, Y1 + 0.25)
			Else
				DistX = System.Math.Abs(System.Math.Abs(X2) - System.Math.Abs(X1))
				DartAngle = ARMDIA1.FN_CalcAngle(xyDartStart, xyDartEnd)
				DartAngle = DartAngle * (PI / 180)
				Upwards = System.Math.Abs(DistX * System.Math.Tan(DartAngle))
				ARMDIA1.PR_MakeXY(xyFlapBottomRight, xyFlapTopRight.X, Y1 + Upwards + 0.25)
			End If
			
			
			' Find Bottom Mid Point
			ARMDIA1.PR_MakeXY(xy5, ChinProfileX(5), ChinProfileY(5))
			ARMDIA1.PR_MakeXY(xy6, ChinProfileX(6), ChinProfileY(6))
			'PR_DrawMarker xy5
			...
			If xyDartStart.Y > xy6.Y Then
				...
				ARMDIA1.PR_MakeXY(xy1, xyChinTopCen.X, xyDartStart.Y)
				ARMDIA1.PR_MakeXY(xy2, xyChinTopCen.X - 10, xyDartStart.Y)
				'PR_DrawLine xy1, xy2
				'Print #txtfNum, "hEnt = AddEntity("; QQ; "circle"; QQ; ",xyStart.x+"; Str$(xyChinTopCen.x); CC; "xyStart.y+"; Str$(xyChinTopCen.y); ","; g_nChinTopRadius; ");"
				i = BDYUTILS.FN_CirLinInt(xy1, xy2, xyChinTopCen, g_nChinTopRadius, xyDartInt)
				'PR_DrawMarker xyDartInt
				'PR_DrawText "xyDartInt", xyDartInt, .05, 0
				aAngle = ARMDIA1.FN_CalcAngle(xyChinTopCen, xy6) - ARMDIA1.FN_CalcAngle(xyChinTopCen, xyDartInt)
				nArc = ((PI / 180) * aAngle) * g_nChinTopRadius
				nOffSet = ARMDIA1.FN_CalcLength(xy6, xy5)
				If nArc > nAlong Then
					'This is an impossible case so we don't deal with it
				ElseIf (nOffSet + nArc) > nAlong Then 
					ARMDIA1.PR_CalcPolar(xy6, ARMDIA1.FN_CalcAngle(xy6, xy5), nAlong - nArc, xyFlapBottomMid)
					'PR_DrawMarker xyFlapBottomMid
					'PR_DrawText "xyFlapBottomMid on line", xyFlapBottomMid, .05, 0
				Else
					nOffSet = nAlong - (nOffSet + nArc)
					aStart = ARMDIA1.FN_CalcAngle(xyChinBotCen, xy5)
					aAngle = aStart - ((nOffSet / (2 * PI * g_nChinBotRadius)) * 360)
					ARMDIA1.PR_CalcPolar(xyChinBotCen, aAngle, g_nChinBotRadius, xyFlapBottomMid)
					'PR_DrawMarker xyFlapBottomMid
			...
			ElseIf xyDartStart.Y > xy5.Y Then 
				...
				ARMDIA1.PR_MakeXY(xyDartInt, xy6.X - (System.Math.Abs(xy6.Y - xyDartStart.Y) / System.Math.Tan(ARMDIA1.FN_CalcAngle(xy5, xy6) * (PI / 180))), xyDartStart.Y)
				nOffSet = ARMDIA1.FN_CalcLength(xyDartInt, xy5)
				If nOffSet < nAlong Then
					aStart = ARMDIA1.FN_CalcAngle(xyChinBotCen, xy5)
					aAngle = aStart - (((nAlong - nOffSet) / (2 * PI * g_nChinBotRadius)) * 360)
					ARMDIA1.PR_CalcPolar(xyChinBotCen, aAngle, g_nChinBotRadius, xyFlapBottomMid)
				Else
					ARMDIA1.PR_CalcPolar(xyDartInt, ARMDIA1.FN_CalcAngle(xy6, xy5), nAlong, xyFlapBottomMid)
				End If
				'PR_DrawMarker xyDartInt
				'PR_DrawText "xyDartInt", xyDartInt, .05, 0
			Else
				'Calculate intersection point on the bottom arc
				ARMDIA1.PR_MakeXY(xy1, xyChinBotCen.X, xyDartEnd.Y)
				ARMDIA1.PR_MakeXY(xy2, xyChinBotCen.X + 10, xyDartEnd.Y)
				'PR_DrawLine xy1, xy2
				'Print #txtfNum, "hEnt = AddEntity("; QQ; "circle"; QQ; ",xyStart.x+"; Str$(xyChinBotCen.x); CC; "xyStart.y+"; Str$(xyChinBotCen.y); ","; g_nChinBotRadius; ");"
				i = BDYUTILS.FN_CirLinInt(xy1, xy2, xyChinBotCen, g_nChinBotRadius, xyDartInt)
				'PR_DrawMarker xyDartInt
				'PR_DrawText "xyDartInt", xyDartInt, .05, 0
				aStart = ARMDIA1.FN_CalcAngle(xyChinBotCen, xyDartInt)
			...
			For ii = 8 To 6 Step -1
				Select Case nn
					...
					Case 6
						nOffSet = 0.08
				End Select
				ARMDIA1.PR_MakeXY(xyTmp, ChinProfileX(ii), ChinProfileY(ii))
				ARMDIA1.PR_CalcPolar(xyChinTopCen, ARMDIA1.FN_CalcAngle(xyChinTopCen, xyTmp), ARMDIA1.FN_CalcLength(xyChinTopCen, xyTmp) - nOffSet, xyTmp)
			...
			ARMDIA1.PR_MakeXY(Point1, xyFlapBottomMid.X + 0.125, xyFlapBottomMid.Y)
			ARMDIA1.PR_MakeXY(Point2, xyFlapBottomMid.X, xyFlapBottomMid.Y + 0.125)
			ARMDIA1.PR_DrawLine(Point1, xyFlapBottomMid)
			
			ARMDIA1.PR_SetLayer("TemplateLeft")
			
			' Add OffSet to Points
			ARMDIA1.PR_MakeXY(xyFlapTopLeft, xyFlapTopLeft.X + XOffset, xyFlapTopLeft.Y)
			ARMDIA1.PR_MakeXY(xyFlapTopRight, xyFlapTopRight.X + XOffset, xyFlapTopRight.Y)
			ARMDIA1.PR_MakeXY(xyFlapBottomRight, xyFlapBottomRight.X + XOffset, xyFlapBottomRight.Y)
			ARMDIA1.PR_MakeXY(xyFlapBottomMid, xyFlapBottomMid.X + XOffset, xyFlapBottomMid.Y)
			
			nOffSet = ARMDIA1.FN_CalcLength(xyFlapBottomMid, xyFlapBottomRight) / 2
			aAngle = ARMDIA1.FN_CalcAngle(xyFlapBottomMid, xyFlapBottomRight)
			ARMDIA1.PR_CalcPolar(xyFlapBottomMid, aAngle, nOffSet, xyTmp)
			ARMDIA1.PR_CalcPolar(xyTmp, aAngle + 270, 9, xyLipCoveringBotCen)
			ARMDIA1.PR_DrawArc(xyLipCoveringBotCen, xyFlapBottomMid, xyFlapBottomRight)
			ARMDIA1.PR_AddEntityID("LipCoveringBot")
			
			
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyFlapTopLeft.X) & CC & "xyStart.y+" & Str(xyFlapTopLeft.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyFlapTopRight.X) & CC & "xyStart.y+" & Str(xyFlapTopRight.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyFlapBottomRight.X) & CC & "xyStart.y+" & Str(xyFlapBottomRight.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LipCovering")
			
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For ii = 1 To 7
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(LipProfileX(ii)) & CC & "xyStart.y+" & Str(LipProfileY(ii)))
			Next ii
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("LipCoveringCurve")
			
			' Add Velcro
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_MakeXY(xy1, xyFlapTopRight.X - 0.75, xyFlapTopRight.Y)
			ARMDIA1.PR_MakeXY(xy2, xy1.X, xyLipCoveringBotCen.Y)
			ii = BDYUTILS.FN_CirLinInt(xy1, xy2, xyLipCoveringBotCen, ARMDIA1.FN_CalcLength(xyLipCoveringBotCen, xyFlapBottomRight), xy2)
			ARMDIA1.PR_DrawLine(xy1, xy2)
			length = ARMDIA1.FN_CalcLength(xy1, xy2) + 0.25
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = FN_InchesToText(ARMEDDIA1.fnRoundInches(length))
			...
			ARMDIA1.PR_MakeXY(TextPoint, xy1.X - 0.25, xy1.Y - 0.25)
			ARMDIA1.PR_DrawText(VelcroText, TextPoint, 0.1, -90)
			length = ARMDIA1.FN_CalcLength(xyFlapTopRight, xyFlapBottomRight) + 0.25
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = FN_InchesToText(ARMEDDIA1.fnRoundInches(length))
			'UPGRADE_WARNING: Couldn't resolve default property of object VelcroText. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			VelcroText = VelcroText & "\""  Velcro"
			ARMDIA1.PR_MakeXY(TextPoint, xyFlapTopRight.X - 0.25, xy1.Y - 0.25)
			ARMDIA1.PR_DrawText(VelcroText, TextPoint, 0.1, -90)
			
			' Add Notes
			ARMDIA1.PR_MakeXY(xyTextPoint, xyFlapTopLeft.X + 0.25, xyFlapTopLeft.Y - 0.1)
			PR_TemplateDetails(xyTextPoint, "Lip Covering")
			
			ARMDIA1.PR_SetLayer("Construct")
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For ii = 3 To 10
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(ChinProfileX(ii) + XOffset) & CC & "xyStart.y+" & Str(ChinProfileY(ii)))
			Next ii
			PrintLine(CInt(txtfNum.Text), ");")
	...
	Private Sub PR_DrawMarker(ByRef xyPoint As xy)
		'Draw a Marker at the given point
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "marker" & QCQ & "xmarker" & QC & "xyStart.x+" & xyPoint.X & CC & "xyStart.y+" & xyPoint.Y & CC & "0.125);")
	...
	Private Sub PR_DrawNeckAndDart()
		...
		Dim NeckRightSideTop, NeckLeftSideTop As xy
		Dim NeckRightSideBot, NeckLeftSideBot As xy
		Dim DartStart, DartEnd As xy
		Dim Tempx, TempY As Double
		...
		ARMDIA1.PR_MakeXY(NeckRightSideTop, Tempx - ZipBit, TempY)
		
		' Top Left Point
		TempX2 = Tempx - FiguredNeck
		ARMDIA1.PR_MakeXY(NeckLeftSideTop, TempX2, TempY)
		...
		ARMDIA1.PR_MakeXY(NeckRightSideBot, Tempx - ZipBit, TempY2)
		
		' Bottom Left Point
		ARMDIA1.PR_MakeXY(NeckLeftSideBot, TempX2, TempY2)
		...
		ARMDIA1.PR_MakeXY(DartStart, Dart, TempY)
		...
		ARMDIA1.PR_MakeXY(DartEnd, DartEndX, DartEndY)
		txtDartEndX.Text = CStr(DartEnd.X)
		txtDartEndY.Text = CStr(DartEnd.Y)
		
		' Draw Neck Piece
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(DartEnd.X) & CC & "xyStart.y+" & Str(DartEnd.Y))
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(DartStart.X) & CC & "xyStart.y+" & Str(DartStart.Y))
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(NeckRightSideTop.X) & CC & "xyStart.y+" & Str(NeckRightSideTop.Y))
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(NeckRightSideBot.X) & CC & "xyStart.y+" & Str(NeckRightSideBot.Y))
		PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(NeckLeftSideBot.X) & CC & "xyStart.y+" & Str(NeckLeftSideBot.Y))
		PrintLine(CInt(txtfNum.Text), ");")
		ARMDIA1.PR_AddEntityID("DartAndNeck")
		...
		If chkVelcro.CheckState = 1 Then
			...
			ARMDIA1.PR_DrawText("2\"" VELCRO", DartEnd, 0.1, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
		End If
	...
	Private Sub PR_DrawNose()
		...
		Dim xyNoseTopLeft, xyNoseTopRight As xy
		'Dim xyNoseDiagTop As xy, xyNoseDiagBot As xy   'Now Global for use in drawing
		'eye flaps
		Dim xyNoseBotRight, xyNoseBotLeft, xyNoseBotRight2 As xy
		Dim xyTopCen, xyBotCen As xy
		...
		ARMDIA1.PR_MakeXY(xyNoseTopLeft, Tempx, TempY)
		ARMDIA1.PR_MakeXY(xyNoseTopRight, Tempx + 0.125, TempY)
		
		' Bottom Part
		TempY2 = Val(txtNoseBottomY.Text)
		ARMDIA1.PR_MakeXY(xyNoseBotLeft, xyNoseTopLeft.X, TempY2)
		ARMDIA1.PR_MakeXY(xyNoseBotRight, xyNoseTopLeft.X + 0.25, TempY2)
		
		' Diagonal Line
		ARMDIA1.PR_MakeXY(xyNoseDiagTop, xyNoseTopRight.X + 0.125, xyNoseTopRight.Y - 0.125)
		ARMDIA1.PR_MakeXY(xyNoseDiagBot, xyNoseBotRight.X + 0.125, TempY2 + 0.125)
		
		' Top Curve
		ARMDIA1.PR_MakeXY(xyTopCen, xyNoseTopRight.X, xyNoseDiagTop.Y)
		
		'Bottom Curve
		ARMDIA1.PR_MakeXY(xyBotCen, xyNoseBotRight.X, xyNoseDiagBot.Y)
		nStartAng = 0
		nDeltaAng = -90
		nRad = 0.125
		...
		If chkNoseCovering.CheckState = 0 Then
			ARMDIA1.PR_DrawLine(xyNoseTopLeft, xyNoseTopRight)
			ARMDIA1.PR_AddEntityID("NoseTop")
			ARMDIA1.PR_DrawLine(xyNoseBotLeft, xyNoseBotRight)
			ARMDIA1.PR_AddEntityID("NoseBottom")
			ARMDIA1.PR_DrawLine(xyNoseDiagTop, xyNoseDiagBot)
			ARMDIA1.PR_AddEntityID("NoseSide")
			ARMDIA1.PR_DrawArc(xyTopCen, xyNoseTopRight, xyNoseDiagTop)
			ARMDIA1.PR_AddEntityID("NoseTopCurve")
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyBotCen.X) & CC & "xyStart.y +" & Str(xyBotCen.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
			ARMDIA1.PR_AddEntityID("NoseBottomCurve")
		Else
			ARMDIA1.PR_MakeXY(xyNoseBotRight2, xyNoseBotRight.X + 0.125, xyNoseBotRight.Y)
			ARMDIA1.PR_DrawLine(xyNoseBotLeft, xyNoseBotRight2)
			ARMDIA1.PR_AddEntityID("NoseCoveringBottom")
		End If
	...
	Private Sub PR_DrawNoseCovering()
		...
		Dim xyNoseTop, xyNoseTip, xyTempTip, xyNoseTipOut As xy
		Dim xyNoseLenStartArc, xyNoseLenCenArc, xyNoseLenEndArc As xy
		Dim xyNoseWidthStartArc, xyNoseWidthCenArc, xyNoseWidthEndArc As xy
		...
		Dim i As Short
		Dim xyNoseRightTip, HeadPt1, HeadPt2, xyNoseRightTip2 As xy
		Dim TipAng3 As Double
		Dim NostralPt1, NostralPt2 As xy
		...
		If chkNoseCovering.CheckState = 1 Then
			...
			ARMDIA1.PR_MakeXY(xyNoseLenCenArc, Adj1 + 0.1875, Opp1 + 0.25)
			'UPGRADE_WARNING: Couldn't resolve default property of object FN_CmToInches(). Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			...
			ARMDIA1.PR_MakeXY(xyNoseWidthCenArc, Adj1, Opp1)
			...
			TipAng = 270 - ARMDIA1.FN_CalcAngle(xyNoseLenCenArc, xyTempTip)
			TipAng = TipAng * (PI / 180)
			TipX = (NoseLen + 0.25) * System.Math.Sin(TipAng)
			TipY = (NoseLen + 0.25) * System.Math.Cos(TipAng)
			ARMDIA1.PR_MakeXY(xyNoseTip, xyNoseLenCenArc.X - TipX, xyNoseLenCenArc.Y - TipY)
			
			
			TipAng2 = ARMDIA1.FN_CalcAngle(xyNoseTip, xyTempTip)
			TipAng2 = 90 - TipAng2
			TipAng2 = TipAng2 * (PI / 180)
			TipOpp = 0.1875 * (System.Math.Sin(TipAng2))
			TipAdj = 0.1875 * (System.Math.Cos(TipAng2))
			
			ARMDIA1.PR_MakeXY(xyNoseTipOut, xyNoseTip.X - TipAdj, xyNoseTip.Y + TipOpp)
			TipAng = ARMDIA1.FN_CalcAngle(xyTempTip, xyNoseLenCenArc) * (PI / 180)
			nLength = (xyNoseLenCenArc.Y - xyNoseTipOut.Y) - 0.125
			nLength = nLength / System.Math.Sin(TipAng)
			ARMDIA1.PR_CalcPolar(xyNoseTipOut, ARMDIA1.FN_CalcAngle(xyTempTip, xyNoseLenCenArc), nLength, xyNoseTop)
			...
			ARMDIA1.PR_MakeXY(HeadPt1, TopX, TopY)
			ARMDIA1.PR_MakeXY(HeadPt2, TopX - 0.03125, TopY - 0.25)
			'PR_DrawMarker HeadPt1
			'PR_DrawText "HeadPt1", HeadPt1, .05, 0
			'PR_DrawMarker HeadPt2
			'PR_DrawText "HeadPt2", HeadPt2, .05, 0
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "open" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(HeadPt1.X) & CC & "xyStart.y+" & Str(HeadPt1.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(HeadPt2.X) & CC & "xyStart.y+" & Str(HeadPt2.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseTop.X) & CC & "xyStart.y+" & Str(xyNoseTop.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("NoseCoveringBridge")
			
			TipX = 0.25 * System.Math.Cos(TipAng2)
			TipY = 0.25 * System.Math.Sin(TipAng2)
			ARMDIA1.PR_MakeXY(xyNoseRightTip, xyNoseTip.X + TipX, xyNoseTip.Y - TipY)
			
			TipAng3 = (90 - TipAng2 * (180 / PI)) * (PI / 180)
			TipX = 0.25 * System.Math.Cos(TipAng3)
			TipY = 0.25 * System.Math.Sin(TipAng3)
			ARMDIA1.PR_MakeXY(xyNoseRightTip2, xyNoseRightTip.X + TipX, xyNoseRightTip.Y + TipY)
			
			' Draw nose
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseTop.X) & CC & "xyStart.y+" & Str(xyNoseTop.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseTipOut.X) & CC & "xyStart.y+" & Str(xyNoseTipOut.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseTip.X) & CC & "xyStart.y+" & Str(xyNoseTip.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseRightTip.X) & CC & "xyStart.y+" & Str(xyNoseRightTip.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(xyNoseRightTip2.X) & CC & "xyStart.y+" & Str(xyNoseRightTip2.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("NoseCovering")
			
			' Calculate and draw nostril
			' Mods GG 4.Mar.96
			aTip = ARMDIA1.FN_CalcAngle(xyNoseLenCenArc, xyNoseTip) + 90
			nLength = ARMDIA1.FN_CalcLength(xyNoseRightTip2, xyNoseWidthCenArc)
			aAngle = ARMDIA1.FN_CalcAngle(xyNoseRightTip2, xyNoseWidthCenArc)
			If aAngle > 180 Then
				aAngle = aAngle - aTip
			Else
			...
			ARMDIA1.PR_CalcPolar(xyNoseRightTip2, aTip, nRad, NostralPt1)
			aStartAngle = ARMDIA1.FN_CalcAngle(NostralPt1, xyNoseRightTip2)
			aAngle = ARMDIA1.FN_CalcAngle(NostralPt1, xyNoseWidthCenArc)
			...
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
			For ii = 0 To 3
				ARMDIA1.PR_CalcPolar(NostralPt1, aStartAngle - (aAngleInc * ii), nRad, NostralPt2)
				PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(NostralPt2.X) & CC & "xyStart.y+" & Str(NostralPt2.Y))
			Next ii
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("Nostril")
	...
	Private Sub PR_DrawOpenFace()
		...
		Dim xyOpeningTop, xyFaceStart, xyOpeningBottom As xy
		Dim xyLipBottom, xyLipTop As xy
		Dim xyPoint1, xyPoint2 As xy
		Dim xDist, YDist As Double
		Dim X1, Y1 As Double
		Dim ii As Short
		Dim XStep, YStep As Double
		
		ARMDIA1.PR_MakeXY(xyFaceStart, -LeftArcAdjOut(CInt(txtRadiusNo.Text)), 0)
		ARMDIA1.PR_MakeXY(xyOpeningTop, -LeftArcAdjOut(CInt(txtRadiusNo.Text)), -0.5)
		ARMDIA1.PR_DrawLine(xyFaceStart, xyOpeningTop)
		ARMDIA1.PR_AddEntityID("OpenFaceEyeBrow")
		...
		If chkLipStrap.CheckState = 1 Then
			...
			ARMDIA1.PR_MakeXY(xyLipBottom, X1, Y1)
			ARMDIA1.PR_MakeXY(xyLipTop, X1, Y1 + Val(txtLipStrapWidth.Text))
			ARMDIA1.PR_DrawLine(xyLipBottom, xyLipTop)
			ARMDIA1.PR_AddEntityID("OpenFaceLipStrap")
			ARMDIA1.PR_MakeXY(xyOpeningBottom, xyLipTop.X, xyLipTop.Y)
		Else
			X1 = CDbl(txtMouthRightX.Text)
			Y1 = Val(txtMouthHeight.Text)
			ARMDIA1.PR_MakeXY(xyOpeningBottom, X1, Y1)
		End If
		
		ARMDIA1.PR_MakeXY(xyPoint1, OpenFaceProfileX(4), OpenFaceProfileY(4))
		ARMDIA1.PR_MakeXY(xyPoint2, xyOpeningBottom.X, OpenFaceProfileY(4))
		xDist = ARMDIA1.FN_CalcLength(xyPoint1, xyPoint2)
		XStep = -xDist / 6
		
		ARMDIA1.PR_MakeXY(xyPoint1, OpenFaceProfileX(4), OpenFaceProfileY(4))
		ARMDIA1.PR_MakeXY(xyPoint2, OpenFaceProfileX(4), xyOpeningBottom.Y)
		YDist = ARMDIA1.FN_CalcLength(xyPoint1, xyPoint2)
		YStep = -YDist / 5
		...
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "fitted" & QQ)
		For ii = 1 To 10
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(OpenFaceProfileX(ii)) & CC & "xyStart.y+" & Str(OpenFaceProfileY(ii)))
		Next ii
		PrintLine(CInt(txtfNum.Text), ");")
		ARMDIA1.PR_AddEntityID("OpenFace")
	...
	Private Sub PR_DrawOpenFaceWithEyes()
		...
		X1 = CDbl(txtMouthRightX.Text)
		ARMDIA1.PR_MakeXY(xyFaceStart, -LeftArcAdjOut(CInt(txtRadiusNo.Text)), 0)
		ARMDIA1.PR_MakeXY(xyOpeningTop, -LeftArcAdjOut(CInt(txtRadiusNo.Text)), Y1)
		ARMDIA1.PR_MakeXY(xyOpeningBottom, X1, y2 + 0.125)
		ARMDIA1.PR_MakeXY(xyMouthRight, X1 - 0.125, y2)
		ARMDIA1.PR_MakeXY(xyCen, xyOpeningBottom.X - 0.125, xyOpeningBottom.Y)
		...
		nRad = 0.125
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCen.X) & CC & "xyStart.y +" & Str(xyCen.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
		ARMDIA1.PR_AddEntityID("OpenFaceBottomCurve")
		
		ARMDIA1.PR_DrawLine(xyFaceStart, xyOpeningTop)
		ARMDIA1.PR_AddEntityID("OpenFaceEyeBrow")
		
		ARMDIA1.PR_DrawLine(xyOpeningBottom, xyOpeningTop)
		ARMDIA1.PR_AddEntityID("OpenFace")
	...
	Private Sub PR_DrawOpenHead()
		...
		Dim Point2, Point1, Point3 As xy
		Dim xyClosureText, Cen As xy
		...
		If chkOpenHeadMask.CheckState = 1 Then
			ARMDIA1.PR_MakeXY(Point1, -LeftArcAdjOut(NormCircum), 0)
			ARMDIA1.PR_MakeXY(Point2, -LeftArcAdjOut(NormCircum), 0.5)
			ARMDIA1.PR_MakeXY(Point3, RightArcOpenAdjUp(Circum), 0.5)
			PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "poly" & QCQ & "polyline" & QQ)
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Point1.X) & CC & "xyStart.y+" & Str(Point1.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Point2.X) & CC & "xyStart.y+" & Str(Point2.Y))
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(Point3.X) & CC & "xyStart.y+" & Str(Point3.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("OpenHeadTop")
			
			'top arc
			ARMDIA1.PR_MakeXY(Pt1, -TopArcAdj(CInt(txtRadiusNo.Text)), TopArcOpp(CInt(txtRadiusNo.Text)))
			ARMDIA1.PR_MakeXY(Pt2, TopArcAdj(CInt(txtRadiusNo.Text)), TopArcOpp(CInt(txtRadiusNo.Text)))
			ArcAngle1 = ARMDIA1.FN_CalcAngle(Cen, Pt1)
			ArcAngle2 = ARMDIA1.FN_CalcAngle(Cen, Pt2)
			...
			' Left Arc
			ARMDIA1.PR_MakeXY(Pt1, -LeftArcAdjOut(CInt(txtRadiusNo.Text)), LeftArcOppOut(CInt(txtRadiusNo.Text)))
			ARMDIA1.PR_MakeXY(Pt2, -LeftArcAdjIn(CInt(txtRadiusNo.Text)), LeftArcOppIn(CInt(txtRadiusNo.Text)))
			ArcAngle1 = ARMDIA1.FN_CalcAngle(Cen, Pt1)
			ArcAngle2 = ARMDIA1.FN_CalcAngle(Cen, Pt2)
			ArcAngle = (ArcAngle1 - ArcAngle2) * (PI / 180)
			...
			'Top Right arc
			ARMDIA1.PR_MakeXY(Pt1, TopRightArcAdjIn(CInt(txtRadiusNo.Text)), TopRightArcOppIn(CInt(txtRadiusNo.Text)))
			ARMDIA1.PR_MakeXY(Pt2, TopRightArcAdjOut(CInt(txtRadiusNo.Text)), TopRightArcOppOut(CInt(txtRadiusNo.Text)))
			ArcAngle1 = ARMDIA1.FN_CalcAngle(Cen, Pt1)
			ArcAngle2 = ARMDIA1.FN_CalcAngle(Cen, Pt2)
			ArcAngle = (ArcAngle1 - ArcAngle2) * (PI / 180)
			Radius = TopArcHyp(CInt(txtRadiusNo.Text))
			...
			'Right arc
			ARMDIA1.PR_MakeXY(Pt1, RightArcAdjUp(CInt(txtRadiusNo.Text)), RightArcOppUp(CInt(txtRadiusNo.Text)))
			ArcAngle1 = ARMDIA1.FN_CalcAngle(Cen, Pt1)
			ArcAngle2 = ARMDIA1.FN_CalcAngle(Cen, Point3)
			ArcAngle = (ArcAngle1 - ArcAngle2) * (PI / 180)
			...
			ARMDIA1.PR_MakeXY(xyTextPoint, Point1.X + 2, Point1.Y)
			ARMDIA1.PR_SetLayer("Notes")
			'UPGRADE_WARNING: Couldn't resolve default property of object Total. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			ARMDIA1.PR_DrawText(Total & Chr(39) & Chr(39) & " Elastic Strap", xyTextPoint, 0.1, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
	...
	Private Sub PR_DrawRightArc()
		...
		ARMDIA1.PR_MakeXY(xyClosureText, RightArcAdjUp(Circum) - 0.35, 0.25)
		ARMDIA1.PR_SetLayer("Notes")
		If chkZipper.CheckState = 1 Then
			ARMDIA1.PR_DrawText("ZIPPER", xyClosureText, 0.25, -90)
		Else
			ARMDIA1.PR_DrawText("VELCRO", xyClosureText, 0.25, -90)
		End If
		ARMDIA1.PR_SetLayer("TemplateLeft")
		...
		If chkOpenHeadMask.CheckState = 1 Then
			ARMDIA1.PR_MakeXY(xyEndArc, OpenAcross, 0.5)
		Else
			ARMDIA1.PR_MakeXY(xyEndArc, AcrossUp, UpwardsUp)
		End If
		
		ARMDIA1.PR_MakeXY(xyStartArc, AcrossDown, -UpwardsDown)
		nRad = ARMDIA1.FN_CalcLength(xyCentreArc, xyStartArc)
		nStartAng = ARMDIA1.FN_CalcAngle(xyCentreArc, xyStartArc)
		nStartAng = 360 - nStartAng
		nEndAng = ARMDIA1.FN_CalcAngle(xyCentreArc, xyEndArc)
		nDeltaAng = -(nEndAng + nStartAng)
		PrintLine(CInt(txtfNum.Text), "hEnt = AddEntity(" & QQ & "arc" & QC & "xyStart.x +" & Str(xyCentreArc.X) & CC & "xyStart.y +" & Str(xyCentreArc.Y) & CC & Str(nRad) & CC & Str(nEndAng) & CC & Str(nDeltaAng) & ");")
		ARMDIA1.PR_AddEntityID("RightArc")
	...
	Private Sub PR_DrawRightCutOut()
		...
		If chkOpenHeadMask.CheckState = 0 Then
			...
			PrintLine(CInt(txtfNum.Text), CC & "xyStart.x+" & Str(RightTip.X) & CC & "xyStart.y+" & Str(RightTip.Y))
			PrintLine(CInt(txtfNum.Text), ");")
			ARMDIA1.PR_AddEntityID("RightCutOut")
		End If
	...
	Private Sub PR_DrawRightJoin()
		...
		If chkOpenHeadMask.CheckState = 0 Then
			...
			ARMDIA1.PR_MakeXY(Start, AcrossLeft, UpwardsLeft)
			ARMDIA1.PR_MakeXY(Finish, AcrossRight, UpwardsRight)
			ARMDIA1.PR_DrawLine(Start, Finish)
			ARMDIA1.PR_AddEntityID("RightArcJoin")
		End If
	...
	Private Sub PR_DrawRightLine()
		...
		ARMDIA1.PR_MakeXY(xyStart, Across, Down)
		ARMDIA1.PR_MakeXY(xyFinish, Across - LineLen, Down)
		ARMDIA1.PR_DrawLine(xyStart, xyFinish)
		ARMDIA1.PR_AddEntityID("RightArcNeckJoin")
		
	End Sub
	
	Private Sub PR_DrawText(ByRef sText As Object, ByRef xyInsert As xy, ByRef nHeight As Object, ByRef nAngle As Object)
		'To the DRAFIX macro file (given by the global txtfNum).
		...
		PrintLine(CInt(txtfNum.Text), "AddEntity(" & QQ & "text" & QCQ & sText & QC & "xyStart.x+" & Str(xyInsert.X) & CC & "xyStart.y+" & Str(xyInsert.Y) & CC & nWidth & CC & nHeight & CC & nAngle & ");")
	...
	Private Sub PR_DrawTopArc()
		...
		If chkOpenHeadMask.CheckState = 0 Then
			...
			ARMDIA1.PR_MakeXY(xyStartArc, -Across, Upwards)
			ARMDIA1.PR_MakeXY(xyEndArc, Across, Upwards)
			ARMDIA1.PR_DrawArc(xyCentreArc, xyStartArc, xyEndArc)
			ARMDIA1.PR_AddEntityID("TopArc")
		End If
	End Sub
	...
	Private Sub PR_DrawTopRightArc()
		...
		If chkOpenHeadMask.CheckState = 0 Then
			...
			ARMDIA1.PR_MakeXY(xyEndArc, AcrossOut, UpwardsOut)
			ARMDIA1.PR_DrawArc(xyCentreArc, xyStartArc, xyEndArc)
			ARMDIA1.PR_AddEntityID("TopRightArc")
		End If
	...
	Private Sub PR_FaceMask()
		...
		If optFaceMask.Checked = True Then
			...
			PR_StampLining()
			PR_NeckElastic()
			ARMDIA1.PR_MakeXY(xyTextPoint, 0.3, 0.5)
	...
	Private Sub PR_NeckElastic()
		...
		' Stamp For Lining
		If chkNeckElastic.CheckState = 1 Then
			ARMDIA1.PR_MakeXY(xyNeckElastic, CDbl(txtChinLeftBotX.Text) + 0.4, CDbl(txtChinLeftBotY.Text) + 0.5)
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawText("1\"" Elastic Sewn Underneath", xyNeckElastic, 0.1, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
		End If
	...
	Private Sub PR_SaveDetails()
		...
		If txtUidHN.Text = "" Then
			...
			PrintLine(CInt(txtfNum.Text), "XY     xyMPD_Origin, xyMPD_Scale ;")
			PrintLine(CInt(txtfNum.Text), "STRING sMPD_Name;")
			PrintLine(CInt(txtfNum.Text), "ANGLE  aMPD_Angle;")
			
			'UPGRADE_WARNING: Couldn't resolve default property of object QC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "hMPD = UID (" & QQ & "find" & QC & Val(txtUidMPD.Text) & ");")
			...
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "  Exit(%cancel," & QQ & "Can't find > mainpatientdetails < symbol, Insert Patient Data" & QQ & ");")
			
			'Insert headneck
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QCQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "if ( Symbol(" & QQ & "find" & QCQ & sSymbol & QQ & ")){")
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QCQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "  Execute (" & QQ & "menu" & QCQ & "SetLayer" & QC & "Table(" & QQ & "find" & QCQ & "layer" & QCQ & "Data" & QQ & "));")
			'UPGRADE_WARNING: Couldn't resolve default property of object QC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QCQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "  hFacemask = AddEntity(" & QQ & "symbol" & QCQ & sSymbol & QC & "xyMPD_Origin);")
			PrintLine(CInt(txtfNum.Text), "  }")
			PrintLine(CInt(txtfNum.Text), "else")
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "  Exit(%cancel, " & QQ & "Can't find >" & sSymbol & "< symbol to insert\nCheck your installation, that JOBST.SLB exists!" & QQ & ");")
		Else
			'Use existing symbol
			'UPGRADE_WARNING: Couldn't resolve default property of object QC. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "hFacemask = UID (" & QQ & "find" & QC & Val(txtUidHN.Text) & ");")
			'UPGRADE_WARNING: Couldn't resolve default property of object QQ. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
			PrintLine(CInt(txtfNum.Text), "if (!hFacemask) Exit(%cancel," & QQ & "Can't find >" & sSymbol & "< symbol to update!" & QQ & ");")
			
		End If
		
		
		' Update data base fields
		PrintLine(CInt(txtfNum.Text), "SetDBData(hFacemask," & QQ & "HeadNeck" & QCQ & txtMeasurements.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hFacemask," & QQ & "Fabric" & QCQ & cboFabric.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hFacemask," & QQ & "WorkOrder" & QCQ & txtWorkOrder.Text & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "SetDBData(hFacemask," & QQ & "Data" & QCQ & txtData.Text & QQ & ");")
	...
	Private Sub PR_SetChart1Details()
		...
		Dim xyMouthStart, xyMouthEnd As xy
		Dim xyEyeStart, xyEyeEnd As xy
		...
		ARMDIA1.PR_MakeXY(xyMouthStart, PointOne, PointTwo + MouthHt)
		...
		' Mark Mouth Height
		ARMDIA1.PR_MakeXY(xyMouthEnd, PointOne, PointTwo + MouthHt + LipWidth)
		
		If optFaceMask.Checked = True Then
			ARMDIA1.PR_DrawLine(xyMouthStart, xyMouthEnd)
			ARMDIA1.PR_AddEntityID("Lip")
		End If
		' set txtNoseBottomY
		txtNoseBottomY.Text = CStr(PointTwo + MouthHt + LipWidth)
		
		' Mark Eye Height
		ARMDIA1.PR_MakeXY(xyEyeStart, PointOne, 0)
		If chkNoseCovering.CheckState = 1 Then
			ARMDIA1.PR_MakeXY(xyEyeEnd, PointOne, -EyeHt + 0.25)
			txtNoseCoverX.Text = CStr(xyEyeEnd.X)
			txtNoseCoverY.Text = CStr(xyEyeEnd.Y)
		Else
			ARMDIA1.PR_MakeXY(xyEyeEnd, PointOne, -EyeHt)
		End If
		If optFaceMask.Checked = True Then
			ARMDIA1.PR_DrawLine(xyEyeStart, xyEyeEnd)
			ARMDIA1.PR_AddEntityID("EyeBrow")
		End If
	...
	Private Sub PR_SetLayer(ByRef sNewLayer As String)
		...
		'
		PrintLine(CInt(txtfNum.Text), "hLayer = Table(" & QQ & "find" & QCQ & "layer" & QCQ & sNewLayer & QQ & ");")
		PrintLine(CInt(txtfNum.Text), "if ( hLayer > %zero && hLayer != 32768)" & "Execute (" & QQ & "menu" & QCQ & "SetLayer" & QC & "hLayer);")
	...
	Private Sub PR_StampLining()
		...
		If chkLining.CheckState = 1 And chkOpenHeadMask.CheckState <> 1 Then
			ARMDIA1.PR_MakeXY(xyStampLining, -0.5, TopArcOpp(CInt(txtRadiusNo.Text)) - 0.5)
			ARMDIA1.PR_SetLayer("Notes")
			ARMDIA1.PR_DrawText("Lining", xyStampLining, 0.25, 0)
			ARMDIA1.PR_SetLayer("TemplateLeft")
		End If
		
	End Sub
	
	Private Sub PR_TemplateDetails(ByRef xyTextPoint As xy, ByRef TitleText As String)
		Dim xyNotes As xy
		Dim sText As String
		...
		ARMDIA1.PR_SetLayer("Notes")
		sText = TitleText & "\n" & txtPatientName.Text & "\n" & txtWorkOrder.Text & "\n" & cboFabric.Text
		ARMDIA1.PR_MakeXY(xyNotes, xyTextPoint.X, xyTextPoint.Y - 0.125)
		ARMDIA1.PR_DrawText(sText, xyNotes, 0.1, 0)
		
		ARMDIA1.PR_SetLayer("Construct")
		sText = txtFileNo.Text & "\n" & txtDiagnosis.Text & "\n" & txtAge.Text & "\n" & txtSex.Text
		ARMDIA1.PR_MakeXY(xyNotes, xyTextPoint.X, xyTextPoint.Y - 0.9)
		ARMDIA1.PR_DrawText(sText, xyNotes, 0.1, 0)
