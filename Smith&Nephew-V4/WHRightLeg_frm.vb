...
Friend Class WHRightLeg
    ...
    Dim idLegCurve As ObjectId
    Dim g_ptCurveColl As Point3dCollection
    Dim g_xyHeel As XY
    Dim g_bIsSmallHeel As Boolean
    Dim idDistalLine As ObjectId
    Dim idTextAsSymbol As ObjectId
    Dim g_sRightXMarkerHandle As String
    Dim g_nLeftThighCir As Double
    Dim g_nRightThighCir As Double
    Dim g_nWaistCir As Double
    Dim g_nThighGivenRed As Double
    Dim g_sCrotchStyle As String
    ...
    Public Function PR_CloseWaistRightLeg() As Boolean
        Dim Response As Short
        ...
        sCurrentValues = FN_ValuesString()
        If sCurrentValues <> g_sChangeChecker Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Waist - Right Leg")
            Select Case Response
                Case IDYES
                    Dim sDrawFile As String = fnGetSettingsPath("PathDRAW") & "\draw.d"
                    PR_CreateMacro_Save(sDrawFile)
    ...
    Private Sub whlegdia_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            idLegCurve = New ObjectId
            g_ptCurveColl = New Point3dCollection
            g_bIsSmallHeel = False
            idDistalLine = New ObjectId
            idTextAsSymbol = New ObjectId
            g_sRightXMarkerHandle = ""
            g_sCrotchStyle = ""
    ...
    Private Sub readDWGInfo()
        Try
            ...
            If (resbuf IsNot Nothing) Then
                ...
                Next ii
                chkStump.Checked = arr(69).Value
                chkHeelContracture.Checked = arr(70).Value
            End If
        Catch ex As Exception
        End Try
    End Sub
    Private Function PR_GetBlkAttributeValues() As Boolean
        ...
        Dim sJustifiedString As String
        Dim strTapeLengths As String = ""
        Dim ii, nValue As Double
        For ii = 0 To 29
            nValue = Val(Text1(ii).Text)
            If nValue <> 0 Then
                nValue = nValue * 10 'Shift decimal place to right
            ...
            End If
            'Tape values
            strTapeLengths = strTapeLengths & sJustifiedString
        Next ii
        If strTapeLengths.Equals(sTapeLengths) = False Then
            sTapeLengths = strTapeLengths
        End If

        'Leg Style
        Dim cboLegStyle As New ComboBox
        ...
        cboLegStyle.SelectedIndex = nLegStyle
        Try
            Dim _sClass As New SurroundingClass()
            Dim resbuf As New ResultBuffer
            ...
            If (resbuf IsNot Nothing) Then
                Dim arr() As TypedValue = resbuf.AsArray()
                Dim strLegStyle As String = arr(27).Value
                For ii = 0 To (cboLegStyle.Items.Count - 1)
                    If VB6.GetItemString(cboLegStyle, ii) = strLegStyle Then
        ...
        End Try
        If nLegStyle <> cboLegStyle.SelectedIndex Then
            g_sLegStyle = Str(cboLegStyle.SelectedIndex) & Mid(LTrim(g_sLegStyle), 2)
        End If
        Return True
    End Function
    ...
    End Sub
    Sub PR_DrawXMarker(ByRef xyInsertion As XY, Optional ByRef bIsOriginXMarker As Boolean = False, Optional ByRef bIsHeelXMarker As Boolean = False)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                ...
                If bIsOriginXMarker = True Then
                    g_sRightXMarkerHandle = blkRef.Handle.ToString()
                End If
                If bIsHeelXMarker = True Then
                    PR_MakeXY(g_xyHeel, blkRef.Position().X, blkRef.Position().Y)
                End If
            End If
            '' Save the new object to the database
            acTrans.Commit()
    ...
    End Sub
    Private Function FN_GetNumber(ByVal sString As String, ByRef iIndex As Short) As Double
        ...
        For ii = 1 To iIndex
            ...
            Else
                ...
                If Len(sString) = 0 Then
                    ...
                    Exit Function
                End If
                If GetChar(sString, 1).Equals(Chr(34)) And sString.Length > 1 Then
                    sString = LTrim(Mid(sString, 2))
                End If
            End If
        Next ii
        FN_GetNumber = -1
    ...
    Private Sub PR_DrawRuler(ByRef sSymbol As String, ByRef xyStart As XY, ByRef sTape As String)
        ...
        End Using
    End Sub
    Sub PR_DrawLine(ByVal xyStart As XY, ByVal xyFinish As XY, Optional ByVal bIsDistalLine As Boolean = False)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            ''If bIsNotScale = False Then
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                acLine.TransformBy(Matrix3d.Scaling(2.54, New Point3d(xyOtemplate.X, xyOtemplate.Y, 0)))
            End If
            ''Else
            'Dim Diff As Double = System.Math.Abs(xyOtemplate.Y - xyStart.Y)
            'Dim YVal As Double = (Diff * 2.54) - Diff
            'acLine = New Line(New Point3d(xyStart.X, xyStart.Y + YVal, 0),
            '                    New Point3d(xyFinish.X, xyFinish.Y + YVal, 0))
            ''End If
            '' Add the new object to the block table record and the transaction
            acBlkTblRec.AppendEntity(acLine)
            acTrans.AddNewlyCreatedDBObject(acLine, True)
            idLastCreated = acLine.ObjectId
            If bIsDistalLine = True Then
                idDistalLine = acLine.ObjectId
            End If
    ...
    End Sub
    Private Sub readStretchInfo(ByRef bIsStretch As Boolean, ByRef bIsLeftStretch As Boolean)
        Try
            Dim _sClass As New SurroundingClass()
            Dim resbuf As New ResultBuffer
            resbuf = _sClass.GetXrecord("WaistFigure", "WHFIGDIC")
            If (resbuf IsNot Nothing) Then
                Dim arr() As TypedValue = resbuf.AsArray()
                bIsStretch = arr(12).Value
                bIsLeftStretch = arr(11).Value
            End If
        Catch ex As Exception
    ...
    Private Sub Button1_Click(sender As Object, e As EventArgs) Handles Button1.Click
        PR_DrawSecondLeg()
    End Sub
    Public Sub PR_DrawWaistSecondLeg()
        'Initialize globals
        QQ = Chr(34) 'Double quotes (")
        NL = Chr(13) 'New Line
        CC = Chr(44) 'The comma (,)
    ...
    Private Sub PR_DrawSecondLeg()
        ...
        If WHBODDIA1.fnGetNumber(g_sLegStyle, 1) = 3 Or WHBODDIA1.fnGetNumber(g_sLegStyle, 1) = 4 Then
            MsgBox("Can't draw a second leg for this style", 16, "Waist - Leg Dialog")
            Exit Sub
        End If
        If WHBODDIA1.fnGetNumber(g_sLegStyle, 1) >= 5 Then
            PR_DrawChapStyleRightLeg()
            Exit Sub
        End If
        ''For Brief Style
        If WHBODDIA1.fnGetNumber(g_sLegStyle, 1) = 2 Then
            PR_DrawBriefStyleRightLeg()
            Exit Sub
        End If
        ...
        ''nBodyLegTapePos = ARMDIA1.round(nFoldHt / 1.5) + 6
        ''Dim FootLess As Boolean = WHRightLeg1.fnDisplayToInches(Val(Text1(5).Text))
        Dim FootLess As Boolean = True
        If Val(Text1(5).Text) > 0 Then
            FootLess = False
        End If
        Dim nFootPleat1, nFootPleat2, nTopLegPleat1, nTopLegPleat2 As Double
        nFootPleat1 = WHRightLeg1.fnDisplayToInches(Val(txtFootPleat1.Text))
        nFootPleat2 = WHRightLeg1.fnDisplayToInches(Val(txtFootPleat2.Text))
        nTopLegPleat1 = WHRightLeg1.fnDisplayToInches(Val(txtTopLegPleat1.Text))
        nTopLegPleat2 = WHRightLeg1.fnDisplayToInches(Val(txtTopLegPleat2.Text))
        nFirstTape = -1
        nLastTape = 30
        ...
        ''Added for #201 in the issue list
        Dim nLoopLast As Double = nBodyLegTapePos
        ...
        If (nFoldHt > nExtTempltTol3min And FootLess = False) Then
            ...
            nLoopLast = nBodyLegTapePos - 2
        End If
        ''Added for #201 in the issue list
        Dim sFilePath As String
        If (nFabricClass = 0) Then
            sFilePath = fnGetSettingsPath("LookupTables") + "\\WH" + StringMiddle(sPressure, 1, 2) + "MMHG.DAT"
        Else
            sFilePath = fnGetSettingsPath("LookupTables") + "\\WH" + StringMiddle(sPressure, 1, 2) + "DS.DAT"
        End If
        Dim hFile As Object
        hFile = FreeFile()
        FileOpen(hFile, sFilePath, VB.OpenMode.Input)
        Dim nNoVal, nSpaceVal, n20LenVal, nReductionVal As Double
        Dim sReadLine As String = ""
        Dim sScaleVal As String = ""
        If (hFile) Then
            sReadLine = LineInput(hFile)
            nNoVal = WHBODDIA1.fnGetNumber(sReadLine, 1)
            sScaleVal = FN_GetString(sReadLine, 2)
            nSpaceVal = WHBODDIA1.fnGetNumber(sReadLine, 3)
            n20LenVal = WHBODDIA1.fnGetNumber(sReadLine, 4)
            nReductionVal = WHBODDIA1.fnGetNumber(sReadLine, 5)
        Else
            MsgBox("Can't open " + sFilePath + "\nCheck installation", 16, "Waist Leg - Dialog")
            Exit Sub
        End If
        Dim nIter As Double = 1
        While (nIter < nFirstTape)
            sReadLine = LineInput(hFile)
            nIter = nIter + 1
        End While
        Dim nLoopVal As Double = nFirstTape
        ScanLine(sReadLine, nNoVal, sScaleVal, nSpaceVal, n20LenVal, nReductionVal)
        Dim bIsStretchExist As Boolean = False
        Dim bIsLeftStretch As Boolean = False
        readStretchInfo(bIsStretchExist, bIsLeftStretch)
        Dim nNeedToAdd As Double = 0
        While (nLoopVal <= nLoopLast)
            If bIsStretchExist = True And nNoVal > 6 Then
                nNeedToAdd = nNeedToAdd + 0.125
            End If
            nLoopVal = nLoopVal + 1
            sReadLine = LineInput(hFile)
            ScanLine(sReadLine, nNoVal, sScaleVal, nSpaceVal, n20LenVal, nReductionVal)
        End While
        FileClose(hFile)
        nLegStartHt = nLegStartHt + nNeedToAdd

        Dim nInchToCM As Double = 1
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            nInchToCM = 2.54
            Dim nVal As Double = (nLegStartHt * nInchToCM) - nLegStartHt
            xyTOS.X = xyTOS.X - nVal
            xyWaist.X = xyWaist.X - nVal
            xyMidPoint.X = xyMidPoint.X - nVal
            xyLargest.X = xyLargest.X - nVal
            xyFold.X = xyFold.X - nVal
            xyFoldPanty.X = xyFoldPanty.X - nVal
            xyButtockArcCen.X = xyButtockArcCen.X - nVal
        End If
        ...
        ''// Offset body origin
        xyO.Y = xyOtemplate.Y
        xyO.X = xyOtemplate.X + (nLegStartHt * nInchToCM)
        ARMDIA1.PR_SetLayer("Construct")
        ''hEnt = AddEntity("marker", "xmarker", xyOtemplate, 0.125)
        PR_DrawXMarker(xyOtemplate, True)
        PR_AddDBValueToLast("Handle", g_sRightXMarkerHandle)
        ''If (hEnt) Then {
        ...
        PR_AddDBValueToLast("OriginID", txtFileNo.Text + g_sTxtLeg + "Origin")
        ''SetDBData(hEnt, "units", sUnits) ;
        PR_AddDBValueToLast("units", txtUnits.Text)
        If (FootLess) Then
            ''SetDBData(hEnt, "Data", "WHPT") ;
            PR_AddDBValueToLast("Data", "WHPT")
        Else
            ''SetDBData(hEnt, "Data", "WHFT") ;
            PR_AddDBValueToLast("Data", "WHFT")
        End If
        ''SetDBData(hEnt, "Pressure", sPressure) ;
        PR_AddDBValueToLast("Pressure", sPressure)
        ''SetDBData(hEnt, "TapeLengthsPt1", StringMiddle(sTapeLengths, 1, 60));
        PR_AddDBValueToLast("TapeLengthsPt1", Mid(sTapeLengths, 1, 60))
        ''SetDBData(hEnt, "TapeLengthsPt2", StringMiddle(sTapeLengths, 61, StringLength(sTapeLengths) - 60));
        PR_AddDBValueToLast("TapeLengthsPt2", Mid(sTapeLengths, 61, sTapeLengths.Length - 60))
        ''SetDBData(hEnt, "patient", sPatient) ;
        PR_AddDBValueToLast("patient", txtPatientName.Text)
        ''SetDBData(hEnt, "age", sAge) ;
        PR_AddDBValueToLast("age", txtAge.Text)
        ''SetDBData(hEnt, "WorkOrder", sWorkOrder) ;
        PR_AddDBValueToLast("WorkOrder", sWorkOrder)
        ...
        PR_AddDBValueToLast("Body", sLegStyle)
        ''SetDBData(hEnt, "FirstTape", MakeString("scalar", nFirstTape));
        PR_AddDBValueToLast("FirstTape", Str(nFirstTape))
        ''SetDBData(hEnt, "LastTape", MakeString("scalar", nLastTape));
        PR_AddDBValueToLast("FirstTape", Str(nLastTape))
        '' }
        ...
        If nLtLastHeel = 0 Then
            nAnkleTape = nFirstTape
        End If
        If (nFabricClass = 2 And FootLess = False) Then
        ...
        If (nLegStyle = 1) Then
            PantyLeg = True
        End If
        ''-------------Added for #77 in issue list---------------
        Dim bIsStretch As Boolean = False
        readStretchInfo(bIsStretch, bIsLeftStretch)
        While (nn <= nLoop)
            ...
            If (FNGetTape(nn)) Then
                ...
                ''------------Changed for #77 in issue list---------------
                If bIsStretch = True And nNo > 6 Then
                ...
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    ...
                    PR_DrawText(Text1(nn - 1).Text, xyText, 0.125, 0, 1)
                Else
                    PR_DrawText(WHRightLeg1.fnInchesToText(nTapeLen), xyText, 0.125, 0, 1)
                End If
                ...
                If (nn = nAnkleTape And PantyLeg = False) Then
                    ...
                    PR_DrawText(Mid(sReductionAnkle, ((nn - 1) * 3) + 1, 3), xyText, 0.125, 0, 1)
                    If (nFabricClass = 0) Then
                        ...
                        PR_DrawText(Mid(sGramsAnkle, ((nn - 1) * 3) + 1, 3) + " grams", xyText, 0.125, 0, 1)
                    Else
                        ''AddEntity("text", sGramsAnkle + " stretch", xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.6)
                        ''PR_DrawText(Mid(sGramsAnkle, (nn * 3) + 1, 3) + " stretch", xyText, 0.125, 0, 1)
                        PR_DrawText(Mid(sGramsAnkle, ((nn - 1) * 3) + 1, 3) + " stretch", xyText, 0.125, 0, 1)
                    End If
                    ...
                    PR_DrawText(Mid(sMMAnkle, ((nn - 1) * 3) + 1, 3) + " mm", xyText, 0.125, 0, 1)
        ...
        ''Added for #134 in the issue list
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            xyLargestFirstBody.X = xyLargest.X + ((nLegStartHt * nInchToCM) - nLegStartHt)
        End If
        ...
        Dim xyTemp As XY
        PR_MakeXY(xyTemp, xyOtemplate.X + nLegStartHt, xyOtemplate.Y)
        PR_DrawLine(xyTemp, xyEnd)
        ...
        PR_DrawLine(xyStart, xyEnd)
        PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "FoldLine")
        ''--------PR_AddDBValueToLast("Data", Str(nThighCir) + " " + Str(nGivenWaistCir))
        ...
        ''// Store figured thigh cir. And actual waist cir
        ''SetDBData(hEnt, "Data", MakeString("scalar", nThighCir) + " " + MakeString("scalar", nGivenWaistCir))
        PR_AddDBValueToLast("EOSID", txtFileNo.Text + g_sTxtLeg + "EOSLine")
        Dim nThighCirOriginal As Double = WHBODDIA1.min(g_nLeftThighCir, g_nRightThighCir)
        ''Changed for #209 in the issue list
        ''Dim nThighCir As Double = ARMDIA1.round(nThighCirOriginal * g_nThighGivenRed)
        ''PR_AddDBValueToLast("Data", Str(nThighCir) + " " + Str(g_nWaistCir))
        Dim nThighCir As Double = WHRightLeg1.fnRoundInches(nThighCirOriginal * g_nThighGivenRed)
        PR_AddDBValueToLast("Data", Str(nThighCir * nYscale + nSeam) + " " + Str(g_nWaistCir))
        ...
        End If
        ''xyPt1.X = xyO.X
        xyPt1.X = xyOtemplate.X + nLegStartHt
        ...
        PR_MakeXY(xyStart, xyOtemplate.X + nLegStartHt, xyO.Y + nSeam)
        ...
        PR_MakeXY(xyStart, xyOtemplate.X + nLegStartHt, xyO.Y + nSeam + 0.5)
        ...
        Else
            MsgBox("Can't open " + sFile + Chr(10) + "Check installation", 16, "Waist Leg - Dialog")
            Exit Sub
        End If
        ...
        Dim nReductionAnkle As Double = Val(Mid(sReductionAnkle, ((nAnkleTape - 1) * 3) + 1, 3))
        ...
        If (SmallHeel) Then
            ...
            nAngle = FN_CalcAngle(xyHeelCntrMidDistal, xyAnkleM) - (System.Math.Acos((nHeelR1 ^ 2 - nHeelD1 ^ 2 - nCalc ^ 2) / (-2 * nHeelD1 * nCalc)) * (180 / PI))
        ...
        If (SmallHeel) Then
            ...
            nAngle = FN_CalcAngle(xyHeelCntrMidProximal, xyAnkle) + (System.Math.Acos((nHeelR3 ^ 2 - nHeelD2 ^ 2 - nCalc ^ 2) / (-2 * nHeelD2 * nCalc)) * (180 / PI))
        ...
        If (sToeStyle.Equals("Self Enclosed") Or (sToeStyle.Equals("Soft Enclosed") And Val(sFootLength))) Then
            ...
            nFootLength = FNDecimalise(Val(sFootLength)) * WHRightLeg1.g_nUnitsFac
        ...
        While (nn <= nLastTape)
            ...
            End If
            xyTmp.Y = xyOtemplate.Y + nSeam + nLength
            If ((nn = nFirstTape) And FootLess = True) Then
                xyTmp.Y = xyLastTape.Y
            End If
            ...
            If (nFabricClass = 2 And FootLess = False) Then
                ...
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    ''PR_DrawText(fnDisplayToCM(nTapeLen), xyText, 0.1, 0, 1)
                    PR_DrawText(Text1(nn - 1).Text, xyText, 0.1, 0, 1)
                Else
                    PR_DrawText(WHRightLeg1.fnInchesToText(nTapeLen), xyText, 0.1, 0, 1)
                End If
        ...
        ''------------Added for #134 in the issue list
        Dim nLeftLegStartHt As Double = 0
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            Dim xyLen As XY
            xyLen.X = xyOtemplate.X + nLegStartHt
            xyLen.Y = xyOtemplate.Y
            aTransAngle = FN_CalcAngle(xyOFirstBody, xyLen)
            nTrans = FN_CalcLength(xyOFirstBody, xyLen)
            Dim LeftFootLess As Boolean
            Dim nLeftFirstTape, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2 As Double
            nLeftFirstTape = -1
            nLeftLastTape = 30
            readLeftLegDWGInfo(LeftFootLess, nLeftFirstTape, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2)
            nLoopLast = nBodyLegTapePos
            If (nFoldHt < nExtTempltTol1max And LeftFootLess = False) Then
                nLeftLegStartHt = FNLegStartHt(nLeftFirstTape, nBodyLegTapePos, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2, LeftFootLess)
            End If
            If (LeftFootLess) Then
                nLeftLegStartHt = FNLegStartHt(nLeftFirstTape, nBodyLegTapePos, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2, LeftFootLess) '' // Panty
            End If
            If ((nFoldHt >= nExtTempltTol2min And nFoldHt <= nExtTempltTol2max) And LeftFootLess = False) Then
                nLeftLegStartHt = FNLegStartHt(nLeftFirstTape, nBodyLegTapePos - 1, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2, LeftFootLess)
                nLoopLast = nBodyLegTapePos - 1
            End If
            If (nFoldHt > nExtTempltTol3min And LeftFootLess = False) Then
                nLeftLegStartHt = FNLegStartHt(nLeftFirstTape, nBodyLegTapePos - 2, nLeftLastTape, nLeftFootPleat1, nLeftFootPleat2, nLeftTopLegPleat1, nLeftTopLegPleat2, LeftFootLess)
                nLoopLast = nBodyLegTapePos - 2
            End If
            ''Added for #201 and #246 in the issue list
            sFilePath = ""
            If (nFabricClass = 0) Then
                sFilePath = fnGetSettingsPath("LookupTables") + "\\WH" + StringMiddle(sPressure, 1, 2) + "MMHG.DAT"
            Else
            ...
            hFile = FreeFile()
            FileOpen(hFile, sFilePath, VB.OpenMode.Input)
            Dim nLeftNoVal, nLeftSpaceVal, nLeft20LenVal, nLeftReductionVal As Double
            sReadLine = ""
            Dim sLeftScaleVal As String = ""
            If (hFile) Then
                sReadLine = LineInput(hFile)
                nLeftNoVal = WHBODDIA1.fnGetNumber(sReadLine, 1)
                sLeftScaleVal = FN_GetString(sReadLine, 2)
                nLeftSpaceVal = WHBODDIA1.fnGetNumber(sReadLine, 3)
                nLeft20LenVal = WHBODDIA1.fnGetNumber(sReadLine, 4)
                nLeftReductionVal = WHBODDIA1.fnGetNumber(sReadLine, 5)
            Else
                MsgBox("Can't open " + sFilePath + Chr(10) + "Check installation", 16, "Waist Leg - Dialog")
                Exit Sub
            End If
            nIter = 1
            While (nIter < nLeftFirstTape)
                sReadLine = LineInput(hFile)
                nIter = nIter + 1
            End While
            nLoopVal = nLeftFirstTape
            ScanLine(sReadLine, nLeftNoVal, sLeftScaleVal, nLeftSpaceVal, nLeft20LenVal, nLeftReductionVal)
            nNeedToAdd = 0
            While (nLoopVal <= nLoopLast)
                If bIsLeftStretch = True And nLeftNoVal > 6 Then
                    nNeedToAdd = nNeedToAdd + 0.125
                End If
                nLoopVal = nLoopVal + 1
                sReadLine = LineInput(hFile)
            ...
            End While
            FileClose(hFile)
            nLeftLegStartHt = nLeftLegStartHt + nNeedToAdd
        End If
        ''--------------------
        While (ii <= nLegVertexCount)
            ...
            ''Added for #134 in the issue list
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                Dim nVal As Double = (nLeftLegStartHt * 2.54) - nLeftLegStartHt
                nX = nX + nVal
            End If
        ...
        PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "LegCurve")
        ...
        If (FootLess = False) Then
            ...
            sTmp = Str(xyHeelCntrProximal.X - xyAnkle.X) + " " + Str(xyHeelCntrProximal.Y - xyAnkle.Y)
            ''SetDBData(hEnt, "Data", sTmp);
            PR_DrawXMarker(xyAnkle)
            PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "Ankle")
            ...
            PR_DrawXMarker(xyHeel, False, True)
            ''g_xyHeel = xyHeel
            PR_AddDBValueToLast("HeelID", txtFileNo.Text + g_sTxtLeg + "Heel")
            ...
            PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "AnkleM")
            PR_AddDBValueToLast("Data", sTmp)
            ...
            If (nFabricClass = 2) Then
                ...
                PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "Fold")
            End If
            ...
            ''PR_DrawLine(xyO, xyEnd, False, True)
            PR_DrawLine(xyTemp, xyEnd)
            ...
            If (sToeStyle.Equals("Soft Enclosed") And nFootLength <> 0) Then
                ...
                sFootLabel = WHRightLeg1.fnInchesToText(nRightOffset) + Chr(34) + " Soft Enclosed"
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            ...
            PR_DrawText(sFootLabel, xyText, 0.1, 0, 3)
            ...
            ''PR_DrawLine(xyStart, xyEnd, False, True)
            PR_DrawLine(xyStart, xyEnd)
        ...
        Else
            ...
            PR_MakeXY(xyToeSeam, xyOtemplate.X, xyOtemplate.Y)
            PR_MakeXY(xyStart, xyToeSeam.X, xyOtemplate.Y + nSeam)
            ''PR_MakeXY(xyEnd, xyO.X, xyOtemplate.Y + nSeam)
            PR_MakeXY(xyEnd, xyOtemplate.X + nLegStartHt, xyOtemplate.Y + nSeam)
            PR_DrawLine(xyStart, xyEnd)
            ...
            PR_MakeXY(xyEnd, xyOtemplate.X + nLegStartHt, xyOtemplate.Y + nSeam + 0.5)
            PR_DrawLine(xyStart, xyEnd)
            ''//Elastic note
            ...
            Dim xySymbol As XY
            PR_MakeXY(xySymbol, xyLastTape.X + 0.25, xyOtemplate.Y + (xyLastTape.Y - xyOtemplate.Y) / 2)
            PR_DrawTextAsSymbol(xySymbol, txtFileNo.Text + g_sTxtLeg + "PantyElasticNote", sFootLabel, 90)
            PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "PantyElasticNote")
            PR_AddDBValueToLast("Data", sFootLabel)
            ...
            PR_DrawLine(xyLastTape, xyOtemplate, True)
            PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "DistalClosingLine")
            ...
            PR_DrawLine(xyTemp, xyOtemplate)
        End If
        ''// Restore to layer 1
        ''Execute("menu", "SetLayer", Table("find", "layer", "1"))
        If chkStump.Checked = True Then
            PR_DrawWaistLegStump()
        End If
        If chkHeelContracture.Checked = True Then
            PR_DrawWaistHeelContracture()
        End If
    End Sub
    ''To read Left Leg Drawing Information
    Private Sub readLeftLegDWGInfo(ByRef FootLess As Boolean, ByRef nFirstTape As Double, ByRef nLastTape As Double, ByRef nFootPleat1 As Double,
                                   ByRef nFootPleat2 As Double, ByRef nTopLegPleat1 As Double, ByRef nTopLegPleat2 As Double)
        Try
            Dim _sClass As New SurroundingClass()
            Dim resbuf As New ResultBuffer
            resbuf = _sClass.GetXrecord("WaistLeftLeg", "WHLEFTDIC")
            If (resbuf IsNot Nothing) Then
                Dim arr() As TypedValue = resbuf.AsArray()
                nFootPleat1 = WHRightLeg1.fnDisplayToInches(Val(arr(0).Value))
                nFootPleat2 = WHRightLeg1.fnDisplayToInches(Val(arr(1).Value))
                nTopLegPleat2 = WHRightLeg1.fnDisplayToInches(Val(arr(2).Value))
                nTopLegPleat1 = WHRightLeg1.fnDisplayToInches(Val(arr(3).Value))
                FootLess = True
                If Val(arr(9).Value) > 0 Then
                    FootLess = False
                End If
                Dim ii As Double
                For ii = 0 To 29
                    Dim nValue As Double = Val(arr(ii + 4).Value)
                    'Set first and last tape (assumes no holes in data)
                    If CDbl(nFirstTape) < 0 And nValue > 0 Then
                        nFirstTape = CStr(ii + 1)
                    End If
    ...
    Private Sub PR_DrawSpline(ByRef PointCollection As Point3dCollection)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acSpline As Spline = New Spline(PointCollection, vecTan, vecTan, 4, 0.0)
                ...
                idLegCurve = acSpline.ObjectId
            End Using
            '' Save the new object to the database
            acTrans.Commit()
        End Using
    End Sub
    Private Sub PR_DrawTextAsSymbol(ByRef xyPoint As XY, ByRef sID As String, ByRef sData As String, ByRef nAngle As Double)
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
            strLayout = BlockTableRecord.PaperSpace
        End If
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                'Create new block reference 
                Dim blkRef As BlockReference = New BlockReference(New Point3d(xyPoint.X + xyOtemplate.X, xyPoint.Y + xyOtemplate.Y, 0), blkRecId)
                ...
                acTrans.AddNewlyCreatedDBObject(blkRef, True)
                idLastCreated = blkRef.ObjectId
                idTextAsSymbol = blkRef.ObjectId
                '' Open the Block table record Model space for write
                Dim blkTblRec As BlockTableRecord
                blkTblRec = acTrans.GetObject(blkRecId, OpenMode.ForRead)
                For Each objID As ObjectId In blkTblRec
    ...
    Sub PR_AddDBValueToLast(ByRef sDBName As String, ByRef sDBValue As String)
        ...
        End Using
    End Sub
    Private Sub PR_DrawWaistLegStump()
        If g_ptCurveColl.Count = 0 Then
            MsgBox("Can't find Leg profile", 16, "Leg Stump")
            Exit Sub
        End If
        If idLegCurve.IsValid <> True Then
            MsgBox("Can't find Leg profile", 16, "Leg Stump")
        End If
        ...
        If selResult.Status <> PromptStatus.OK Then
            MsgBox("Origin Marker not found", 16, "Leg Stump")
            Exit Sub
        End If
        ...
        If (OriginFound = False) Then
            MsgBox("Origin Marker not found!" & Chr(10) & "Unable to continue.", 16, "Leg Stump")
            Exit Sub
        End If

        ''Dim sTapeLengths As String = g_sTxtTapeLengths
        ''Dim nFirstTape As Double = Val(g_sTxtFirstTape)
        ''Dim nLastTape As Double = Val(g_sTxtLastTape)
        Dim nFirstTape As Double = -1
        Dim nLastTape As Double = 30
        Dim sTapeLengths As String = ""
        Dim sJustifiedString As String = ""
        Dim ii, nValue As Double
        For ii = 0 To 29
            nValue = Val(Text1(ii).Text)
            ...
            End If
            'Tape values
            sTapeLengths = sTapeLengths & sJustifiedString
        ...
        Dim nTapeLen As Double = ARMDIA1.round(FNDecimalise(FNGetTape(nFirstTape)) * WHRightLeg1.g_nUnitsFac)
        Dim nSeam As Double = 0.1875
        Dim nLength As Double = ((nTapeLen * 0.78) / 2) + nSeam
        Dim nTol As Double = System.Math.Abs(nLength - ((nTapeLen * 0.95) + nSeam)) + 0.125
        Dim xyDistal, xyTmp As XY
        ...
        If g_ptCurveColl.Count > 0 Then
            PR_MakeXY(xyTmp, g_ptCurveColl(0).X, g_ptCurveColl(0).Y)
        End If
        ...
        End If
        '' // Draw Stumps
        Dim xyFirstProfile As XY = xyDistal
        ...
        If (nLength > 3) Then
            ...
            Dim xyPt2, xyMidPt, xyApexPt, xyInt, xyPt3 As XY
            ...
            PR_DrawLine(xyPt2, xyTmp)
            Dim xyStart, xyEnd, xyArrow As XY
            ...
            PR_DrawWaistClosedArrow(xyArrow, 270)
        Else
            ''// Circular stump
            'StartPoly("polyline") ;
            '  AddVertex(xyOtemplate);
            ...
            PR_DrawText(txtPatientName.Text, xyTmp, 0.125, 0, 2)
            ''----------SetData("TextVertJust", 32) ;
            ''-----------AddEntity("text", sWorkOrder, xyTmp);
            PR_DrawText(sWorkOrder, xyTmp, 0.125, 0, 8)
        End If
        ''// Clean up
        ''--------If (hElasticNote) Then DeleteEntity(hElasticNote) ;
        ''----------If (hClosingLine) Then DeleteEntity(hClosingLine) ;
        Using acTransaction As Transaction = acCurDb.TransactionManager.StartTransaction()
            If idTextAsSymbol.IsNull <> True And idTextAsSymbol.IsValid = True Then
                Dim acFstEnt As Entity = acTransaction.GetObject(idTextAsSymbol, OpenMode.ForWrite)
                acFstEnt.Erase()
            End If
            If idDistalLine.IsNull <> True And idDistalLine.IsValid = True Then
                Dim acSecEnt As Entity = acTransaction.GetObject(idDistalLine, OpenMode.ForWrite)
                acSecEnt.Erase()
            End If
    ...
    End Sub
    Sub PR_DrawWaistClosedArrow(ByRef xyPoint As XY, ByRef nAngle As Object)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        End Using
    End Sub
    Sub PR_DrawStumpCircle(ByRef xyCen As XY, ByRef nRadius As Object)
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
            strLayout = BlockTableRecord.PaperSpace
        End If
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acCirc As Circle = New Circle()
                ...
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acCirc.TransformBy(Matrix3d.Scaling(2.54, New Point3d(xyOtemplate.X, xyOtemplate.Y, 0)))
                End If
        ...
        End Using
    End Sub
    Private Sub PR_DrawWaistHeelContracture()
        Dim FootLess As Boolean = True
        If Val(Text1(5).Text) > 0 Then
            FootLess = False
        End If
        If FootLess = True Then
            MsgBox("Heel marker data is missing!", 16, "Heel Contracture")
            Exit Sub
        End If

        ''// Calculate contracture points
        Dim nHeelOff As Double
        If (g_bIsSmallHeel) Then
            nHeelOff = 0.25
        ...
        End If
        Dim xyHeel As XY
        PR_MakeXY(xyHeel, g_xyHeel.X, g_xyHeel.Y)
        Dim nHeelCir As Double = xyHeel.Y - xyOtemplate.Y
        Dim xyPt1, xyPt2, xyPt3 As XY
        ...
        ''nHeelCir = Val(Text1(5).Text) / 2.7
        ''xyPt2.Y = xyOtemplate.Y + (nHeelCir / 3)
        nHeelCir = (Val(Text1(5).Text) / nCmToInch) / 2.75
    ...
    Private Sub PR_DrawHeelContracturePoly(ByRef PointCollection As Point3dCollection)
        ...
        End Using
    End Sub
    Private Sub PR_DrawHeelContractureText(ByRef sText As Object, ByRef xyInsert As XY, ByRef nHeight As Object, ByRef nAngle As Object, ByVal nTextmode As Double)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
    ...
    Private Sub PR_DrawChapStyleRightLeg()
        ''----------------------WHLGCORG.D-----------------------------
        PR_GetInsertionPoint(xyOtemplate)
        Dim ii, nFirstTape, nLastTape, nValue As Double
        nFirstTape = -1
        nLastTape = 30
        For ii = 0 To 29
            nValue = Val(Text1(ii).Text)
        ...
        PR_AddDBValueToLast("units", txtUnits.Text)
        PR_AddDBValueToLast("age", txtAge.Text)
        PR_AddDBValueToLast("Data", "CHAP")
        PR_AddDBValueToLast("Pressure", sPressure)
        PR_AddDBValueToLast("TapeLengthsPt1", Mid(sTapeLengths, 1, 60))
        PR_AddDBValueToLast("TapeLengthsPt2", StringMiddle(sTapeLengths, 61, sTapeLengths.Length - 60))
        ''SetDBData(hEnt, "WaistCir", sWaistCir)
        ''SetDBData(hEnt, "WaistHt", sWaistHt)
        ''SetDBData(hEnt, "TOSCir", sTOSCir)
        ''SetDBData(hEnt, "TOSHt", sTOSHt)
        PR_AddDBValueToLast("patient", txtPatientName.Text)
        PR_AddDBValueToLast("WorkOrder", sWorkOrder)
        ''PR_AddDBValueToLast("TOSGivenRed", sTOSRed)
        ''PR_AddDBValueToLast("WaistGivenRed", sWaistRed)
        ''PR_AddDBValueToLast("FoldHt", sFoldHt)
        Dim sLegStyle As String = ""
        PR_AddDBValueToLast("Body", sLegStyle)
        PR_AddDBValueToLast("FirstTape", Str(nFirstTape))
        PR_AddDBValueToLast("LastTape", Str(nLastTape))
        ...
        ''//   hEnt = AddEntity("marker","xmarker",xyOtemplate , 0.125) ;	
        ''//   SetDBData (hEnt,"ID",sFileNo + sLeg + "Origin" );
        ''//   SetDBData (hEnt,"units", sUnits) ;
        ...
        ''// Loop through displaying points and scales
        ''//
        Dim nLoop, nAnkleTape, nPleatGiven As Double
        Dim nLtLastHeel As Double = WHRightLeg1.fnDisplayToInches(Val(Text1(5).Text))
        Dim PantyLeg As Boolean = False
        Dim nLegStyle As Double = WHBODDIA1.fnGetNumber(g_sLegStyle, 1)
        If (nLegStyle = 1) Then
            PantyLeg = True
        ...
        Dim sToeStyle As String = cboToeStyle.Text
        Dim nTapeLen, nLength, jj, nTickStep, nTickHt As Double
        Dim xyTmp, xyText, xyStart, xyEnd As XY
        ''-------------Added for #77 in issue list---------------
        Dim bIsStretch As Boolean = False
        Dim bIsLeftStretch As Boolean = False
        readStretchInfo(bIsStretch, bIsLeftStretch)
        While (nn <= nLoop)
            ...
            If (FNGetTape(nn)) Then
                ...
                End If
                xyTmp.X = xyPt1.X
                ''nTapeLen = FNRound(FNDecimalise(FNGetTape(nn)) * WHRightLeg1.g_nUnitsFac)
                nTapeLen = FNDecimalise(FNGetTape(nn)) * WHRightLeg1.g_nUnitsFac
                nLength = n20Len / 20 * nTapeLen
                xyTmp.Y = xyOtemplate.Y + nSeam + nLength
                PR_DrawXMarker(xyTmp)
                ...
                If (nn = nAnkleTape And PantyLeg = False) Then
                    ''AddEntity("text", sReductionAnkle, xyPt1.X + 0.4, nTickHt - 0.4)
                    PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.4)
                    ''PR_DrawText(sReductionAnkle(nAnkleTape), xyText, 0.125, 0, 1)
                    ''PR_DrawText(Mid(sReductionAnkle, (nn * 3) + 1, 3), xyText, 0.125, 0, 1)
                    PR_DrawText(Mid(sReductionAnkle, ((nn - 1) * 3) + 1, 3), xyText, 0.125, 0, 1)
                    If (nFabricClass = 0) Then
                        ''AddEntity("text", sGramsAnkle + " grams", xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.6)
                        ''PR_DrawText(sGramsAnkle(nAnkleTape) + " grams", xyText, 0.125, 0, 1)
                        ''PR_DrawText(Mid(sGramsAnkle, (nn * 3) + 1, 3) + " grams", xyText, 0.125, 0, 1)
                        PR_DrawText(Mid(sGramsAnkle, ((nn - 1) * 3) + 1, 3) + " grams", xyText, 0.125, 0, 1)
                    Else
                        ''AddEntity("text", sGramsAnkle + " stretch", xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.6)
                        ''PR_DrawText(sGramsAnkle(nAnkleTape) + " stretch", xyText, 0.125, 0, 1)
                        ''PR_DrawText(Mid(sGramsAnkle, (nn * 3) + 1, 3) + " stretch", xyText, 0.125, 0, 1)
                        PR_DrawText(Mid(sGramsAnkle, ((nn - 1) * 3) + 1, 3) + " stretch", xyText, 0.125, 0, 1)
                    End If
                    ''AddEntity ("text", sMMAnkle + " mm", xyPt1.x + 0.4, nTickHt - 0.8) 
                    PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.8)
                    ''PR_DrawText(sMMAnkle(nAnkleTape) + " mm", xyText, 0.125, 0, 1)
                    ''PR_DrawText(Mid(sMMAnkle, (nn * 3) + 1, 3) + " mm", xyText, 0.125, 0, 1)
                    PR_DrawText(Mid(sMMAnkle, ((nn - 1) * 3) + 1, 3) + " mm", xyText, 0.125, 0, 1)
                    ''AddEntity("marker", "xmarker", xyPt1.X + 0.3, nTickHt - 0.5, 0.2, 0.2)
                    PR_MakeXY(xyText, xyPt1.X + 0.3, nTickHt - 0.5)
                ...
                While (ii <= jj)
                    ...
                    ''AddEntity("text", MakeString("long", ii), xyPt1.X + nSeam, nTickHt - 0.01, 0.06, 0.1, 90)
                    PR_MakeXY(xyText, xyPt1.X + nSeam, nTickHt - 0.01)
                    PR_DrawText(Str(ii), xyText, 0.1, (90 * (PI / 180)), nTextMode)
                    nTickLength = 0.05
                    ''AddEntity("line", xyPt1.X, nTickHt + nTickStep, xyPt1.X + nTickLength, nTickHt + nTickStep) 
                    PR_MakeXY(xyStart, xyPt1.X, nTickHt + nTickStep)
                    PR_MakeXY(xyEnd, xyPt1.X + nTickLength, nTickHt + nTickStep)
            ...
            If ((nn = 7) Or (FootLess = True And nn = nFirstTape + 2)) Then
                ARMDIA1.PR_SetLayer("Notes")
                ''SetData("TextVertJust", 32);		// Bottom
                ''SetData("TextHorzJust", 1);		// Left
                ''SetData("TextHeight", 0.1);
        ...
        nSpace = 0
        Dim nLastTape20Len, nLastTapeTmpltReduction, nAnkleMTape, nAnkleM20Len, nAnkleMTmpltReduction As Double
        Dim nAnkleMPrev20Len, nAnkleMPrevTmpltReduction, nAnkleMPrevHt, nPrev20Len, nPrevTmpltReduction, nPrevHt As Double
        Dim nHeel20Len, nHeelTmpltReduction, nAnkle20Len, nAnkleTmpltReduction As Double
        Dim xyFirstTape, xyAnkleM, xyHeel, xyAnkle, xyAnkleMPrev, xyHeelCntrMidDistal As XY
        While (nn <= nAnkleTape)
            If ((nn = nFirstTape + 1) And (nFootPleat1 <> 0)) Then
                nSpace = nFootPleat1
            End If
        ...
        nToeCurvedReduction = 34
        ''// Choose heel plate
        Dim nHeelR1, nHeelR2, nHeelR3, nHeelD1, nHeelD2, nBackHeelOff, nFrontHeelOff, nRedStep As Double
        If (SmallHeel) Then
            ''// Heel plate #1
            nHeelR1 = 1.1760709
            nHeelR2 = 0.8716718
        ...
        ''// At Minus Ankle ( ie -3 or -1.5 Tape)
        ''nAnkleMTapeLen = FNRound(FNDecimalise(FNGetTape(nAnkleMTape)) * nUnitsFac)
        Dim nAnkleMTapeLen As Double = FNDecimalise(FNGetTape(nAnkleMTape)) * WHRightLeg1.g_nUnitsFac
        nLength = nAnkleM20Len / 20 * nAnkleMTapeLen
        nRedStep = nAnkleM20Len / (20 * 8)
        nLength = nLength + ((nAnkleMTmpltReduction - nAnkleMChartReduction) * nRedStep)
        xyAnkleM.Y = xyOtemplate.Y + nSeam + nLength
        ...
        Dim nCalc, nAngle As Double
        Dim xyHeelCntrDistal, xyInt, xyHeelCntrMidProximal, xyHeelCntrProximal As XY
        Dim nError As Short
        If (SmallHeel) Then
            nCalc = FN_CalcLength(xyAnkleM, xyHeelCntrMidDistal)
            nAngle = FN_CalcAngle(xyHeelCntrMidDistal, xyAnkleM) - (System.Math.Acos((nHeelR1 ^ 2 - nHeelD1 ^ 2 - nCalc ^ 2) / (-2 * nHeelD1 * nCalc)) * (180 / PI))
        ...
        ''// Toes
        ''// Quick And dirty ;
        Dim xyToeSeam, xyToeOFF As XY
        xyToeSeam.Y = xyOtemplate.Y + nSeam
        Dim sFootLabel As String = " "
        Dim nAge As Double = Val(txtAge.Text)
        Dim nToeOffset As Double
        If (nAge <= 10) Then
            If (nAge <= 2) Then
                nToeOffset = 2.75
            End If
        ...
        If (sToeStyle.Equals("Self Enclosed") Or (sToeStyle.Equals("Soft Enclosed") And Val(sFootLength))) Then
            ...
            Else
                sFootLabel = "CAP"
            End If
            ''if (nFootLength = FNRound( FNDecimalise (Val(sFootLength)) * nUnitsFac) )
            nFootLength = FNDecimalise(Val(sFootLength)) * WHRightLeg1.g_nUnitsFac
            If nFootLength <> 0 Then
                If (SmallHeel) Then
                    ''nFootLength = FNRound(nFootLength * 0.9)
        ...
        ''// Toe circle constants
        ''// Quick And Dirty
        Dim nToeCntrMidToCntrLowY As Double = 3.3359296
        Dim nToeCntrMidToCntrLowX As Double = 8.0276805
        Dim nToeCntrMidToCntrHighY As Double = 3.1684888
        Dim nToeCntrMidToCntrHighX As Double = 2.1960337
        Dim nToeMidR As Double = 0.2528866
        Dim nToeLowR As Double = 8.4403592
        Dim nToeHighR As Double = 4.1080042

        ''// Establish Low Toe Arc center
        nLength = xyFirstTape.Y - xyToeSeam.Y       ''// Toe Point line To seam
        Dim xyToeCntrLow, xyToeCntrMid, xyToeCntrHigh As XY
        If (nToeCntrMidToCntrLowY < nLength) Then
            ''// Simple case
            ''// End of toe curve Is above seam line
            xyToeCntrLow.Y = xyFirstTape.Y - nToeCntrMidToCntrLowY
        ...
        Dim ptColl As Point3dCollection = New Point3dCollection
        Dim aAngle, aPrevAngle, aAngleInc As Double
        Dim xyToePnt, xyPlr As XY
        Dim ShortLeg As Boolean = False
        If (FootLess = False) Then
            ''// Draw for Straight & Straight types
            If (sToeStyle.Equals("Straight") Or sToeStyle.Equals("Soft Enclosed B/M")) Then
                ''// Draw for straight & Straight types
        ...
        Else
            ''// Draw as a panty leg
            ''StartPoly("fitted") 
            If ((nLastTape - nFirstTape) < 3) Then
                ShortLeg = True
            End If
        End If
        ''//
        ...
        Else
            nn = nAnkleTape
        End If
        Dim xyLastTape, xyPt2, xyPt3 As XY
        While (nn <= nLastTape)
            ''nTapeLen = FNRound(FNDecimalise(FNGetTape(nn)) * nUnitsFac)
            nTapeLen = FNDecimalise(FNGetTape(nn)) * WHRightLeg1.g_nUnitsFac
            If (nFabricClass = 2) Then
            ...
            End If
            xyTmp.Y = xyOtemplate.Y + nSeam + nLength
            If (ShortLeg = False) Then
                ''AddVertex(xyTmp)
                ptColl.Add(New Point3d(xyTmp.X, xyTmp.Y, 0))
            End If

            If (ShortLeg = True And nn = nFirstTape) Then
                xyPt1 = xyTmp
            End If
            If (ShortLeg = True And nn = nFirstTape + 1) Then
                xyPt2 = xyTmp
            End If
            If (ShortLeg = True And nn = nFirstTape + 2) Then
                xyPt3 = xyTmp
            End If
            ...
            If (nFabricClass = 2) Then
                ...
                If nNo = 0 Then
                    strLab = "Pleat"
                End If
                PR_DrawRuler(sSymbol, xyTmp, strLab)
                ''AddEntity ("text", Format("length", nTapeLen), xyTmp.x, xyTmp.y - 0.5)
                PR_MakeXY(xyText, xyTmp.X, xyTmp.Y - 0.5)
                ''--------------PR_DrawText(WHRightLeg1.fnInchesToText(nTapeLen), xyText, 0.1, 0, 1)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        ''//
        If (ShortLeg) Then
            ''AddEntity("poly","openfitted",xyPt1,xyPt2,xyPt3) 
            Dim ptSplineColl As Point3dCollection = New Point3dCollection
            ptSplineColl.Add(New Point3d(xyPt1.X, xyPt1.Y, 0))
            ptSplineColl.Add(New Point3d(xyPt2.X, xyPt2.Y, 0))
            ptSplineColl.Add(New Point3d(xyPt3.X, xyPt3.Y, 0))
            PR_DrawSpline(ptSplineColl)
        End If
        ...
        If (FootLess = False) Then
            ...
            If (sToeStyle.Contains("Cut-Back") Or (sToeStyle.Equals("Soft Enclosed") And nFootLength = 0 And (nAge > 10))) Then
                ...
                'xyToeCL)
                Dim xyPlr3 As XY
                PR_CalcPolar(xyToeCntrLow, nToeLowR - nLeftOffset, aPrevAngle + aAngleInc, xyPlr3)
                Dim ptColl2 As Point3dCollection = New Point3dCollection
                ptColl2.Add(New Point3d(xyTmp.X, xyTmp.Y, 0))
                ptColl2.Add(New Point3d(xyPlr3.X, xyPlr3.Y, 0))
                PR_DrawPoly(ptColl2)
                ARMDIA1.PR_SetLayer("Notes")
                'AddEntity("poly",
                '"openfitted",
                ...
                'CalcXY("relpolar", xyToeCntrLow, nToeLowR + nRightOffset, aPrevAngle + aAngleInc),
                'xyToeCL)
                PR_CalcPolar(xyToeCntrLow, nToeLowR + nRightOffset, aPrevAngle + aAngleInc, xyPlr3)
                Dim ptColl3 As Point3dCollection = New Point3dCollection
                ptColl3.Add(New Point3d(xyTmp.X, xyTmp.Y, 0))
                ptColl3.Add(New Point3d(xyPlr3.X, xyPlr3.Y, 0))
                PR_DrawPoly(ptColl3)
            End If
            ...
            PR_MakeXY(xyStart, xyToeSeam.X, xyOtemplate.Y + nSeam + 0.5)
            PR_MakeXY(xyEnd, xyOtemplate.X, xyOtemplate.Y + nSeam + 0.5)
            PR_DrawLine(xyStart, xyEnd)
            ''AddEntity("line", xyToeSeam.x,  xyOtemplate.y + nSeam,  xyOtemplate.x,  xyOtemplate.y + nSeam )
            PR_MakeXY(xyStart, xyToeSeam.X, xyOtemplate.Y + nSeam)
            PR_MakeXY(xyEnd, xyOtemplate.X, xyOtemplate.Y + nSeam)
            PR_DrawLine(xyStart, xyEnd)
        Else
            ...
            End If
            ''// Add the text as a symbol (For latter use by the StumpTool)
            'If (!Symbol("find", "TextAsSymbol")) Then Display("message","error", "Elastic at distal end of support not added.\nCheck and add text manually");
            'hEnt = AddEntity("symbol", "TextAsSymbol", xyLastTape.X + 0.25, xyOtemplate.Y + (xyLastTape.Y - xyOtemplate.Y) / 2, 1, 1, 90) ;
            'SetDBData(hEnt, "ID", sID + "PantyElasticNote") ;
            'SetDBData(hEnt, "Data", sFootLabel)
            Dim xySymbol As XY
            PR_MakeXY(xySymbol, xyLastTape.X + 0.25, xyOtemplate.Y + (xyLastTape.Y - xyOtemplate.Y) / 2)
            PR_DrawTextAsSymbol(xySymbol, txtFileNo.Text + g_sTxtLeg + "PantyElasticNote", sFootLabel, 90)
            ''xyToeSeam = xyOtemplate  ''// dummy, used To draw tramlines As below
            PR_AddDBValueToLast("ID", txtFileNo.Text + g_sTxtLeg + "PantyElasticNote")
            PR_AddDBValueToLast("Data", sFootLabel)
        End If
        If chkStump.Checked = True Then
        ...
        End If
    End Sub
    Private Function readCutOutInfoForBrief(ByRef xyThigh As XY, ByRef xyCO_ArcCen As XY) As Boolean
        Dim bIsExist As Boolean = False
        Try
            Dim _sClass As New SurroundingClass()
            Dim resbuf As New ResultBuffer
            ...
            If (resbuf IsNot Nothing) Then
                Dim arr() As TypedValue = resbuf.AsArray()
                Dim strValue As String
                If arr.Length > 9 Then
                    strValue = arr(9).Value
                    xyThigh.X = WHBODDIA1.fnGetNumber(strValue, 1)
                    xyThigh.Y = WHBODDIA1.fnGetNumber(strValue, 2)
                    strValue = arr(11).Value
                    xyCO_ArcCen.X = WHBODDIA1.fnGetNumber(strValue, 1)
                    xyCO_ArcCen.Y = WHBODDIA1.fnGetNumber(strValue, 2)
                    bIsExist = True
                End If
            End If
        Catch ex As Exception
        ...
        Return bIsExist
    End Function
    Private Sub PR_DrawBriefStyleRightLeg()
        ''--------------File Name : WHLG2LEG.D --------------------
        Dim ptFstLegColl As Point3dCollection = New Point3dCollection
        If readLegCurveInfo(ptFstLegColl) = False Then
            MsgBox("Can't find First Leg Curve, to copy body curve" & Chr(10) & "Draw or Re-Draw First Leg.", 16, "Waist - Leg Dialog")
        ...
        Dim xyOBody, xyTOS, xyWaist, xyMidPoint, xyLargest, xyFold, xyFoldPanty, xyButtockArcCen, xyThigh, xyCO_ArcCen As XY
        Dim BodyFound As Boolean = False
        Dim nFoldHt As Double
        If readCutOutInfo(xyOBody, xyTOS, xyWaist, xyMidPoint, xyLargest, xyFold, xyFoldPanty, xyButtockArcCen, nFoldHt) = False Then
            MsgBox("No BODY found!, draw Cut-Out and try again.", 16, "Waist - Leg Dialog")
            Exit Sub
        End If
        If readCutOutInfoForBrief(xyThigh, xyCO_ArcCen) = False Then
            MsgBox("No BODY found!, draw Cut-Out and try again.", 16, "Waist - Leg Dialog")
            Exit Sub
        End If
        BodyFound = True
        ...
        Dim nLegVertexCount As Double = ptFstLegColl.Count
        Dim sLine As String = ""
        Dim xyTmp, xyEnd As XY
        While (ii <= nLegVertexCount)
            sLine = LineInput(hFileCurve)
            '' ScanLine(sLine, "blank", & nX, & nY)
            nX = FN_GetNumber(sLine, 1)
            nY = FN_GetNumber(sLine, 2)
            If (nX >= xyThigh.X) Then
                xyTmp.X = nX
        ...
        ''// Closing lines
        ''Execute("menu", "SetLayer", hTemplateLayer);
        Dim xyLineSt, xyLineEd, xyPt1, xyPt2, xyInt As XY
        If (Val(sTOSCir) = 0) Then
            ''AddEntity("line", xyWaist.X, xyWaist.Y + 2 * (xyOBody.Y - xyWaist.Y),xyTOS.X, xyTOS.Y + 2 * (xyOBody.Y - xyTOS.Y))
            PR_MakeXY(xyLineSt, xyWaist.X, xyWaist.Y + 2 * (xyOBody.Y - xyWaist.Y))
            PR_MakeXY(xyLineEd, xyTOS.X, xyTOS.Y + 2 * (xyOBody.Y - xyTOS.Y))
            PR_DrawLine(xyLineSt, xyLineEd)
        End If

        ''AddEntity("line", xyTOS.X, xyOBody.Y,xyTOS.X, xyTOS.Y + 2 * (xyOBody.Y - xyTOS.Y))
        PR_MakeXY(xyLineSt, xyTOS.X, xyOBody.Y)
        PR_MakeXY(xyLineEd, xyTOS.X, xyTOS.Y + 2 * (xyOBody.Y - xyTOS.Y))
        PR_DrawLine(xyLineSt, xyLineEd)
        ...
        PR_DrawLine(xyThigh, xyPt1)

        'Fold offset
        Dim nFoldOff As Double
        If g_sCrotchStyle = "Horizontal Fly" Then
            nFoldOff = 0
        Else
            nFoldOff = 0.75
        End If
        ...
        PR_MakeXY(xyLineEd, xyEnd.X, xyEnd.Y + 1.5)
        PR_DrawLine(xyEnd, xyLineEd)
        xyEnd.Y = xyEnd.Y + 1.5
        ...
        Dim aAngle As Double = 180
        Dim aAngleInc As Double = 15
        Dim nRadius As Double = xyCO_ArcCen.X - xyThigh.X
        ii = 1
        While (ii <= 2)
            ''AddVertex(CalcXY("relpolar", xyCO_ArcCen, nRadius, aAngle + aAngleInc * ii))
            PR_CalcPolar(xyCO_ArcCen, nRadius, aAngle + aAngleInc * ii, xyInt)
            ptPolyColl.Add(New Point3d(xyInt.X, xyInt.Y, 0))
            ii = ii + 1
        End While
        ''AddVertex ( xyPt2)
        ...
        PR_CalcPolar(xyEnd, nRadius, 0, xyPt1)
        While (ii < 2)
            ''AddVertex(CalcXY("relpolar", xyPt1, nRadius, aAngle + aAngleInc * ii))
            PR_CalcPolar(xyPt1, nRadius, aAngle + aAngleInc * ii, xyInt)
            ptPolyColl.Add(New Point3d(xyInt.X, xyInt.Y, 0))
            ii = ii + 1
        End While
        ''AddVertex ( xyEnd)
        ptPolyColl.Add(New Point3d(xyEnd.X, xyEnd.Y, 0))
        PR_DrawSpline(ptPolyColl)
        If chkStump.Checked = True Then
            PR_DrawWaistLegStump()
        End If
        If chkHeelContracture.Checked = True Then
