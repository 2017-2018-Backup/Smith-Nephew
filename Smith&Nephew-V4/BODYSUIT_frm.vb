...
Friend Class bodysuit
    ...
    Private Sub btnCancel_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles btnCancel.Click
        ...
        If sCurrentValues <> BODYSUIT1.g_sChangeChecker Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Bodysuit - Body")
    ...
    End Sub
    Public Function PR_CloseBodyDialog() As Boolean
        Dim Response As Short
    ...
    Private Function fnInchestoText(ByRef nInches As Double) As String
        ...
        If nDec <> 0 Then 'Avoid overflow
            ...
            iEighths = round(nDec / nPrecision)
        Else
            iEighths = 0
        End If
        ...
        For ii = min(m_iLeftFirstTape, m_iRightFirstTape) To ARMDIA1.max(m_iLeftLastTape, m_iRightLastTape)
            ...
            If m_nRightOriginalTapeValues(ii) > 0 And m_nLeftOriginalTapeValues(ii) > 0 Then
                If System.Math.Abs(m_nRightOriginalTapeValues(ii) - m_nLeftOriginalTapeValues(ii)) <= INCH3_8 Then
                    ...
                    ''Changed for #191 in the issue list
...
Continue_Loop:
    ...
    Sub PR_DrawTextForHeight(ByRef sText As Object, ByRef xyInsert As BODYSUIT1.XY, ByRef nHeight As Object, ByRef nAngle As Double, ByVal nTextMode As Double, ByVal nIndex As Integer)
        'To the DRAFIX macro file (given by the global fNum).
        'Write the syntax to draw TEXT at the given height.
        '
        'For this to work it assumes that the following DRAFIX variables
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acText As DBText = New DBText()
                ...
                acText.TextString = sText
                acText.Justify = nTextMode
                acText.Color = Autodesk.AutoCAD.Colors.Color.FromColorIndex(Autodesk.AutoCAD.Colors.ColorMethod.ByAci, nIndex)
                acText.AlignmentPoint = New Point3d(xyInsert.X + xyInsertion.X, xyInsert.Y + xyInsertion.Y, 0)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acText.TransformBy(Matrix3d.Scaling(2.54, New Point3d(xyInsertion.X, xyInsertion.Y, 0)))
                End If
    ...
    Sub PR_DrawLegTemplate(ByRef sLeg As String, ByRef xyStart As BODYSUIT1.XY)
        ...
        Dim LegProfile As curve
        Dim sMinHtColour, sMaxHtColour As String
        ...
        If sLeg = "Both" Then
            ...
            For ii = iFirstTape To iLastTape
                ...
                PR_DrawTapeScaleForBoth(nX, m_n20Len(ii), nMaxHt, nMinHt, m_nReduction(ii), Trim(Str(ii)) & "tape", ii, sMinHtColour, sMaxHtColour)
                nX = nX + m_nSpace(ii)
            Next ii
        Else
    ...
    Private Sub PR_DrawTapeScale(nX As Double, n20Len As Double, nHt As Double, nReduction As Double, Symbol As String, nTape As Double)
        ...
        ''Changed for #204 in the issue list
        ''PR_DrawText(fnInchestoText(nHt), xyText, 0.125, 0, 1)
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            PR_DrawText(fnDisplayToCM(nHt), xyText, 0.125, 0, 1)
        Else
            PR_DrawText(fnInchestoText(nHt) & Chr(34), xyText, 0.125, 0, 1)
        End If
        ''// Add tape ID
        ...
        ''Changed for #203 in the issue list
        ''Dim strTape As String = LTrim(Mid(g_sTextList, ((nTape - 1) * 3) + 1, 3))
        Dim strTape As String = LTrim(Mid(g_sTextList, (nTape * 3) + 1, 3))
        ...
        End While
    End Sub
    Private Sub PR_DrawTapeScaleForBoth(nX As Double, n20Len As Double, nMaxHt As Double, nMinHt As Double, nReduction As Double,
                                        Symbol As String, nTape As Double, sMinHtColor As String, sMaxHtColor As String)
        Dim xyPt1, xyTmp, xyStart, xyText, xyLineSt, xyLineEnd As BODYSUIT1.XY
        xyPt1 = xyStart
        xyPt1.X = xyPt1.X + nX
        xyTmp.X = xyPt1.X

        Dim nMaxLength, nSeam, nMinLength, ii, jj, nTickStep, nTickHt, nTickLength As Double
        ...
        PR_MakeXY(xyText, xyPt1.X + 0.2, nTickHt - nSeam * 3)
        PR_DrawText(Str(nReduction), xyText, 0.125, 0, 1)

        ''// Maximum text	
        ''hEnt = AddEntity("text", Format("length", nMaxHt), xyPt1.X + 0.2, nTickHt - nSeam) ;
        ''SetEntityData(hEnt, "color", Table("find", "color", sMaxHtColour));
        Dim nColIndex As Integer = 250
        If (sMaxHtColor.Equals("blue")) Then
            nColIndex = 5
        ElseIf (sMaxHtColor.Equals("red")) Then
            nColIndex = 1
        End If
        PR_MakeXY(xyText, xyPt1.X + 0.2, nTickHt - nSeam)
        ''PR_DrawText(fnInchestoText(nMaxHt), xyText, 0.125, 0, 1)
        ''Changed for #204 in the issue list
        ''PR_DrawTextForHeight(fnInchestoText(nMaxHt), xyText, 0.125, 0, 1, nColIndex)
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            PR_DrawTextForHeight(fnDisplayToCM(nMaxHt), xyText, 0.125, 0, 1, nColIndex)
        Else
            PR_DrawTextForHeight(fnInchestoText(nMaxHt) & Chr(34), xyText, 0.125, 0, 1, nColIndex)
        End If

        ''// Minimum text	
        If (nMinHt <> 0 And nMinHt <> nMaxHt) Then
            ''hEnt = AddEntity("text", Format("length", nMinHt), xyPt1.X + 0.2, nTickHt - nSeam * 2) ;
            ''SetEntityData(hEnt, "color", Table("find", "color", sMinHtColour));
            nColIndex = 250
            If (sMinHtColor.Equals("blue")) Then
                nColIndex = 5
            ElseIf (sMinHtColor.Equals("red")) Then
                nColIndex = 1
            End If
            PR_MakeXY(xyText, xyPt1.X + 0.2, nTickHt - nSeam * 2)
            ''PR_DrawText(fnInchestoText(nMinHt), xyText, 0.125, 0, 1)
            ''Changed for #204 in the issue list
            ''PR_DrawTextForHeight(fnInchestoText(nMinHt), xyText, 0.125, 0, 1, nColIndex)
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                PR_DrawTextForHeight(fnDisplayToCM(nMinHt), xyText, 0.125, 0, 1, nColIndex)
            Else
                PR_DrawTextForHeight(fnInchestoText(nMinHt) & Chr(34), xyText, 0.125, 0, 1, nColIndex)
            End If
        End If
...
Error_Close:
    ...
    Private Sub PR_DrawBodySuit()
        ...
        ElseIf InStr(g_sBodyFrontNeckStyle, "Turtle") > 0 Then
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                sText = "NECK CIRC. " & fnDisplayToCM(g_nNeckCir) & "\nTURTLE NECK WIDTH " & fnDisplayToCM(g_nFrontNeckSize)
            End If
        ...
        Else 'Regular
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                sText = "NECK CIRC. " & fnDisplayToCM(g_nNeckCir) & BODYSUIT1.QQ
            End If
            PR_MakeXY(xyNeckText, xyBodyCutOutFrontNeck.X - 3, xyBodyCutOutFrontNeck.Y + 0.25)
        End If
        ...
        If g_sLegStyle = "Panty" Then
            ...
            If Not g_bDrawSingleLeg Then
                ...
                PR_DrawText("edge", xyText, 0.125, 0, 1)
            End If
        End If
