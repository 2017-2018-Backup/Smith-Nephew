...
Friend Class torsodia
    ...
    Private Sub Cancel_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Cancel.Click
        ...
        If sCurrentValues <> TORSODIA1.g_sChangeChecker Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Vest - Torso")
    ...
    End Sub
    Public Function PR_CloseTorsoDialog() As Boolean
        Dim Response As Short
    ...
    Private Function FN_ValidateData() As Short
        ...
        If sError <> "" Then
            MsgBox(sError, 16, "Vest - Torso Dialog")
            FN_ValidateData = False
        Else
    ...
    Private Sub torsodia_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If (blkId.IsNull()) Then
                MsgBox("No Patient Details have been found in drawing!", 16, "Error, Vest - Torso Dialog")
                Me.Close()
                Exit Sub
            End If
    ...
    Private Sub PR_GetComboListFromFile(ByRef Combo_Name As System.Windows.Forms.ComboBox, ByRef sFileName As String)
        ...
        If FileLen(sFileName) = 0 Then
            MsgBox(sFileName & "Not found", 48, "Vest - Torso Dialog")
            Exit Sub
        End If
        ...
        ElseIf flag.Text = "inches" Then
            ...
            If Value >= 1 Then
                ...
                If remainder > 0.75 Then
                    MsgBox("Invalid Measurements, Inches and Eights only", 48, "Vest - Torso Dialog")
                    'UPGRADE_WARNING: Couldn't resolve default property of object FN_CmToInches. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
                    FN_CmToInches = "-1"
                    Exit Function
                Else
    ...
    Function FN_InchesValue(ByRef TextBox As System.Windows.Forms.Control) As Double
        ...
        For nn = 1 To iLen
            ...
            If Asc(sChar) > 57 Or Asc(sChar) < 46 Or Asc(sChar) = 47 Then
                MsgBox("Invalid - Dimension has been entered", 48, "Vest - Torso Dialog")
                TextBox.Focus()
                FN_InchesValue = -1
                Exit For
            End If
        ...
        If nLen = -1 Then
            MsgBox("Invalid - Length has been entered", 48, "Vest - Torso Dialog")
            TextBox.Focus()
            FN_InchesValue = -1
        Else
            FN_InchesValue = nLen
    ...
    Private Sub VestToSlv_Click(sender As Object, e As EventArgs) Handles VestToSlv.Click
        ...
        If ptEntRes.Status <> PromptStatus.OK Then
            MsgBox("Vest Raglan Profile not selected", 16, "Vest - Torso Dialog")
            Me.Show()
        ...
        If (sSleeve.ToUpper().Equals("LEFT", StringComparison.InvariantCultureIgnoreCase) = False And sSleeve.ToUpper().Equals("RIGHT", StringComparison.InvariantCultureIgnoreCase) = False) Then
            MsgBox("Select a Right or Left Raglan profile only", 16, "Vest - Torso Dialog")
            Me.Show()
            VestMain.VestMainDlg.Show()
            Exit Sub
        End If
        ...
        Me.Close()
        VestMain.VestMainDlg.Close()
        MsgBox("Data Transfer Finished", 0, "Vest - Torso Dialog")
    End Sub
    Public Sub PR_SetTorsoFabricValue(ByRef sVestFabric As String)
        Try
            If sVestFabric <> "" Then
                cboFabric.Text = sVestFabric
                'If TORSODIA1.g_sChangeChecker.Contains("Burn") = False And TORSODIA1.g_sChangeChecker.Contains("Pow") = False Then
                '    TORSODIA1.g_sChangeChecker = TORSODIA1.g_sChangeChecker & sVestFabric
                '    Exit Sub
                'End If
                'Dim strLast As String = Mid(TORSODIA1.g_sChangeChecker, TORSODIA1.g_sChangeChecker.Length - 12)
                'Dim strPrev As String = Mid(TORSODIA1.g_sChangeChecker, 1, TORSODIA1.g_sChangeChecker.Length - 13)
                'TORSODIA1.g_sChangeChecker = strPrev & sVestFabric
                Exit Sub
            End If
            'Dim _sClass As New SurroundingClass()
            'Dim resbuf As New ResultBuffer
            'resbuf = _sClass.GetXrecord("VestTorsoFabric", "VESTTORSOFABDIC")
            'If (resbuf IsNot Nothing) Then
            '    Dim arr() As TypedValue = resbuf.AsArray()
