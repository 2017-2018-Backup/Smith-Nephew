...
Public Class VESTARM_frm
    ...
    Private Function FN_LeftArmValuesString() As String
        ...
        sString = sString & cboPressure.Text & Str(chkNone.Checked) & Str(chkStump.Checked) & Str(chkGauntlets.Checked)
    ...
    Private Function FN_RightArmValuesString() As String
        ...
        sString = sString & RightcboPressure.Text & Str(RightchkNone.Checked) & Str(RightchkStump.Checked) & Str(RightchkGauntlets.Checked)
        ...
        sString = sString & RightcboFabric.Text
    ...
    End Sub
    Public Function PR_CloseVestArmDialog() As Boolean
        Dim Response As Short
        Dim sLeftCurrentValue As String = FN_LeftArmValuesString()
        Dim sRightCurrentValue As String = FN_RightArmValuesString()
        Dim bIsChanged As Boolean = False
    ...
    Private Sub Draw_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Draw.Click
        ...
        VESTARM.PR_DrawFitted(Profile, True, g_sbIsBody)
        ...
        For tps = Val(strLeftFirstTape) To Val(strLeftLastTape)
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                ''VESTARM.PR_PutTapeLabel(tps, xyPt1, fnDisplayToCM(Measurement), mms(tps - 1), gms(tps - 1), reds(tps - 1))
                VESTARM.PR_PutTapeLabel(tps, xyPt1, Val(Length(tps - 1).Text), mms(tps - 1), gms(tps - 1), reds(tps - 1))
            Else
                VESTARM.PR_PutTapeLabel(tps, xyPt1, Measurement, mms(tps - 1), gms(tps - 1), reds(tps - 1))
            End If
        ...
        PR_DrawVestXMarker(xyVestInsertion, True, g_sbIsBody)
        VestMain.VestMainDlg.Close()
        BodyMain.BodyMainDlg.Close()
        Return
    ...
    Private Function readDWGInfo()
        Try
            ...
            Dim sInfo As String = "VestArmInfo"
            Dim sDict As String = "VESTARMDIC"
    ...
    Private Function saveInfoToDWG()
        Try
            ...
            If g_sbIsBody = True Then
                ...
                sDict = "BODYARMDIC"
            End If
            If (_sClass.GetXrecord(sInfo, sDict) IsNot Nothing) Then
                _sClass.RemoveXrecord(sInfo, sDict)
            End If
            ...
            _sClass.SetXrecord(resbuf, sInfo, sDict)
    ...
    Private Sub Elastic(ByRef Profile As VESTARM.curve)
        ...
        ElseIf Val(strLeftAge) <= 10 And Val(strLeftLastTape) = 11 Then
            ...
            ''Changed for #172 in the issue list on 22nd June 2019
            ''VESTARM.PR_DrawTextCenter("1/2'' Elastic", xyEpt1, 0.125, (90 * (VESTARM.PI / 180)))
            VESTARM.PR_DrawTextCenter("Elastic", xyEpt1, 0.125, (90 * (VESTARM.PI / 180)))
    ...
    Private Sub armdia_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            If g_sbIsBody = True Then
                ...
                ''Changed for #211 in the issue list
                ''Label1.Text = "bodysuit - arm"
                Label1.Text = "left arm - bodysuit"
            End If
    ...
    Public Sub PR_DrawShoulderFlaps(ByRef Profile As VESTARM.curve, ByRef numtapes As Object)
        ...
        If strLeftFlap <> "" And strLeftFlap <> "Vest" And strLeftFlap <> "Body" Then
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                Strap = "STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strLeftInchFlag, txtStrap)))
            End If
            If Val(txtFrontStrapLength.Text) > 0 Then
                'Display FRONT strap length if given
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    Strap = "BACK  STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strLeftInchFlag, txtStrap))) & "CM"
                    Strap = Strap & Chr(10) & "FRONT STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strLeftInchFlag, txtFrontStrapLength))) & "CM"
                Else
            ...
            End If
                ...
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    Strap = Strap & Chr(10) & "WAIST = " & Trim(fnDisplayStrapToCM(cmtoinches(strLeftInchFlag, txtWaistCir)))
                Else
    ...
    Private Sub TabControl1_SelectedIndexChanged(sender As Object, e As EventArgs) Handles TabControl1.SelectedIndexChanged
        ...
        Label1.Text = strLeftSleeve.ToLower() + " arm - vest"
        If g_sbIsBody = True Then
            Label1.Text = strLeftSleeve.ToLower() + " arm - bodysuit"
        End If
        If g_sAlreadyLoad = False Then
            g_sAlreadyLoad = True
            Dim strFabric As String = RightcboFabric.Text
            RightDialogLoad()
            Try
                ...
                If (resbuf IsNot Nothing) Then
                    Dim arr() As TypedValue = resbuf.AsArray()
                    RightcboFabric.Text = arr(37).Value
                End If
            Catch ex As Exception

            End Try
            If g_sbIsBody = True Then
                RightcboFabric.Text = BodyMain.g_sBodyFabric
            End If
            If g_sRightArmValues.Contains("Burn") = True Or g_sRightArmValues.Contains("Pow") = True Then
                Dim strPrev As String = Mid(g_sRightArmValues, 1, g_sRightArmValues.Length - 13)
                g_sRightArmValues = strPrev & RightcboFabric.Text
            Else
                g_sRightArmValues = g_sRightArmValues & RightcboFabric.Text
            End If
        End If
    End Sub
    ...
    Private Sub RightDraw_Click()
        ...
        VESTARM.g_sID = sStyle & strLeftFileNo & strLeftSleeve
        ...
        For tps = Val(strFirstTape) To Val(strLastTape)
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                ''VESTARM.PR_PutTapeLabel(tps, xyPt1, fnDisplayToCM(Measurement), Rightmms(tps - 1), Rightgms(tps - 1), Rightreds(tps - 1))
                VESTARM.PR_PutTapeLabel(tps, xyPt1, Val(RightLength(tps - 1).Text), Rightmms(tps - 1), Rightgms(tps - 1), Rightreds(tps - 1))
            Else
                VESTARM.PR_PutTapeLabel(tps, xyPt1, Measurement, Rightmms(tps - 1), Rightgms(tps - 1), Rightreds(tps - 1))
            End If
    ...
    Private Sub VESTARM_frm_FormClosing(sender As Object, e As FormClosingEventArgs) Handles MyBase.FormClosing
    ...
    Private Function saveRightInfoToDWG()
        Try
            Dim _sClass As New VestSurroundingClass()
            Dim sInfo As String = "VestArmRightInfo"
            Dim sDict As String = "VESTARMRIGHTDIC"
            If g_sbIsBody = True Then
    ...
    Public Sub PR_DrawRightShoulderFlaps(ByRef Profile As VESTARM.curve, ByRef numtapes As Object)
        ...
        If RightcboFlaps.Text <> "" And RightcboFlaps.Text <> "Vest" And RightcboFlaps.Text <> "Body" Then
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                Strap = "STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strRightInchFlag, RighttxtStrapLength)))
            End If
            If Val(RighttxtFrontStrapLength.Text) > 0 Then
                'Display FRONT strap length if given
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    Strap = "BACK  STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strRightInchFlag, RighttxtStrapLength))) & "CM"
                    Strap = Strap & Chr(10) & "FRONT STRAP = " & Trim(fnDisplayStrapToCM(cmtoinches(strRightInchFlag, RighttxtFrontStrapLength))) & "CM"
                Else
            ...
            End If
                ...
                'Display Waist Circumference for D-Style flaps only
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    Strap = Strap & Chr(10) & "WAIST = " & Trim(fnDisplayStrapToCM(cmtoinches(strRightInchFlag, RighttxtWaistCir)))
                Else
    ...
    Private Function PR_GetBodyBlkAttributeValues() As Boolean
        ...
        Return True
    End Function
    Public Sub PR_SetArmFabricValue(ByRef sVestFabric As String)
        Try
            If sVestFabric <> "" Then
                cboFabric.Text = sVestFabric
                RightcboFabric.Text = "1"
                RightcboFabric.Text = sVestFabric
                'If g_sLeftArmValues.Contains("Burn") = True Or g_sLeftArmValues.Contains("Pow") = True Then
                '    Dim strPrev As String = Mid(g_sLeftArmValues, 1, g_sLeftArmValues.Length - 13)
                '    g_sLeftArmValues = strPrev & sVestFabric
                'Else
                '    g_sLeftArmValues = g_sLeftArmValues & sVestFabric
                'End If
                'If g_sRightArmValues.Contains("Burn") = True Or g_sRightArmValues.Contains("Pow") = True Then
                '    Dim strPrev As String = Mid(g_sRightArmValues, 1, g_sRightArmValues.Length - 13)
                '    g_sRightArmValues = strPrev & RightcboFabric.Text
            ...
            'Dim _sClass As New SurroundingClass()
            'Dim resbuf As New ResultBuffer
            'resbuf = _sClass.GetXrecord("VestArmFabric", "VESTARMFABDIC")
            'If (resbuf IsNot Nothing) Then
            '    Dim arr() As TypedValue = resbuf.AsArray()
            '    cboFabric.Text = arr(0).Value
            '    RightcboFabric.Text = arr(0).Value
    ...
    Private Sub PR_GetRightStyle(ByRef sStyle As String)
        ...
        sDistalStyle = "XX"
        If VESTARM.g_iRightStyleFirstTape = VESTARM.g_iRightFirstTape Then
            'Plain
            sDistalStyle = "PL"
        Else
            'Start at tape
            If VESTARM.g_iStyleFirstTape < 10 Then
                sDistalStyle = "0" & LTrim(Str(VESTARM.g_iStyleFirstTape))
            Else
                sDistalStyle = LTrim(Str(VESTARM.g_iStyleFirstTape))
            End If
        End If
        If RightchkGauntlets.Checked = True Then
            'Gauntlet
        ...
        'Proximal
        sProximalStyle = "XX"
        If VESTARM.g_iRightStyleLastTape = VESTARM.g_iRightLastTape Then
            'Plain
            sProximalStyle = "PL"
        Else
            'Start at tape
            If VESTARM.g_iRightStyleLastTape < 10 Then
                sProximalStyle = "0" & LTrim(Str(VESTARM.g_iRightStyleLastTape))
            Else
                sProximalStyle = LTrim(Str(VESTARM.g_iRightStyleLastTape))
            End If
        End If
        If RightchkDetachable.Checked = True And VESTARM.g_iRightStyleLastTape <= 6 Then
    ...
    Private Sub RightLength_Enter(eventSender As Object, e As EventArgs) Handles RightLength.Enter
        Dim Index As Short = RightLength.GetIndex(eventSender)
