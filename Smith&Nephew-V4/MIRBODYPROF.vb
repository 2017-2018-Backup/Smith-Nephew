...
Public Class MIRBODYPROF
    ...
    Sub PR_DrawText(ByRef sText As String, ByRef xyInsert As ARMDIA1.XY, ByRef nHeight As Object, ByRef nAngle As Double)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acText As DBText = New DBText()
                ...
                acText.Height = nHeight
                acText.TextString = sText
                acText.Rotation = nAngle * (ARMDIA1.PI / 180)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acText.TransformBy(Matrix3d.Scaling(2.54, New Point3d(xyInsert.X, xyInsert.y, 0)))
                End If
                acBlkTblRec.AppendEntity(acText)
    ...
    Public Function PR_DrawWaistLateralBodyZipper() As Boolean
        ...
        Using tr As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If (Panty) Then
                xyHorizEnd.y = xyOtemplate.y + (((nThighFiguredCir * 2) / 6) * nInchToCM)
            Else
                xyHorizEnd.y = xyOtemplate.y + ((nWaistActualCir / 6) * nInchToCM)
            End If
            ...
            If (EOStoLegTape) Then
                xyHorizStart.X = xyCurveStart.X + (nTapeSpacing * nInchToCM)
            End If
            If (EOStoEOS) Then
        ...
        ''Changed for #165 on 22-5-2019
        ''PR_DrawTextAsSymbol(xyBlock, sZipperID, sText + " LATERAL ZIPPER", 0)
        PR_DrawTextAsSymbol(xyBlock, sZipperID, "ZIPPER", 0)
    ...
    Public Sub PR_DrawAnkleZipper()
        ...
        Using tr As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            nZipOff = 1.125 * nInchToCM
            xyConstruct.X = xyAnkle.X + (nX * nInchToCM) ''// nX And nY stored relative position Of the 
            xyConstruct.y = xyAnkle.y + (nY * nInchToCM)  ''// heel construction circle center point 
            Dim xyStart, xyEnd As ARMDIA1.XY
            ...
            If bIsEOS Then
                ...
                For Each idObject As ObjectId In selectionSetCL.GetObjectIds()
                    ...
                    If (acEntity.GetType().Name.ToUpper.Equals("ARC")) Then
                        Dim acXdataArc As Arc = TryCast(acEntity, Arc)
                        resbuf = acXdataArc.GetXDataForApplication("ID")
                    ElseIf (acEntity.GetType().Name.ToUpper.Equals("LINE")) Then
                        Dim acXdataLine As Line = TryCast(acEntity, Line)
                        resbuf = acXdataLine.GetXDataForApplication("ID")
                    End If
                    If Not IsNothing(resbuf) Then
                        ' Get the values in the xdata
                        For Each typeVal As TypedValue In resbuf
                            If typeVal.TypeCode = DxfCode.ExtendedDataRegAppName Then
                                Continue For
                            End If
                            strXData = strXData & typeVal.Value
                        Next
                    End If
                    If (strXData.Contains("") = False) Then
                        Continue For
                    End If
    ...
    Public Sub PR_DrawGlovePalmZipper()
        ...
        Dim xyPt1, xyPt2, xyGivenPoint, xyDORSAL As ARMDIA1.XY
        Dim nDrawnLength As Double
        ...
        Using tr As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            End If
            Dim selectionSet As SelectionSet = selResult.Value
            Dim bIsDorsalExist As Boolean = False
            For Each idObject As ObjectId In selectionSet.GetObjectIds()
                Dim acEntity As Entity = tr.GetObject(idObject, OpenMode.ForRead)
                ...
                If strZipper.Equals("EOS") Then
                    ...
                    End If
                End If
                ''Added for change request #156 in the issue list
        ...
        Dim nB As Double = System.Math.Cos(aAngle * (ARMDIA1.PI / 180)) * nLength
        ...
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            nInchToCM = 2.54
        End If
        ARMDIA1.PR_CalcPolar(xyStartEOS, aEOS, (nB + (0.625 * nInchToCM)) - nInsertSize, xyZipStart)
        ...
        Dim nZipConstructLen As Double = System.Math.Abs(System.Math.Sin(aAngle * (ARMDIA1.PI / 180)) * nLength)
        ...
        If (EOStoCalculatedPoint = True) Then
            ...
            ARMDIA1.PR_CalcPolar(xyZipStart, aZipper, nZipConstructLen - (nWebOffSet * nInchToCM), xyZipEnd)
        End If
        If (EOStoSelectedPoint = True) Then
            ...
            Dim nA As Double = System.Math.Abs(System.Math.Sin(aAngle * (ARMDIA1.PI / 180)) * nLength)
        ...
        ''Added for change request #156 in the issue list
        Dim nLen As Double = 0.125
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            nLen = 0.125 * 2.54
        End If
        ARMDIA1.PR_CalcPolar(xyDORSAL, aEOS - 180, nLen, xyDORSAL)
        xyZipStart.X = xyDORSAL.X
        xyZipEnd.X = xyDORSAL.X
        ...
        ''--------Changed for the change request #158 in the issue list
        ''PR_DrawTextAsSymbol(xyTmp, sZipperID, sZipLength + " Palmer", aZipper - 180)
        PR_DrawTextAsSymbol(xyTmp, sZipperID, "Palmer", aZipper - 180)
    ...
    Public Sub PR_DrawGloveDorsalZipper()
        ...
        Dim nB As Double = System.Math.Abs(System.Math.Cos(aAngle * (ARMDIA1.PI / 180)) * nLength)
        ...
        ARMDIA1.PR_CalcPolar(xyStartEOS, aEOS, nB, xyZipStart)
        ...
        PR_DrawTextAsSymbol(xyTmp, sZipperID, "Dorsal", aZipper - 180)
        '// Label entity with ID string and make Zipper %true
        SetDBData("ID", sZipperID)
        SetDBData("Zipper", "1")
