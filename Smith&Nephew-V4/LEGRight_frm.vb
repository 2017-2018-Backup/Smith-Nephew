...
Friend Class LEGRight_frm
    ...
    Public xyToeSeam, xyProfileLast, xyPrevProfileLast, xyProfileStart As LEGRIGHT.XY
    ...
    Private Sub Cancel_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Cancel.Click
        ...
        If g_sChangeChecker <> FN_ConcatData() Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Right Leg")
    ...
    Public Function PR_CloseRightLegDialog() As Boolean
        Dim Response As Short
        'Check if data has been modified
        If g_sChangeChecker <> FN_ConcatData() Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Right Leg")
    ...
    Private Sub chkLeftZipper_CheckStateChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles chkLeftZipper.CheckStateChanged
        If LEGRIGHT.g_JOBSTEX = True Or g_JOBSTEX_FL = True Then
    ...
    Private Sub ExtendLegTapes_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ExtendLegTapes.Click
        ...
        nValue = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(ii).Text))
        ...
        txtLeft(ii + 1).Text = CStr(LEGRIGHT.fnInchesToDisplay(nValue))
        '----grdLeftInches.Row = ii + 1
        '-----grdLeftInches.Text = LEGRIGHT.fnInchesToText(nValue)
    ...
    Private Function FN_BuildStyleString() As String
        ...
        If LEGRIGHT.g_POWERNET = True Then sFabricClass = "0"
        If LEGRIGHT.g_JOBSTEX = True Then sFabricClass = "1"
        ...
        sString = Str(g_iRightLegStyle) & " "
        If g_iRightLegStyle < 3 Then
    ...
    Private Function FN_POWERNET_Pressure(ByRef iReduction As Short, ByRef nAnkleCir As Double, ByRef iModulus As Short) As Short
        ...
        sConversion = ""
        For ii = 0 To 17
            If iModulus = Val(LEGRIGHT.POWERNET.Modulus(ii)) Then
                sConversion = LEGRIGHT.POWERNET.Conversion_Renamed(ii)
                Exit For
            End If
        Next ii
    ...
    Private Sub Form_LinkClose()
        ...
        If txtUnits.Text = "cm" Then
            LEGRIGHT.g_nUnitsFac = 10 / 25.4
        Else
            LEGRIGHT.g_nUnitsFac = 1
        End If
        ...
        For ii = 0 To 29
            ...
            If nValue > 0 Then
                ...
                nValue = LEGRIGHT.fnDisplaytoInches(nValue)
        ...
        g_iRightLegStyle = fnGetNumber(txtChosenStyle.Text, 1)

        If g_iRightLegStyle >= 0 Then
            optType_CheckedChanged(optType.Item(g_iRightLegStyle), New System.EventArgs())
            optType(g_iRightLegStyle).Checked = True
        ...
        If g_iRightLegStyle >= 3 Then PR_SetFirstTape(g_iStyleFirstTape)
        ...
        '
        iFabricClass = -1
        If fnGetNumber(g_sRightStyleString, 4) <> -1 Then
            iFabricClass = fnGetNumber(g_sRightStyleString, 11)
        End If
        ...
        If iFabricClass < 0 Then
            ...
            End If
        Else
            If iFabricClass = 0 Then LEGRIGHT.g_POWERNET = True
            If iFabricClass = 1 Then LEGRIGHT.g_JOBSTEX = True
            If iFabricClass = 2 Then LEGRIGHT.g_JOBSTEX = True
            '        If iFabricClass = 2 Then g_JOBSTEX_FL = True
        End If

        If LEGRIGHT.g_JOBSTEX = True Then optFabric(1).Checked = True
        ...
        If LEGRIGHT.g_POWERNET = True Then
            optFabric(0).Checked = True
            chkStretch.Enabled = True
            ''-------LEGRIGHT.PR_GetComboListFromFile(cboFabric, g_sPathJOBST & "\WHFABRIC.DAT")
            ''-------PR_LoadFabricFromFile(g_sPathJOBST & "\TEMPLTS\POWERNET.DAT")
            ''LEGRIGHT.PR_GetComboListFromFile(cboFabric, sSettingsPath & "\WHFABRIC.DAT")
            cboFabric.Items.Clear()
            LEGRIGHT.PR_GetComboListFromFile(cboFabric, sSettingsPath & "\FABRIC.DAT")
        ...
        If LEGRIGHT.g_POWERNET = True Then PR_EnablePOWERNET()
        If LEGRIGHT.g_JOBSTEX = True Or g_JOBSTEX_FL = True Then PR_EnableJOBSTEX()
        '   If g_JOBSTEX_FL = True Then PR_EnableFL_JOBSTEX


        'From the g_sRightStyleString set above extract the saved ankle figuring
        ...
        If g_sRightStyleString <> "" And g_iLtAnkle <> 0 Then
            iZipper = fnGetNumber(g_sRightStyleString, 10)
            If iZipper >= 0 Then chkLeftZipper.CheckState = iZipper 'Order is v.important here
            iMMHg = fnGetNumber(g_sRightStyleString, 5)
            If iMMHg >= 0 Then
                g_iLtMM(g_iLtAnkle) = iMMHg
                txtLeftMM(g_iLtAnkle).Text = CStr(iMMHg)
            End If
            If g_JOBSTEX_FL = True Then
                nStretch = fnGetNumber(g_sRightStyleString, 6)
    ...
    Private Sub lglegdia_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            LEGRIGHT.MainForm = Me

            LEGRIGHT.g_nUnitsFac = 1 'Default to inches
            g_iRightLegStyle = -1
            LEGRIGHT.g_JOBSTEX = False
            g_JOBSTEX_FL = False
            LEGRIGHT.g_POWERNET = False
            ...
            LEGRIGHT.POWERNET.Modulus = strMod
            LEGRIGHT.POWERNET.Conversion_Renamed = strConv
    ...
    Private Sub optFabric_CheckedChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles optFabric.CheckedChanged
        If eventSender.Checked Then
            ...
            If LEGRIGHT.g_POWERNET = True And Index = 0 Then Exit Sub
            If LEGRIGHT.g_JOBSTEX = True And Index = 1 Then Exit Sub
    ...
    Private Sub optType_CheckedChanged(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles optType.CheckedChanged
        If eventSender.Checked Then
            ...
            g_iLastRightLegStyle = g_iRightLegStyle
            g_iRightLegStyle = Index

            'get the Style String g_sRightStyleString from the relevent DDE box
            'or rebuild string from existing displayed data
            Select Case g_iRightLegStyle
                Case 0
                    g_sRightStyleString = txtAnklet.Text
                    '            g_sBestCatNo = "0105"   'Anklet
                    '            g_nOptions = 0
                Case 1
                    g_sRightStyleString = txtKneeLength.Text
                    '            g_sBestCatNo = "0101"   'Knee length
                    '            g_nOptions = 0
                Case 2
                    g_sRightStyleString = txtThighLength.Text
                    '            g_sBestCatNo = "0201"   'Thigh length
                    '            g_nOptions = 0
                Case 3
                    g_sRightStyleString = txtKneeBand.Text
                ...
                Case 4
                    g_sRightStyleString = txtThighBandAK.Text
                ...
                Case 5
                    g_sRightStyleString = txtThighBandBK.Text
                    '            g_sBestCatNo = "0019"    'Thigh band
                    '            g_nOptions = 3
                    '            g_sCatNo(1) = "0019"    'Thigh band
                    '            g_sCatNo(2) = "1131"    'Stump Support Below Knee
            ...
            Select Case Index
                Case 0 'Anklet
                    ...
                    If g_iLtAnkle <> 0 Then
                        ...
                        cboToeStyle.SelectedIndex = fnGetNumber(g_sRightStyleString, 12)
                    End If

                    If LEGRIGHT.g_POWERNET = True Then cboLeftTemplate.SelectedIndex = 0
                ...
                Case 1 To 2 'Knee Length, Thigh Length
                    ...
                    If g_sRightStyleString <> "" Then
                        PR_SetLastTape(fnGetNumber(g_sRightStyleString, 3) - 1)
                    End If
                ...
                Case 3, 4, 5 'Knee Band, Thigh Band AK and Thigh Band BK
                    ...
                    If g_sRightStyleString <> "" Then
                        PR_SetFirstTape(fnGetNumber(g_sRightStyleString, 2) - 1)
                        PR_SetLastTape(fnGetNumber(g_sRightStyleString, 3) - 1)
                    End If
            ...
            If g_sRightStyleString <> "" Then
                ...
                chkLeftZipper.CheckState = fnGetNumber(g_sRightStyleString, 10)

                nValue = fnGetNumber(g_sRightStyleString, 13)
                If nValue <> 0 Then
                ...
                If fnGetNumber(g_sRightStyleString, 14) <= cboLeftTemplate.Items.Count - 1 Then
                    cboLeftTemplate.SelectedIndex = fnGetNumber(g_sRightStyleString, 14)
                Else
                    cboLeftTemplate.SelectedIndex = -1
                End If
                If g_iLtAnkle <> 0 And g_iRightLegStyle > 0 And g_iRightLegStyle < 3 And fnGetNumber(g_sRightStyleString, 5) <> 0 Then
                    txtLeftMM(g_iLtAnkle).Text = CStr(fnGetNumber(g_sRightStyleString, 5))
                    g_iLtLastFabric = -1000 'Use this to force a refigure
                    PR_FigureLeftAnkle()
                End If
            Else
                'Build style string from exisisting data
                g_sRightStyleString = FN_BuildStyleString()
            End If
    ...
    Private Sub PR_DisplayFiguredAnkle(ByRef sLeg As String, ByRef nStretch As Double, ByRef nAnkleCir As Double, ByRef nHeelCir As Double, ByRef iFabric As Short)
        ...
        nPatternDim = (nAnkleCir / (1 + (0.01 * nStretch))) / LEGRIGHT.g_nUnitsFac
        iReduction = ARMDIA1.round((1 - 1 / (0.01 * nStretch + 1)) * 100)
        iDonningStretch = ARMDIA1.round((((nHeelCir / LEGRIGHT.g_nUnitsFac) - nPatternDim) / nPatternDim) * 100)
    ...
    Private Sub PR_DrawAndSaveLeg()
        ...
        PR_PutNumberAssign("nUnitsFac", LEGRIGHT.g_nUnitsFac)
        ...
        If fnGetNumber(g_sRightStyleString, 4) < 0 Or fnGetNumber(g_sRightStyleString, 1) > 2 Or g_iLtAnkle = 0 Then
            PR_PutLine("FootLess = %true;")
            FootLess = True
        Else
            PR_PutLine("FootLess = %false;")
            'Ankle tape values
            PR_PutNumberAssign("nAnkleTape", fnGetNumber(g_sRightStyleString, 4))
            PR_PutStringAssign("sAnkleTape", Str(fnGetNumber(g_sRightStyleString, 4)))
            PR_PutStringAssign("sMMAnkle", Str(fnGetNumber(g_sRightStyleString, 5)))
            PR_PutStringAssign("sGramsAnkle", Str(fnGetNumber(g_sRightStyleString, 6)))
            PR_PutStringAssign("sReductionAnkle", Str(fnGetNumber(g_sRightStyleString, 7)))
            PR_PutNumberAssign("nReductionAnkle", fnGetNumber(g_sRightStyleString, 7))
            nReductionAnkle = fnGetNumber(g_sRightStyleString, 7)
            nHeel = fnGetNumber(g_sRightStyleString, 9)
        End If

        nFabricClass = fnGetNumber(g_sRightStyleString, 11)
        ...
        nFootPleat1 = LEGRIGHT.fnDisplaytoInches(Val(txtFootPleat1.Text))
        nFootPleat2 = LEGRIGHT.fnDisplaytoInches(Val(txtFootPleat2.Text))
        PR_PutNumberAssign("nFootPleat1", LEGRIGHT.fnDisplaytoInches(Val(txtFootPleat1.Text)))
        PR_PutNumberAssign("nFootPleat2", LEGRIGHT.fnDisplaytoInches(Val(txtFootPleat2.Text)))
        ...
        nStyleFirstTape = fnGetNumber(g_sRightStyleString, 2)
        PR_PutNumberAssign("nStyleFirstTape", nStyleFirstTape)

        nStyleLastTape = fnGetNumber(g_sRightStyleString, 3)
        PR_PutNumberAssign("nStyleLastTape", nStyleLastTape)

        'Get last tape value for use in calculating thigh length ending
        nValue = LEGRIGHT.fnDisplaytoInches(Val(Mid(sLengths, ((nStyleLastTape - 1) * 4) + 1, 4)) / 10)

        'Style type etc
        'NOTE for a footless thigh length treat as a thigh band (BK)
        nLegStyle = fnGetNumber(g_sRightStyleString, 1)
        If nLegStyle = 2 And FootLess Then nLegStyle = 5

        PR_PutNumberAssign("nLegStyle", nLegStyle)
        PR_PutStringAssign("sType", g_sRightStyleString)

        Dim nTopLegPleat1, nTopLegPleat2 As Double
        nTopLegPleat1 = LEGRIGHT.fnDisplaytoInches(Val(txtTopLegPleat1.Text))
        nTopLegPleat2 = LEGRIGHT.fnDisplaytoInches(Val(txtTopLegPleat2.Text))
        PR_PutNumberAssign("nTopLegPleat1", LEGRIGHT.fnDisplaytoInches(Val(txtTopLegPleat1.Text)))
        PR_PutNumberAssign("nTopLegPleat2", LEGRIGHT.fnDisplaytoInches(Val(txtTopLegPleat2.Text)))
        ...
        Dim xyPt1 As LEGRIGHT.XY '' = LEGRIGHT.xyRightLegInsertion ''LEGRIGHT.xyRightLegInsertion is insertion pt
        ''-----xyPt1.y = LEGRIGHT.xyRightLegInsertion.y + nOffset + nSeam
        xyPt1.y = 0 + nOffset + nSeam
        ''Add a marker at the Origin for later use in drawing the foot 
        ''--------hEnt = AddEntity("marker", "xmarker", LEGRIGHT.xyRightLegInsertion, 0.1, 0.1)
        Dim xyBase As LEGRIGHT.XY
        ...
        PR_AddDBValueToLast("Data", g_sRightStyleString)
        ...
        Dim nLoop As Double
        Dim nAnkleTape As Double = fnGetNumber(g_sRightStyleString, 4)
        If (nFabricClass = 2) Then
        ...
        While (nn <= nLoop)
            If (FNGetTape(nn)) Then
                ...
                End If
                Dim xyTmp As LEGRIGHT.XY
                xyTmp.X = xyPt1.X
                Dim nTapeLen, nLength, jj, nTickStep, nTickHt As Double
                ''------nTapeLen = FN_Round(FN_Decimalise(FNGetTape(nn)) * LEGRIGHT.g_nUnitsFac)
                nTapeLen = FN_Decimalise(FNGetTape(nn)) * LEGRIGHT.g_nUnitsFac
                nLength = n20Len / 20 * nTapeLen
                ''------xyTmp.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
                xyTmp.y = 0 + nSeam + nLength
                ''--------AddEntity("marker", "xmarker", xyTmp, 0.2, 0.2)
                PR_DrawXMarker(xyTmp)

                '' Add Pleat indication
                Dim xyText As LEGRIGHT.XY
                ...
                ''--------nTickHt = LEGRIGHT.xyRightLegInsertion.y + nSeam + ii * nTickStep * 8
                ...
                Else
                    PR_DrawText(LEGRIGHT.fnInchesToText(nTapeLen), xyText, 0.125, 0, 1)
                End If

                '' Print revised reduction at ankle tape except for anklets
                If (nn = nAnkleTape And nLegStyle <> 0) Then
                    ''--------AddEntity("text", Str(fnGetNumber(g_sRightStyleString, 7)), xyPt1.X + 0.4, nTickHt - 0.4)
                    PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.4)
                    PR_DrawText(Str(fnGetNumber(g_sRightStyleString, 7)), xyText, 0.125, 0, 1)
                    If (nFabricClass = 0) Then
                        ''------AddEntity("text", Str(fnGetNumber(g_sRightStyleString, 6)) + " grams", xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_DrawText(Str(fnGetNumber(g_sRightStyleString, 6)) + " grams", xyText, 0.125, 0, 1)
                    Else
                        ''---------AddEntity("text", Str(fnGetNumber(g_sRightStyleString, 6)) + " stretch", xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.6)
                        PR_DrawText(Str(fnGetNumber(g_sRightStyleString, 6)) + " stretch", xyText, 0.125, 0, 1)
                    End If
                    ''--------AddEntity("text", Str(fnGetNumber(g_sRightStyleString, 5)) + " mm", xyPt1.X + 0.4, nTickHt - 0.8)
                    PR_MakeXY(xyText, xyPt1.X + 0.4, nTickHt - 0.8)
                    PR_DrawText(Str(fnGetNumber(g_sRightStyleString, 5)) + " mm", xyText, 0.125, 0, 1)
                ...
                While (ii <= jj)
                    ...
                    Dim xyStart, xyEnd As LEGRIGHT.XY
                    ...
                    PR_DrawText(Str(ii), xyText, 0.1, (90 * (LEGRIGHT.PI / 180)), nTextMode)
                    nTickLength = 0.05
            ...
            If ((nn = 7) Or (FootLess And nn = nStyleFirstTape + 2)) Then
                ...
                ''----------nTxtY = LEGRIGHT.xyRightLegInsertion.y + nSeam + 2.25
                nTxtY = 0 + nSeam + 2.25
                nTxtX = xyPt1.X
                Dim sText As String
        ...
        Dim xyO As LEGRIGHT.XY
        xyO.X = xyPt1.X
        ''--------xyO.y = LEGRIGHT.xyRightLegInsertion.y
        ''-----Close("file", hChan) 
        FileClose()
        ...
        Select Case nLegStyle
            Case 0
                ...
                Dim xyEnd As LEGRIGHT.XY
                ''------------If (LEGRIGHT.xyRightLegInsertion.X < xyToeSeam.X) Then
    ...
    Private Sub PR_EstablishAnkles()
        ...
        g_iLtAnkle = 0
        g_nLtLastAnkle = 0
        g_nLtLastHeel = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(5).Text))
        ...
        g_nLtLastAnkle = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(g_iLtAnkle).Text))
    ...
    Private Sub PR_FigureLeftAnkle()
        ...
        If LEGRIGHT.g_POWERNET = True And Val(txtLeftMM(g_iLtAnkle).Text) = 0 Then Exit Sub

        'MMHg and Stretch for JOBSTEX fabric
        If (LEGRIGHT.g_JOBSTEX = True Or g_JOBSTEX_FL = True) And Val(txtLeftMM(g_iLtAnkle).Text) = 0 Then Exit Sub
        ...
        'Heel Tape
        If LEGRIGHT.fnDisplaytoInches(Val(txtLeft(5).Text)) <> g_nLtLastHeel Then GoTo CALC

        'Ankle Tape
        If LEGRIGHT.fnDisplaytoInches(Val(txtLeft(g_iLtAnkle).Text)) <> g_nLtLastAnkle Then GoTo CALC
...
CALC:
        ...
        nHeelCir = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(5).Text))
        g_nLtLastHeel = nHeelCir

        nAnkleCir = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(g_iLtAnkle).Text))
        ...
        If g_JOBSTEX_FL = True Then
            ...
            'be 25%
            nLastTapeCir = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(nLastTape).Text))
            nLastTapeMMHg = LEGRIGHT.round(FN_JOBSTEX_Pressure(nLastTapeCir, nHeelCir, 25, iZipper, iFabric))
            ...
            For ii = g_iLtAnkle + 1 To nLastTape - 1
                nTapeCir = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(ii).Text))
    ...
    Private Sub PR_FigureLeftTape(ByRef Index As Short)
        ...
        If (LEGRIGHT.g_JOBSTEX = True Or g_JOBSTEX_FL = True) And Val(txtLeftMM(g_iLtAnkle).Text) = 0 Or g_iLtStretch(g_iLtAnkle) = 0 Then Exit Sub


        'If we get to here we can assume that there is enough data to process and
        'we can calculate the relevant values.

        nTapeCir = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(Index).Text))
        iZipper = chkLeftZipper.CheckState
        iFabric = Val(VB.Left(VB6.GetItemString(cboFabric, cboFabric.SelectedIndex), 2))
    ...
    Private Sub PR_HeelChange(ByRef sLeg As String)
        ...
        If LEGRIGHT.fnDisplaytoInches(Val(txtLeft(5).Text)) = g_nLtLastHeel Then Exit Sub
    ...
    Private Sub PR_LoadFabricFromFile(ByRef sFileName As String)
        ...
        ii = 0
        Do Until EOF(fnum)
            Input(fnum, LEGRIGHT.POWERNET.Modulus(ii))
            Input(fnum, LEGRIGHT.POWERNET.Conversion_Renamed(ii))
            ii = ii + 1
        Loop
        FileClose(fnum)
    ...
    Private Sub ShiftTapesDown_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles ShiftTapesDown.Click
        ...
        For ii = 29 To 1 Step -1
            ...
            nValue = LEGRIGHT.fnDisplaytoInches(Val(txtLeft(ii - 1).Text))
            '-----grdLeftInches.Row = ii
            '------grdLeftInches.Text = LEGRIGHT.fnInchesToText(nValue)
        Next ii
        ...
        If g_iRightLegStyle = 3 Or g_iRightLegStyle = 4 Then
    ...
    Private Sub txtFootLength_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtFootLength.Enter
        LEGRIGHT.Select_Text_In_Box(txtFootLength)
    End Sub
    ...
    Private Sub txtFootLength_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtFootLength.Leave
        ''-------LEGRIGHT.Validate_And_Display_Text_In_Box(txtFootLength, labFootLengthInInches, -1)
        Dim sInchVal As String = ""
        LEGRIGHT.Validate_And_Display_Text_In_Box(txtFootLength, sInchVal)
        If sInchVal <> "" Then
    ...
    Private Sub txtLeft_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtLeft.Enter
        ...
        LEGRIGHT.Select_Text_In_Box(txtLeft(Index))
    End Sub
    ...
    Private Sub txtLeft_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtLeft.Leave
        Dim Index As Short = txtLeft.GetIndex(eventSender)
        '--------LEGRIGHT.Validate_And_Display_Text_In_Box(txtLeft(Index), grdLeftInches, Index)
        Dim sInchVal As String = ""
        LEGRIGHT.Validate_And_Display_Text_In_Box(txtLeft(Index), sInchVal)
        If sInchVal <> "" Then
    ...
    Private Sub Update_DDE_Text_Boxes()
        ...
        Select Case g_iRightLegStyle
            Case 0
                txtAnklet.Text = g_sRightStyleString
            Case 1
                txtKneeLength.Text = g_sRightStyleString
            Case 2
                txtThighLength.Text = g_sRightStyleString
            Case 3
                txtKneeBand.Text = g_sRightStyleString
            Case 4
                txtThighBandAK.Text = g_sRightStyleString
            Case 5
                txtThighBandBK.Text = g_sRightStyleString
        End Select
        txtChosenStyle.Text = Str(g_iRightLegStyle)
    End Sub
    ...
    Private Function Validate_Data() As Short
        ...
        If g_iLtAnkle = 7 And Val(txtLeft(3).Text) = 0 And g_iRightLegStyle < 3 Then
        ...
        If g_iLtAnkle = 6 And Val(txtLeft(4).Text) = 0 And g_iRightLegStyle < 3 Then
        ...
        If g_iRightLegStyle > 0 And g_iRightLegStyle < 3 And g_iLtAnkle <> 0 And (g_iLtStretch(g_iLtAnkle) = 0 Or g_iLtRed(g_iLtAnkle) = 0 Or g_iLtMM(g_iLtAnkle) = 0) Then
        ...
        If (g_iFirstTape < 5) And (cboToeStyle.SelectedIndex <= 0) And g_iRightLegStyle < 3 Then
        ...
        If g_nLtLastHeel > 0 And (VB6.GetItemString(cboToeStyle, cboToeStyle.SelectedIndex) = "Self Enclosed" Or (VB6.GetItemString(cboToeStyle, cboToeStyle.SelectedIndex) = "Soft Enclosed" And Val(txtFootLength.Text) <> 0)) Then
            ...
            nFootLength = LEGRIGHT.fnDisplaytoInches(Val(txtFootLength.Text))
    ...
    Function FN_Decimalise(ByRef nDisplay As Double) As Double
        ...
        If LEGRIGHT.g_nUnitsFac <> 1 Then
    ...
    End Function
    Sub PR_CalcPolar(ByRef xyStart As LEGRIGHT.XY, ByRef nLength As Double, ByVal nAngle As Double, ByRef xyReturn As LEGRIGHT.XY)
        Dim A, B As Double
        'Convert from degees to radians
        nAngle = nAngle * LEGRIGHT.PI / 180
        B = System.Math.Sin(nAngle) * nLength
        A = System.Math.Cos(nAngle) * nLength
        xyReturn.X = xyStart.X + A
        xyReturn.y = xyStart.y + B
    End Sub
    Sub PR_DrawXMarker(ByRef xyBlkInsert As LEGRIGHT.XY, Optional ByRef bIsOriginXMarker As Boolean = False)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim xyStart, xyEnd, xyBase, xySecSt, xySecEnd As LEGRIGHT.XY
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                ...
                Dim blkRef As BlockReference = New BlockReference(New Point3d(xyBlkInsert.X + LEGRIGHT.xyRightLegInsertion.X, xyBlkInsert.y + LEGRIGHT.xyRightLegInsertion.y, 0), blkRecId)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    blkRef.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
        ...
        End Using
    End Sub
    Private Sub PR_DrawText(ByRef sText As Object, ByRef xyInsert As LEGRIGHT.XY, ByRef nHeight As Object, ByRef nAngle As Object, ByVal nTextmode As Double)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acText As DBText = New DBText()
                acText.Position = New Point3d(xyInsert.X + LEGRIGHT.xyRightLegInsertion.X, xyInsert.y + LEGRIGHT.xyRightLegInsertion.y, 0)
                ...
                acText.AlignmentPoint = New Point3d(xyInsert.X + LEGRIGHT.xyRightLegInsertion.X, xyInsert.y + LEGRIGHT.xyRightLegInsertion.y, 0)
                ''End If
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acText.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
        ...
        End Using
    End Sub
    Sub PR_MakeXY(ByRef xyReturn As LEGRIGHT.XY, ByRef X As Double, ByRef y As Double)
        'Utility to return a point based on the X and Y values
        'given
        xyReturn.X = X
        xyReturn.y = y
    End Sub
    Sub PR_DrawLine(ByRef xyStart As LEGRIGHT.XY, ByRef xyFinish As LEGRIGHT.XY)
        'To the DRAFIX macro file (given by the global fNum).
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Dim acLine As Line = New Line(New Point3d(xyStart.X + LEGRIGHT.xyRightLegInsertion.X, xyStart.y + LEGRIGHT.xyRightLegInsertion.y, 0),
                                    New Point3d(xyFinish.X + LEGRIGHT.xyRightLegInsertion.X, xyFinish.y + LEGRIGHT.xyRightLegInsertion.y, 0))

            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                acLine.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
            End If
        ...
        End Using
    End Sub
    Sub PR_DrawMText(ByRef sText As Object, ByRef xyInsert As LEGRIGHT.XY, ByRef bIsCenter As Boolean)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            mtx.Location = New Point3d(xyInsert.X + LEGRIGHT.xyRightLegInsertion.X, xyInsert.y + LEGRIGHT.xyRightLegInsertion.y, 0)
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                mtx.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
            End If
    ...
    Private Sub PR_CalculateFootPoints(ByRef nFabricClass As Short, ByRef nFirstTape As Short, ByRef nStyleLastRed As Short,
        ...
        If (fnGetNumber(g_sRightStyleString, 4) = 7) Then
            SmallHeel = True
        ...
        ''---------nHt = LEGRIGHT.xyRightLegInsertion.X
        nHt = 0
        ...
        nSpace = 0
        Dim xyFirstTape, xyAnkleM, xyHeel, xyAnkle As LEGRIGHT.XY
        ...
        Dim nSeam As Double = 0.1875
        ''----------nTapeLen = FN_Round(FN_Decimalise(FNGetTape(nAnkleTape)) * LEGRIGHT.g_nUnitsFac)
        nTapeLen = FN_Decimalise(FNGetTape(nAnkleTape)) * LEGRIGHT.g_nUnitsFac
        If (nFabricClass = 2) Then
        ...
        End If
        ''--------xyAnkle.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
        xyAnkle.y = 0 + nSeam + nLength
        ''// At Heel (ie at 0 Tape)

        ''--------nTapeLen = FN_Round(FN_Decimalise(FNGetTape(nHeelTape)) * LEGRIGHT.g_nUnitsFac)
        nTapeLen = FN_Decimalise(FNGetTape(nHeelTape)) * LEGRIGHT.g_nUnitsFac
        nLength = nHeel20Len / 20 * nTapeLen
        nRedStep = nHeel20Len / (20 * 8)
        nLength = nLength + ((nHeelTmpltReduction - nHeelChartReduction) * nRedStep)
        ''----------xyHeel.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
        xyHeel.y = 0 + nSeam + nLength

        ''// At Minus Ankle ( ie -3 Or -1.5 Tape)

        ''--------nAnkleMTapeLen = FN_Round(FN_Decimalise(FNGetTape(nAnkleMTape)) * LEGRIGHT.g_nUnitsFac)
        nAnkleMTapeLen = FN_Decimalise(FNGetTape(nAnkleMTape)) * LEGRIGHT.g_nUnitsFac
        ...
        ''-------------xyAnkleM.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
        xyAnkleM.y = 0 + nSeam + nLength

        Dim xyAnkleMPrev, xyHeelCntrMidDistal As LEGRIGHT.XY
        ...
        ''-----------xyAnkleMPrev.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
        ...
        ''---------xyFirstTape.y = LEGRIGHT.xyRightLegInsertion.y + nLength + nSeam
        ...
        Dim xyHeelCntrDistal, xyHeelCntrMidProximal, xyHeelCntrProximal As LEGRIGHT.XY
        Dim nError As Short
        Dim xyStart, xyEnd, xyInt As LEGRIGHT.XY
        If (SmallHeel) Then
            nCalc = FN_CalcLength(xyAnkleM, xyHeelCntrMidDistal)
            nAngle = FN_CalcAngle(xyHeelCntrMidDistal, xyAnkleM) - (System.Math.Acos((nHeelR1 ^ 2 - nHeelD1 ^ 2 - nCalc ^ 2) / (-2 * nHeelD1 * nCalc)) * (180 / LEGRIGHT.PI))
        ...
        If (SmallHeel) Then
            ...
            nAngle = FN_CalcAngle(xyHeelCntrMidProximal, xyAnkle) + (System.Math.Acos((nHeelR3 ^ 2 - nHeelD2 ^ 2 - nCalc ^ 2) / (-2 * nHeelD2 * nCalc)) * (180 / LEGRIGHT.PI))
        ...
        ''// Toes
        ''// Quick and dirty ;
        ''-----------xyToeSeam.y = LEGRIGHT.xyRightLegInsertion.y + nSeam
        xyToeSeam.y = 0 + nSeam
        ...
        If (sToeStyle.Equals("Straight") Or sToeStyle.Equals("Soft Enclosed B/M")) Then
            ...
            ''-----------xyToeOFF.y = LEGRIGHT.xyRightLegInsertion.y + nSeam + nLength
        ...
        If (sToeStyle.Equals("Self Enclosed") Or (sToeStyle.Equals("Soft Enclosed") And Val(sFootLength))) Then
            ...
            Else
                sFootLabel = "CAP"
            End If
            ''----------If (nFootLength = FN_Round(FN_Decimalise(Value("scalar", sFootLength)) * LEGRIGHT.g_nUnitsFac)) Then
            ''------------If (nFootLength = FN_Round(FN_Decimalise(Val(sFootLength)) * LEGRIGHT.g_nUnitsFac)) Then
            nFootLength = FN_Decimalise(Val(sFootLength)) * LEGRIGHT.g_nUnitsFac
            If (nFootLength <> 0) Then
                ...
                End If
                ''---------nLength = (nHeel20Len / 20 * 12.5) + LEGRIGHT.xyRightLegInsertion.y + nSeam
                nLength = (nHeel20Len / 20 * 12.5) + 0 + nSeam
                If (nLength < xyHeel.y) Then
                    ''-------------
                    'nError = FN_CirLinInt(LEGRIGHT.xyRightLegInsertion.X - 20, LEGRIGHT.xyRightLegInsertion.y + nSeam,
                    '   xyHeel.X, LEGRIGHT.xyRightLegInsertion.y + nSeam,
                    '          xyHeel.X, nLength,
                    '          nFootLength)
                    ''------------PR_MakeXY(xyStart, xyRightLegInsertion.X - 20, xyRightLegInsertion.y + nSeam)
                    PR_MakeXY(xyStart, 0 - 20, 0 + nSeam)
                    ''------------PR_MakeXY(xyEnd, xyHeel.X, xyRightLegInsertion.y + nSeam)
                    PR_MakeXY(xyEnd, xyHeel.X, 0 + nSeam)
                    Dim xyCen As LEGRIGHT.XY
        ...
        Dim xyToeCntrLow, xyToeCntrMid, xyToeCntrHigh As LEGRIGHT.XY
        ...
        ''// Skip to ankletape 
        Dim nStyleFirstTape As Double = fnGetNumber(g_sRightStyleString, 2)
        ''------------xyTmp = LEGRIGHT.xyRightLegInsertion
        PR_MakeXY(xyTmp, 0, 0)
        While (nn < nAnkleTape)
            ''-------GetLine(hChan, & sLine)
            sLine = LineInput(hChan)
        ...
        nn = nAnkleTape
        Dim nStyleLastTape As Double = fnGetNumber(g_sRightStyleString, 3)
        Dim nLegStyle As Double = fnGetNumber(g_sRightStyleString, 1)
        ...
        For ii = 0 To ptPolyColl.Count - 1
            ptCollSpline.Add(New Point3d(LEGRIGHT.xyRightLegInsertion.X + ptPolyColl(ii).X, LEGRIGHT.xyRightLegInsertion.y + ptPolyColl(ii).Y, 0))
        Next ii
        ARMDIA1.PR_SetLayer("TemplateRight")
        ...
        PR_AddDBValueToLast("Insertion", Str(LEGRIGHT.xyRightLegInsertion.X) & " " & Str(LEGRIGHT.xyRightLegInsertion.y))
        ''---------------------------
        ...
        PR_AddDBValueToLast("ID", sLegStyle + txtFileNo.Text + txtLeg.Text + "Ankle")
        ...
        PR_AddDBValueToLast("HeelID", sLegStyle + txtFileNo.Text + txtLeg.Text + "Heel")
        ...
        PR_AddDBValueToLast("ID", sLegStyle + txtFileNo.Text + txtLeg.Text + "AnkleM")
        ...
        If (SmallHeel = True) Then
            ...
            PR_DrawArc(xyHeelCntrProximal, nHeelR3, (180 * (LEGRIGHT.PI / 180)), (270 * (LEGRIGHT.PI / 180)))
        End If
        ...
        If (sToeStyle.Equals("Soft Enclosed") And nFootLength) Then
            ...
            sFootLabel = LEGRIGHT.fnInchesToText(nRightOffset) + Chr(34) + " Soft Enclosed"
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
    ...
    End Sub
    Private Sub PR_DrawArc(ByRef xyCen As LEGRIGHT.XY, ByRef nRad As Double, ByRef nStartAng As Double, ByRef nEndAng As Double)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acArc As Arc = New Arc(New Point3d(LEGRIGHT.xyRightLegInsertion.X + xyCen.X, LEGRIGHT.xyRightLegInsertion.y + xyCen.y, 0),
                                         nRad, nStartAng, nEndAng)

                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acArc.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
                '' Add the new object to the block table record and the transaction
        ...
        End Using
    End Sub
    Function FN_CalcAngle(ByRef xyStart As LEGRIGHT.XY, ByRef xyEnd As LEGRIGHT.XY) As Double
        ...
        'All other cases
        rAngle = System.Math.Atan(y / X) * (180 / LEGRIGHT.PI) 'Convert to degrees
    ...
    Function FN_CalcLength(ByRef xyStart As LEGRIGHT.XY, ByRef xyEnd As LEGRIGHT.XY) As Double
    ...
    End Function
    Function FN_CirLinInt(ByRef xyStart As LEGRIGHT.XY, ByRef xyEnd As LEGRIGHT.XY, ByRef xyCen As LEGRIGHT.XY, ByRef nRad As Double, ByRef xyInt As LEGRIGHT.XY) As Short
    ...
    Private Sub PR_DrawSpline(ByRef PointCollection As Point3dCollection)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acSpline As Spline = New Spline(PointCollection, vecTan, vecTan, 4, 0.0)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acSpline.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
                '' Add the new object to the block table record and the transaction
        ...
        End Using
    End Sub
    Private Sub PR_DrawClosingLines(ByRef xyO As LEGRIGHT.XY, ByRef xyProfileLast As LEGRIGHT.XY,
        ...
        Else
            ...
            aAngle = FN_CalcAngle(xyPrevProfileLast, xyProfileLast) * (LEGRIGHT.PI / 180)
            nB = (xyO.X - nThighPltXoff) - xyPrevProfileLast.X
            nA = System.Math.Tan(aAngle) * nB
            Dim xyThighPlt, xyTmp As LEGRIGHT.XY
            ...
            If (xyTmp.y < xyO.y) Then
                ''If (xyTmp.y < LEGRIGHT.xyRightLegInsertion.y) Then
                ''// Special case where center point Is below fold line
                Dim xyPt1 As LEGRIGHT.XY
                xyPt1.y = xyO.y + 0.5
                xyPt1.X = xyO.X
                ...
                nThighPltStartAngle = FN_CalcAngle(xyTmp, xyPt1) * (LEGRIGHT.PI / 180)
                nThighPltDeltaAngle = (FN_CalcAngle(xyTmp, xyThighPlt) - nThighPltStartAngle) * (LEGRIGHT.PI / 180)

            Else
                ''// Normal case
                nThighPltStartAngle = 0
                nThighPltDeltaAngle = (FN_CalcAngle(xyTmp, xyThighPlt) * (LEGRIGHT.PI / 180)) - nThighPltStartAngle
            End If
            nThighPltDeltaAngle = nThighPltDeltaAngle + (1 * (LEGRIGHT.PI / 180))
            ...
            ''--------SetDBData(hEnt, "ID", sLegStyle + sFileNo + sLeg + "ClosingLine")
            PR_CalcPolar(xyTmp, nThighPltRad, nThighPltStartAngle * (180 / LEGRIGHT.PI), xyPolar)
        ...
        End If
        ''// Closing line to TOE
        Dim xyStart, xyEnd, xyLeg As LEGRIGHT.XY
        PR_MakeXY(xyLeg, 0, 0)
        ...
        PR_DrawLine(xyStart, xyEnd)
    End Sub
    Private Sub PR_DrawLegForThighLand(ByRef nFabricClass As Short, ByRef nStyleLastRed As Short, ByRef xyO As LEGRIGHT.XY,
                                      ByRef nB As Double, ByRef nA As Double)
        ''-----------PR_PutLine("@" & g_sPathJOBST & "\LEG\LGBNDDWG.D;")
        ''// Establish layer
        Dim strLayer As String = ""
        ...
        If bIsSpline = True Then
            ...
            Dim ii As Double
            For ii = 0 To ptPolyColl.Count - 1
                ptColl.Add(New Point3d(LEGRIGHT.xyRightLegInsertion.X + ptPolyColl(ii).X, LEGRIGHT.xyRightLegInsertion.y + ptPolyColl(ii).Y, 0))
            Next ii
        ...
        If (nLegStyle = 2 Or nLegStyle = 4 Or nLegStyle = 5) Then
            ...
            '                 nThighPltDeltaAngle)
            'SetDBData(hEnt, "ID", sLegStyle + sFileNo + sLeg + "ClosingLine")
            PR_DrawThighArc(xyTmp, nThighPltRad, (nThighPltStartAngle * (LEGRIGHT.PI / 180)), (nThighPltDeltaAngle * (LEGRIGHT.PI / 180)))
            PR_AddDBValueToLast("ID", sLegStyle + txtFileNo.Text + txtLeg.Text + "ClosingLine")
        ...
        If (nElastic = 1) Then
            ...
            PR_DrawText("ELASTIC", xyStart, 0.1, (90 * (LEGRIGHT.PI / 180)), 1)
        Else
            ''-----------AddEntity("text", "NO ELASTIC", xyProfileStart.X + 0.25, xyOtemplate.y + (xyProfileStart.y - xyOtemplate.y) / 2)
            PR_MakeXY(xyStart, xyProfileStart.X + 0.25, xyProfileStart.y / 2)
            PR_DrawText("NO ELASTIC", xyStart, 0.1, (90 * (LEGRIGHT.PI / 180)), 1)
        End If
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acPoly As Polyline = New Polyline()
                For ii = 0 To PointCollection.Count - 1
                    acPoly.AddVertexAt(ii, New Point2d(LEGRIGHT.xyRightLegInsertion.X + PointCollection(ii).X, LEGRIGHT.xyRightLegInsertion.y + PointCollection(ii).Y), 0, 0, 0)
                Next ii

                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acPoly.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
                '' Add the new object to the block table record and the transaction
    ...
    Private Sub readDWGInfo()
        Try
            ...
            resbuf = _sClass.GetXrecord("LegRightInfo", "LEGRIGHTDIC")
    ...
    Private Sub saveInfoToDWG()
        Try
            Dim _sClass As New SurroundingClass()
            If (_sClass.GetXrecord("LegRightInfo", "LEGRIGHTDIC") IsNot Nothing) Then
                _sClass.RemoveXrecord("LegRightInfo", "LEGRIGHTDIC")
            End If
            ...
            _sClass.SetXrecord(resbuf, "LegRightInfo", "LEGRIGHTDIC")
    ...
    Private Sub PR_GetInsertionPoint()
        ...
        PR_MakeXY(LEGRIGHT.xyRightLegInsertion, ptStart.X, ptStart.Y)
    End Sub
    Private Sub PR_DrawRuler(ByRef sSymbol As String, ByRef xyStart As LEGRIGHT.XY, ByRef sTape As String)
        Dim xyPt As LEGRIGHT.XY
        Dim xyTapeFst, xyTapeSec, xyTapeEnd, xyTapeText As LEGRIGHT.XY
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                ...
                Next
                'Create new block reference 
                Dim blkRef As BlockReference = New BlockReference(New Point3d(xyStart.X + LEGRIGHT.xyRightLegInsertion.X, xyStart.y + LEGRIGHT.xyRightLegInsertion.y, 0), blkRecId)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    blkRef.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
                '' Open the Block table record Model space for write
    ...
    Private Sub PR_DrawLegCommonBlock()
        ...
        End Using
    End Sub
    Private Sub PR_DrawLegRightBlock()
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim blkId As ObjectId = New ObjectId()
        Dim obj As New BlockCreation.BlockCreation
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Dim ptPosition As Point3d = blkRefPatient.Position
            Dim blkRecId As ObjectId = ObjectId.Null
            If Not acBlkTbl.Has("LEGRIGHT") Then
                Dim blkTblRecCross As BlockTableRecord = New BlockTableRecord()
                blkTblRecCross.Name = "LEGRIGHT"
                Dim acPoly As Polyline = New Polyline()
                Dim ii As Double
                For ii = 1 To 5
                    acPoly.AddVertexAt(ii - 1, New Point2d(xyStart(ii), xyEnd(ii)), 0, 0, 0)
        ...
        End Using
    End Sub
    Private Sub PR_DrawThighArc(ByRef xyCen As LEGRIGHT.XY, ByRef nRad As Double, ByRef nStartAng As Double, ByRef nDeltaAng As Double)
        ' this procedure draws an arc between two points
    ...
    Sub PR_DrawKneeContracturePoly(ByRef PointCollection As Point3dCollection, Optional ByVal bIsSetScale As Boolean = False)
        ...
        End Using
    End Sub
    Private Sub PR_DrawKneeContractureText(ByRef sText As Object, ByRef xyInsert As LEGRIGHT.XY, ByRef nHeight As Object, ByRef nAngle As Object, ByVal nTextmode As Double)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
    ...
    Private Sub PR_DrawKneeContracture()
        ...
        Dim xyOtemplate As LEGRIGHT.XY
        ...
        If (nn + 1 > fitPtsCount) Then
            ...
            Exit Sub
        End If
        Dim xyPt1, xyPt2, xyInt As LEGRIGHT.XY
    ...
    End Sub
    Sub PR_DrawStumpLine(ByRef xyStart As LEGRIGHT.XY, ByRef xyFinish As LEGRIGHT.XY)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        End Using
    End Sub
    Sub PR_DrawStumpCircle(ByRef xyCen As LEGRIGHT.XY, ByRef nRadius As Object)
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
            strLayout = BlockTableRecord.PaperSpace
        End If
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            Using acCirc As Circle = New Circle()
                ...
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    acCirc.TransformBy(Matrix3d.Scaling(2.54, New Point3d(LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y, 0)))
                End If
        ...
        End Using
    End Sub
    Sub PR_DrawClosedArrow(ByRef xyPoint As LEGRIGHT.XY, ByRef nAngle As Object)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
        ...
        End Using
    End Sub
    Private Sub PR_DrawStumpText(ByRef sText As Object, ByRef xyInsert As LEGRIGHT.XY, ByRef nHeight As Object, ByRef nAngle As Object, ByVal nTextmode As Double)
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim strLayout As String = BlockTableRecord.ModelSpace
        If fnGetSettingsPath("DRAW_IN_LAYOUT").ToUpper().Equals("YES", StringComparison.InvariantCultureIgnoreCase) Then
    ...
    Private Sub PR_DrawLegStump()
        ...
        End If
        ''// Get leg box data
        Dim sSymbol As String = "LEGRIGHT"
        ...
        Dim nTapeLen As Double = ARMDIA1.round(FN_Decimalise(FNGetTape(nFirstTape)) * LEGRIGHT.g_nUnitsFac)
        Dim nSeam As Double = 0.1875
        Dim nLength As Double = ((nTapeLen * 0.78) / 2) + nSeam
        Dim nTol As Double = System.Math.Abs(nLength - ((nTapeLen * 0.95) + nSeam)) + 0.125
        Dim xyDistal, xyOtemplate, xyTmp As LEGRIGHT.XY
        PR_MakeXY(xyOtemplate, LEGRIGHT.xyRightLegInsertion.X, LEGRIGHT.xyRightLegInsertion.y)
        ...
        End If
        ''// Draw Stumps
        Dim xyFirstProfile As LEGRIGHT.XY = xyDistal
        ...
        If nLength > 3 Then
            ...
            Dim xyPt2, xyMidPt, xyApexPt, xyInt, xyPt3 As LEGRIGHT.XY
            ...
            Dim xyStart, xyEnd, xyArrow As LEGRIGHT.XY
    ...
    Private Sub PR_DrawHeelContracture()
        ...
        Dim xyOtemplate, xyHeel As LEGRIGHT.XY
        ...
        Dim xyPt1, xyPt2, xyPt3 As LEGRIGHT.XY
