...
Friend Class WEBSPACRRight_frm
    ...
    Private Sub Form_LinkClose()
        ...
        If sInsert <> "" And Val(sInsert) > 0 Then
    ...
    Private Sub webspacr_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            For ii = 1 To WEBSPACRRight_bas.WEB_NOFF_ARMTAPES
                RightWebTapeNote(ii).nCir = 0
                RightWebTapeNote(ii).iMMs = 0
                RightWebTapeNote(ii).iRed = 0
                RightWebTapeNote(ii).iGms = 0
                RightWebTapeNote(ii).sNote = ""
                RightWebTapeNote(ii).iTapePos = 0
                RightWebTapeNote(ii).sTapeText = ""
            Next ii
    ...
    Private Sub PR_CreateDrawMacro(ByRef sFileName As String)
        ...
        If g_RightWebExtendTo = RIGHT_WEB_GLOVE_ELBOW Or g_RightWebExtendTo = RIGHT_GLOVE_PASTWRIST Then
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                sText = Trim(fnDisplayToCM(WEBSPACRRight_bas.FN_CalcLength(xyPt1, xyPt2) + 1.5)) & " ELASTIC"
            Else
                sText = sText & Chr(34) & " ELASTIC"
            End If
            WEBSPACRRight_bas.PR_DrawText(sText, xyPt3, 0.1, 0, True)
        ...
        ''WEBSPACRRight_bas.PR_DrawArc(xyRightTopThumbRad, xyRightWebT(1), xyRightWebT(2))
        WEBSPACRRight_bas.PR_DrawArc(xyRightTopThumbRad, xyRightWebT(2), xyRightWebT(1))
        X1(1) = xyRightWebT(2).X
        Y1(1) = xyRightWebT(2).Y
        X1(2) = xyRightWebT(3).X
        Y1(2) = xyRightWebT(3).Y
    ...
    Private Sub PR_DrawTapeNotes(ByRef iVertex As Short, ByRef xyStart As WEBSPACRRight_bas.xy, ByRef xyEnd As WEBSPACRRight_bas.xy, ByRef nOffSet As Double, ByRef sZipperID As String)
        ...
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            nInchToCM = 2.54
        End If
        iInt = Int(RightWebTapeNote(iVertex).nCir * nInchToCM)
        ...
        nDec = ((RightWebTapeNote(iVertex).nCir * nInchToCM) - iInt)
        If nDec > 0 Then
            iInt = ARMDIA1.round((nDec / RIGHT_WEB_EIGHTH) * 10) / 10
            If iInt <> 0 Then
                WEBSPACRRight_bas.PR_AddDBValueToLast("TapeLengths2", Trim(Str(iInt)))
                bIsTapeLen2 = True
            End If
        End If
        WEBSPACRRight_bas.PR_AddDBValueToLast("Reduction", "14")
        PR_DrawWebTapeNotes(xyDatum, RightWebTapeNote(iVertex).sTapeText, "14", strTapeLen, bIsTapeLen2, Trim(Str(iInt)))
            ...
            For nn = 8 To 23
                ...
                If nValue > 0 Then
                    ...
                    If nLen <> -1 Then
                        ...
                        lblExtCir(nn - 8).Text = ARMEDDIA1.fnInchesToText(nLen)
    ...
    Private Sub PR_GetBlkAttributeValues()
            ...
            If blkRecIdRight <> ObjectId.Null Then
                ...
                For Each objID As ObjectId In blkTblRec
                    ...
                    If TypeOf dbObj Is AttributeDefinition Then
                        ...
                        ElseIf acAttDef.Tag.ToUpper().Equals("TapeLengthPt1", StringComparison.InvariantCultureIgnoreCase) Then
                            txtTapeLengthPt1.Text = acAttDef.TextString
                        ElseIf acAttDef.Tag.ToUpper().Equals("Data", StringComparison.InvariantCultureIgnoreCase) Then
                            txtDataGlove.Text = acAttDef.TextString
                        ElseIf acAttDef.Tag.ToUpper().Equals("WristPleat", StringComparison.InvariantCultureIgnoreCase) Then
                            txtWristPleat.Text = acAttDef.TextString
                        ElseIf acAttDef.Tag.ToUpper().Equals("ShoulderPleat", StringComparison.InvariantCultureIgnoreCase) Then
                            txtShoulderPleat.Text = acAttDef.TextString
                        End If
                    End If
                Next
            End If
        End Using
    End Sub
    Private Sub PR_ReadRightGlvInfo()
        Try
            ...
            If (resbuf IsNot Nothing) Then
                ...
                txtFabric.Text = arr(172).Value
                txtTapeLengths.Text = arr(173).Value
                txtTapeLengths2.Text = arr(174).Value
                txtTapeLengthPt1.Text = arr(175).Value
                txtDataGlove.Text = arr(176).Value
                txtWristPleat.Text = arr(177).Value
                txtShoulderPleat.Text = arr(178).Value
            End If
        Catch ex As Exception
    ...
    Function fnDisplayToCM(ByRef nInches As Double) As String
        ...
        End If
    End Function
    Sub PR_DrawWebTapeNotes(ByRef xyPoint As WEBSPACRRight_bas.xy, ByRef sData As String, ByRef sReduction As String, ByRef sTapeLen As String,
                            ByRef bIsTapeLen2 As Boolean, ByRef sTapeLen2 As String)
        ...
        Dim acDoc As Document = Application.DocumentManager.MdiActiveDocument
        Dim acCurDb As Database = acDoc.Database
        Dim sName As String = "WEBTAPENOTES"
        Dim xyBase, xyCenter, xyData, xyTapeLen, xyTapeLen2 As WEBSPACRRight_bas.xy
        WEBSPACRRight_bas.PR_MakeXY(xyBase, 0, 0)
        WEBSPACRRight_bas.PR_MakeXY(xyCenter, xyBase.X + 0.75, xyBase.Y + 0.135)
        WEBSPACRRight_bas.PR_MakeXY(xyData, xyBase.X - 0.25, xyBase.Y - 0.15)
        WEBSPACRRight_bas.PR_MakeXY(xyTapeLen, xyBase.X - 0.6, xyBase.Y + 0.1)
        If sTapeLen.Length > 1 Then
            WEBSPACRRight_bas.PR_MakeXY(xyTapeLen, xyBase.X - 0.65, xyBase.Y + 0.1)
        End If
        WEBSPACRRight_bas.PR_MakeXY(xyTapeLen2, xyBase.X - 0.4994, xyBase.Y + 0.1655)
        ...
        strTag(1) = "Data"
        strTag(2) = "Reduction"
        strTag(3) = "TapeLengths"
        strTextString(1) = sData
        strTextString(2) = sReduction
        strTextString(3) = sTapeLen
        '' Start a transaction
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                ...
                Dim blkRef As BlockReference = New BlockReference(New Point3d(xyPoint.X + WEBSPACRRight_bas.xyWebInsertion.X, xyPoint.Y + WEBSPACRRight_bas.xyWebInsertion.Y, 0), blkRecId)
                ''blkRef.Rotation = nAngle * (PI / 180)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    blkRef.TransformBy(Matrix3d.Scaling(2.54, New Point3d(WEBSPACRRight_bas.xyWebInsertion.X, WEBSPACRRight_bas.xyWebInsertion.Y, 0)))
                End If
