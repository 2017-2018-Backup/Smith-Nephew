...
Friend Class webspacr
    ...
    Private Sub webspacr_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            For ii = 1 To WEBSPACR1.WEB_NOFF_ARMTAPES
                WebTapeNote(ii).nCir = 0
                WebTapeNote(ii).iMMs = 0
                WebTapeNote(ii).iRed = 0
                WebTapeNote(ii).iGms = 0
                WebTapeNote(ii).sNote = ""
                WebTapeNote(ii).iTapePos = 0
                WebTapeNote(ii).sTapeText = ""
            Next ii
    ...
    Private Sub PR_CreateDrawMacro(ByRef sFileName As String)
        ...
        If g_WebExtendTo = WEB_GLOVE_ELBOW Or g_WebExtendTo = GLOVE_PASTWRIST Then
            ...
            If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                sText = Trim(fnDisplayToCM(WEBSPACR1.FN_CalcLength(xyPt1, xyPt2) + 1.5)) & " ELASTIC"
            Else
                sText = sText & Chr(34) & " ELASTIC"
            End If
            WEBSPACR1.PR_DrawText(sText, xyPt3, 0.1, 0, True)
        ...
        ''WEBSPACR1.PR_DrawArc(xyTopThumbRad, xyWebT(1), xyWebT(2))
        WEBSPACR1.PR_DrawArc(xyTopThumbRad, xyWebT(2), xyWebT(1))
        'BDYUTILS.PR_StartPoly()
        'BDYUTILS.PR_AddVertex(xyWebT(2), 0)
        'BDYUTILS.PR_AddVertex(xyWebT(3), 0)
        'BDYUTILS.PR_AddVertex(xyWebT(4), 0)
    ...
    Private Sub PR_DrawTapeNotes(ByRef iVertex As Short, ByRef xyStart As WEBSPACR1.xy, ByRef xyEnd As WEBSPACR1.xy, ByRef nOffSet As Double, ByRef sZipperID As String)
        ...
        If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
            nInchToCM = 2.54
        End If
        iInt = Int(WebTapeNote(iVertex).nCir * nInchToCM)
        ...
        nDec = ((WebTapeNote(iVertex).nCir * nInchToCM) - iInt)
        If nDec > 0 Then
            iInt = ARMDIA1.round((nDec / WEB_EIGHTH) * 10) / 10
            If iInt <> 0 Then
                WEBSPACR1.PR_AddDBValueToLast("TapeLengths2", Trim(Str(iInt)))
                bIsTapeLen2 = True
            End If
        End If
        WEBSPACR1.PR_AddDBValueToLast("Reduction", "14")
        PR_DrawWebTapeNotes(xyDatum, WebTapeNote(iVertex).sTapeText, "14", strTapeLen, bIsTapeLen2, Trim(Str(iInt)))
    ...
    Private Sub PR_GetBlkAttributeValues()
        ...
        End Using
    End Sub
    Private Sub PR_ReadLeftGlvInfo()
        Try
    ...
    Function fnDisplayToCM(ByRef nInches As Double) As String
        ...
        End If
    End Function
    Sub PR_DrawWebTapeNotes(ByRef xyPoint As WEBSPACR1.xy, ByRef sData As String, ByRef sReduction As String, ByRef sTapeLen As String,
        ...
        Dim xyBase, xyCenter, xyData, xyTapeLen, xyTapeLen2 As WEBSPACR1.xy
        WEBSPACR1.PR_MakeXY(xyBase, 0, 0)
        WEBSPACR1.PR_MakeXY(xyCenter, xyBase.X + 0.75, xyBase.Y + 0.135)
        WEBSPACR1.PR_MakeXY(xyData, xyBase.X - 0.25, xyBase.Y - 0.15)
        WEBSPACR1.PR_MakeXY(xyTapeLen, xyBase.X - 0.6, xyBase.Y + 0.1)
        If sTapeLen.Length > 1 Then
            WEBSPACR1.PR_MakeXY(xyTapeLen, xyBase.X - 0.65, xyBase.Y + 0.1)
        End If
        WEBSPACR1.PR_MakeXY(xyTapeLen2, xyBase.X - 0.4994, xyBase.Y + 0.1655)
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If blkRecId <> ObjectId.Null Then
                ...
                Dim blkRef As BlockReference = New BlockReference(New Point3d(xyPoint.X + WEBSPACR1.xyWebInsertion.X, xyPoint.Y + WEBSPACR1.xyWebInsertion.Y, 0), blkRecId)
                ''blkRef.Rotation = nAngle * (PI / 180)
                If fnGetSettingsPath("UNITS").ToUpper().Equals("CM", StringComparison.InvariantCultureIgnoreCase) Then
                    blkRef.TransformBy(Matrix3d.Scaling(2.54, New Point3d(WEBSPACR1.xyWebInsertion.X, WEBSPACR1.xyWebInsertion.Y, 0)))
                End If
                ...
                Using rb As New ResultBuffer
                    ...
                    rb.Add(New TypedValue(DxfCode.ExtendedDataAsciiString, g_sWebFileNo + "Left"))
                    blkRef.XData = rb
                End Using
