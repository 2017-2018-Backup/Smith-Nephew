...
Friend Class BDLEGRight_frm
    ...
    Private Sub Cancel_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Cancel.Click
        ...
        If sCurrentValues <> BDLEGRIGHT.g_sChangeChecker Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Bodysuit - Right Leg")
    ...
    Public Function PR_CloseBodyRightLeg() As Boolean
        Dim Response As Short
    ...
    Private Sub cmdExtendLegTapes_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles cmdExtendLegTapes.Click
        ...
        Text1(ii + 1).Text = CStr(BDLEGRIGHT.fnInchesToDisplay(nValue))
    ...
    Private Function FN_Open(ByRef sDrafixFile As String, ByRef sType As String, ByRef sName As Object, ByRef sFileNo As Object) As Short
        ...
        'Open file
        BDLEGRIGHT.fNum = FreeFile()
        FileOpen(BDLEGRIGHT.fNum, sDrafixFile, VB.OpenMode.Output)
        FN_Open = BDLEGRIGHT.fNum

        'Write header information etc. to the DRAFIX macro file
        '
        PrintLine(BDLEGRIGHT.fNum, "//DRAFIX Macro created - " & DateString & "  " & TimeString)
        PrintLine(BDLEGRIGHT.fNum, "//Patient - " & sName & ", " & sFileNo & "")
        PrintLine(BDLEGRIGHT.fNum, "//by Visual Basic, Waist Height Leg")
        PrintLine(BDLEGRIGHT.fNum, "//type - " & sType & "")

        'Clear user selections etc
        PrintLine(BDLEGRIGHT.fNum, "UserSelection (" & BDLEGRIGHT.QQ & "clear" & BDLEGRIGHT.QQ & ");")
        PrintLine(BDLEGRIGHT.fNum, "Execute (" & BDLEGRIGHT.QQ & "menu" & BDLEGRIGHT.QCQ & "SetColor" & BDLEGRIGHT.QC & "Table(" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QCQ & "color" & BDLEGRIGHT.QCQ & "bylayer" & BDLEGRIGHT.QQ & "));")
        PrintLine(BDLEGRIGHT.fNum, "Execute (" & BDLEGRIGHT.QQ & "menu" & BDLEGRIGHT.QCQ & "SetStyle" & BDLEGRIGHT.QC & "Table(" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QCQ & "style" & BDLEGRIGHT.QCQ & "bylayer" & BDLEGRIGHT.QQ & "));")

        'Display Hour Glass symbol
        PrintLine(BDLEGRIGHT.fNum, "Display (" & BDLEGRIGHT.QQ & "cursor" & BDLEGRIGHT.QCQ & "wait" & BDLEGRIGHT.QCQ & sType & BDLEGRIGHT.QQ & ");")
    End Function
    ...
    Private Sub Form_LinkClose()
        ...
        If txtUnits.Text = "cm" Then
            BDLEGRIGHT.g_nUnitsFac = 10 / 25.4
        Else
            BDLEGRIGHT.g_nUnitsFac = 1
        End If
        ...
        BDLEGRIGHT.g_nAnkleTape = 0
        BDLEGRIGHT.g_nAnkleLen = 0
        BDLEGRIGHT.g_nHeelLen = ARMEDDIA1.fnDisplayToInches(Val(Text1(5).Text))

        If (BDLEGRIGHT.g_nHeelLen <> 0) And (BDLEGRIGHT.g_nHeelLen < HEEL_TOL) Then
            BDLEGRIGHT.g_nAnkleTape = 6 'Small heel Ankle at +1 1/2 tape
            BDLEGRIGHT.g_nAnkleLen = ARMEDDIA1.fnDisplayToInches(Val(Text1(BDLEGRIGHT.g_nAnkleTape).Text))
        Else
            BDLEGRIGHT.g_nAnkleTape = 7 'Large heel Ankle at +3 tape
            BDLEGRIGHT.g_nAnkleLen = ARMEDDIA1.fnDisplayToInches(Val(Text1(BDLEGRIGHT.g_nAnkleTape).Text))
        End If
        ...
        If BDLEGRIGHT.g_nHeelLen = 0 Then
            optBelowKnee.Enabled = True
            optAboveKnee.Enabled = True
        ...
        BDLEGRIGHT.g_sChangeChecker = FN_ValuesString()
        Show()
    ...
    Private Sub bdlegdia_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            BDLEGRIGHT.MainForm = Me
            'Initialize globals
            BDLEGRIGHT.QQ = Chr(34) 'Double quotes (")
            BDLEGRIGHT.NL = Chr(13) 'New Line
            BDLEGRIGHT.CC = Chr(44) 'The comma (,)
            BDLEGRIGHT.QCQ = BDLEGRIGHT.QQ & BDLEGRIGHT.CC & BDLEGRIGHT.QQ
            BDLEGRIGHT.QC = BDLEGRIGHT.QQ & BDLEGRIGHT.CC
            BDLEGRIGHT.CQ = BDLEGRIGHT.CC & BDLEGRIGHT.QQ

            BDLEGRIGHT.g_sPathJOBST = fnPathJOBST()
            BDLEGRIGHT.g_nUnitsFac = 1 'Default to inches
    ...
    Private Sub PR_CreateMacro_Save(ByRef sDrafixFile As String)
        ...
        BDLEGRIGHT.fNum = FN_Open(sDrafixFile, "Save Data ONLY", (txtPatientName.Text), (txtFileNo.Text))
        ...
        If txtUidLeg.Text = "" Then
            'Define DB Fields
            PR_PutLine("@" & BDLEGRIGHT.g_sPathJOBST & "\WAIST\WHFIELDS.D;")
            ...
            PR_PutLine("hMPD = UID (" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QC & Val(txtUidTitle.Text) & ");")
            PR_PutLine("if (hMPD)")
            PR_PutLine("  GetGeometry(hMPD, &sMPD_Name, &xyMPD_Origin, &xyMPD_Scale, &aMPD_Angle);")
            PR_PutLine("else")
            PR_PutLine("  Exit(%cancel," & BDLEGRIGHT.QQ & "Can't find > mainpatientdetails < symbol, Insert Patient Data" & BDLEGRIGHT.QQ & ");")

            'Insert symbol
            PR_PutLine("if ( Symbol(" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QCQ & sSymbol & BDLEGRIGHT.QQ & ")){")
            PR_PutLine("  Execute (" & BDLEGRIGHT.QQ & "menu" & BDLEGRIGHT.QCQ & "SetLayer" & BDLEGRIGHT.QC & "Table(" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QCQ & "layer" & BDLEGRIGHT.QCQ & "Data" & BDLEGRIGHT.QQ & "));")
            If txtLeg.Text = "Left" Then
                PR_PutLine("  hLeg = AddEntity(" & BDLEGRIGHT.QQ & "symbol" & BDLEGRIGHT.QCQ & sSymbol & BDLEGRIGHT.QC & "xyMPD_Origin.x + 1.5,xyMPD_Origin.y );")
            Else
                PR_PutLine("  hLeg = AddEntity(" & BDLEGRIGHT.QQ & "symbol" & BDLEGRIGHT.QCQ & sSymbol & BDLEGRIGHT.QC & "xyMPD_Origin.x + 3,xyMPD_Origin.y );")
            End If
            PR_PutLine("  }")
            PR_PutLine("else")
            PR_PutLine("  Exit(%cancel, " & BDLEGRIGHT.QQ & "Can't find >" & sSymbol & "< symbol to insert\nCheck your installation, that JOBST.SLB exists!" & BDLEGRIGHT.QQ & ");")
        Else
            'Use existing symbol
            PR_PutLine("hLeg = UID (" & BDLEGRIGHT.QQ & "find" & BDLEGRIGHT.QC & Val(txtUidLeg.Text) & ");")
            PR_PutLine("if (!hLeg) Exit(%cancel," & BDLEGRIGHT.QQ & "Can't find >" & sSymbol & "< symbol to update!" & BDLEGRIGHT.QQ & ");")
        End If

        'Update the BODY Box symbol
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "fileno" & BDLEGRIGHT.QCQ & txtFileNo.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "Leg" & BDLEGRIGHT.QCQ & txtLeg.Text & BDLEGRIGHT.QQ & ");")

        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "TapeLengthsPt1" & BDLEGRIGHT.QCQ & Mid(txtTapeLengths.Text, 1, 60) & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "TapeLengthsPt2" & BDLEGRIGHT.QCQ & Mid(txtTapeLengths.Text, 61) & BDLEGRIGHT.QQ & ");")

        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "AnkleTape" & BDLEGRIGHT.QCQ & txtAnkleTape.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "FirstTape" & BDLEGRIGHT.QCQ & txtFirstTape.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "LastTape" & BDLEGRIGHT.QCQ & txtLastTape.Text & BDLEGRIGHT.QQ & ");")

        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "ToeStyle" & BDLEGRIGHT.QCQ & txtToeStyle.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "FootPleat1" & BDLEGRIGHT.QCQ & txtFootPleat1.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "FootPleat2" & BDLEGRIGHT.QCQ & txtFootPleat2.Text & BDLEGRIGHT.QQ & ");")

        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "TopLegPleat1" & BDLEGRIGHT.QCQ & txtTopLegPleat1.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "TopLegPleat2" & BDLEGRIGHT.QCQ & txtTopLegPleat2.Text & BDLEGRIGHT.QQ & ");")

        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "FootLength" & BDLEGRIGHT.QCQ & txtFootLength.Text & BDLEGRIGHT.QQ & ");")
        PR_PutLine("SetDBData( hLeg" & BDLEGRIGHT.CQ & "Data" & BDLEGRIGHT.QCQ & txtData.Text & BDLEGRIGHT.QQ & ");")

        FileClose(BDLEGRIGHT.fNum)
    End Sub
    ...
    Private Sub PR_PutLine(ByRef sLine As String)
        ...
        'UPGRADE_WARNING: Couldn't resolve default property of object fNum. Click for more: 'ms-help://MS.VSCC.v90/dv_commoner/local/redirect.htm?keyword="6A50421D-15FE-4896-8A1B-2EC21E9037B2"'
        PrintLine(BDLEGRIGHT.fNum, sLine)
    ...
    Private Sub Text1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Text1.Leave
        Dim Index As Short = Text1.GetIndex(eventSender)
        Dim nValue As Double
        nValue = BDLEGRIGHT.Validate_And_Display_Text_In_Box(Text1(Index), labTapeInInches(Index))
    ...
    Private Sub txtFootLength_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtFootLength.Leave
        Dim nValue As Double
        nValue = BDLEGRIGHT.Validate_And_Display_Text_In_Box(txtFootLength, labFootLengthInInches)
    End Sub
    ...
    Private Sub txtFootPleat1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtFootPleat1.Leave
        BDLEGRIGHT.Validate_Text_In_Box(txtFootPleat1)
    End Sub
    ...
    Private Sub txtFootPleat2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtFootPleat2.Leave
        BDLEGRIGHT.Validate_Text_In_Box(txtFootPleat2)
    End Sub
    ...
    Private Sub txtTopLegPleat1_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtTopLegPleat1.Leave
        BDLEGRIGHT.Validate_Text_In_Box(txtTopLegPleat1)
    End Sub
    ...
    Private Sub txtTopLegPleat2_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtTopLegPleat2.Leave
        BDLEGRIGHT.Validate_Text_In_Box(txtTopLegPleat2)
    End Sub
    ...
    Private Sub Update_DDE_Text_Boxes()
        ...
        '
        If (nHeelLen <> BDLEGRIGHT.g_nHeelLen) Or (nAnkleLen <> BDLEGRIGHT.g_nAnkleLen) Then
    ...
    End Function
    Private Sub PR_DrawBodyRightLeg()
        ...
        Dim strBlkName As String = "BODYRIGHTLEG"
    ...
    Private Sub saveInfoToDWG()
        Try
            ...
            If (_sClass.GetXrecord("BodyRightLeg", "BODYRIGHTDIC") IsNot Nothing) Then
                _sClass.RemoveXrecord("BodyRightLeg", "BODYRIGHTDIC")
            End If
            ...
            _sClass.SetXrecord(resbuf, "BodyRightLeg", "BODYRIGHTDIC")
        Catch ex As Exception
