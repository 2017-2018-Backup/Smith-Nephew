...
Friend Class whfigure
    ...
    Public g_sSettingsPath As String
    ...
    Private Sub Cancel_Click(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles Cancel.Click
        ...
        If sCurrentValues <> g_sChangeChecker Then
            Response = MsgBox("Changes have been made, Save changes before closing", 35, "Waist Figure - Dialogue")
    ...
    Public Function PR_CloseFigureDialog() As Boolean
        Dim Response As Short
	...
	Private Function FN_POWERNET_Pressure(ByRef iReduction As Short, ByRef nAnkleCir As Double, ByRef iModulus As Short) As Short
		...
		sConversion = ""
		For ii = 0 To 17
            If iModulus = Val(Project1.WHFIGURE1.POWERNET.Modulus(ii)) Then
                sConversion = Project1.WHFIGURE1.POWERNET.Conversion_Renamed(ii)
                Exit For
            End If
        Next ii
	...
	Private Sub Form_LinkClose()
        ...
        Project1.WHFIGURE1.g_sDiagnosis = txtDiagnosis.Text

        ''g_iUidBody = CShort(txtUidBody.Text)
        g_iUidBody = Val(txtUidBody.Text)
        ...
        If txtUnits.Text = "cm" Then
            WHFIGURE1.g_nUnitsFac = 10 / 25.4
        Else
            WHFIGURE1.g_nUnitsFac = 1
        End If
        ...
        If g_POWERNET = True Then
            ...
            ''WHFIGURE1.PR_GetComboListFromFile(cboFabric, g_sPathJOBST & "\WHFABRIC.DAT")
            ''PR_LoadFabricFromFile(g_sPathJOBST & "\TEMPLTS\POWERNET.DAT")
            WHFIGURE1.PR_GetComboListFromFile(cboFabric, g_sSettingsPath & "\WHFABRIC.DAT")
            PR_LoadFabricFromFile(g_sSettingsPath & "\POWERNET.DAT")
        End If
        If Val(txtLeft(5).Text) <> 0 Then
            If _optFabric_0.Checked = True Or _optFabric_1.Checked = True Then
                For ii = 7 To 29
                    txtLeftMM(ii).Text = ""
        ...
        For ii = 0 To 29
            ...
            If nValue > 0 Then
                ...
                nValue = WHFIGURE1.fnDisplayToInches(nValue)
                '-------------------grdLeftInches.Row = ii
                '-------------------grdLeftInches.Text = WHFIGURE1.fnInchesToText(nValue)
                lblLeft(ii).Text = WHFIGURE1.fnInchesToText(nValue)
            End If
        Next ii
        ...
        For ii = 0 To 29
            ...
            If nValue > 0 Then
                ...
                '----------------grdRightInches.Text = WHFIGURE1.fnInchesToText(nValue)
                lblRight(ii).Text = WHFIGURE1.fnInchesToText(nValue)
            End If
        Next ii
            ...
            If txtRightMMs.Text <> "" And g_iRtAnkle <> 0 Then
                For ii = g_iRtAnkle + 1 To 29
                    ...
                    If iValue > 0 Then
                        ...
                        Rightgms(ii - 6).Text = Str(g_iRtStretch(ii))
                        Rightred(ii - 6).Text = Str(g_iRtRed(ii))
                    End If
                Next ii
            End If
            If txtLeftMMs.Text <> "" And g_iLtAnkle <> 0 Then
                For ii = g_iLtAnkle + 1 To 29
                    ...
                    If iValue > 0 Then
                        ...
                        Leftgms(ii - 6).Text = Str(g_iLtStretch(ii))
                        Leftred(ii - 6).Text = Str(g_iLtRed(ii))
                    End If
                Next ii
            End If
    ...
    Private Sub whfigure_Load(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles MyBase.Load
        Try
            ...
            Project1.WHFIGURE1.MainForm = Me

            WHFIGURE1.g_nUnitsFac = 1 'Default to inches
            g_JOBSTEX = False
            g_JOBSTEX_FL = False
            g_POWERNET = False
            ...
            g_sSettingsPath = fnGetSettingsPath("LookupTables")
            Dim strMod(18), strConv(18) As String
            WHFIGURE1.POWERNET.Modulus = strMod
            WHFIGURE1.POWERNET.Conversion_Renamed = strConv
            ...
            If (blkId.IsNull()) Then
                MsgBox("No Patient Details have been found in drawing!", 16, "Waist Figure - Dialogue")
                Me.Close()
                Exit Sub
            End If
            ...
            If Val(txtLeft(5).Text) <> 0 Then
                If _optFabric_0.Checked = True Or _optFabric_1.Checked = True Then
                    ...
                    For ii = 7 To 29
                        ...
                        txtRightMM(ii).Visible = False
                    Next ii
                ElseIf _optFabric_2.Checked = True Then
                    For ii = 6 To 29
    ...
    Private Sub PR_DisplayFiguredAnkle(ByRef sLeg As String, ByRef nStretch As Double, ByRef nAnkleCir As Double, ByRef nHeelCir As Double, ByRef iFabric As Short)
        ...
        nPatternDim = (nAnkleCir / (1 + (0.01 * nStretch))) / WHFIGURE1.g_nUnitsFac
        iReduction = ARMDIA1.round((1 - 1 / (0.01 * nStretch + 1)) * 100)
        iDonningStretch = ARMDIA1.round((((nHeelCir / WHFIGURE1.g_nUnitsFac) - nPatternDim) / nPatternDim) * 100)
        ...
        If sLeg = "Left" Then
            ...
            Leftgms(g_iLtAnkle - 6).Text = Str(nStretch)
            Leftred(g_iLtAnkle - 6).Text = Str(iReduction)
        ...
        Else
            ...
            Rightgms(g_iRtAnkle - 6).Text = Str(nStretch)
            Rightred(g_iRtAnkle - 6).Text = Str(iReduction)
            'Max Stretch
    ...
    Private Sub PR_EstablishAnkles()
        ...
        g_iLtAnkle = 0
        g_nLtLastAnkle = 0
        g_nLtLastHeel = WHFIGURE1.fnDisplayToInches(Val(txtLeft(5).Text))
        ...
        g_iRtAnkle = 0
        g_nRtLastAnkle = 0
        g_nRtLastHeel = WHFIGURE1.fnDisplayToInches(Val(txtRight(5).Text))
        ...
        g_nLtLastAnkle = WHFIGURE1.fnDisplayToInches(Val(txtLeft(g_iLtAnkle).Text))
        g_nRtLastAnkle = WHFIGURE1.fnDisplayToInches(Val(txtRight(g_iRtAnkle).Text))
    ...
    Private Sub PR_FigureLeftAnkle()
        ...
        'Heel Tape
        If WHFIGURE1.fnDisplayToInches(Val(txtLeft(5).Text)) <> g_nLtLastHeel Then GoTo CALC

        'Ankle Tape
        If WHFIGURE1.fnDisplayToInches(Val(txtLeft(g_iLtAnkle).Text)) <> g_nLtLastAnkle Then GoTo CALC
...
CALC:
        ...
        nHeelCir = WHFIGURE1.fnDisplayToInches(Val(txtLeft(5).Text))
        g_nLtLastHeel = nHeelCir

        nAnkleCir = WHFIGURE1.fnDisplayToInches(Val(txtLeft(g_iLtAnkle).Text))
        ...
        Else
            ...
            'grdLeftDisplay.Col = 1
            'grdLeftDisplay.Text = Str(iReduction)
            Leftgms(g_iLtAnkle - 6).Text = Str(iGrams)
            Leftred(g_iLtAnkle - 6).Text = Str(iReduction)
        ...
        If g_JOBSTEX_FL = True Then
            ...
            'be 25%
            nLastTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtLeft(nLastTape).Text))
            ...
            For ii = g_iLtAnkle + 1 To nLastTape - 1
                nTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtLeft(ii).Text))
    ...
    Private Sub PR_FigureLeftTape(ByRef index As Short)
        ...
        nTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtLeft(index).Text))
        iZipper = chkLeftZipper.CheckState
        iFabric = Val(VB.Left(VB6.GetItemString(cboFabric, cboFabric.SelectedIndex), 2))
        ...
        Leftgms(index - 6).Text = Str(g_iLtStretch(index))
        Leftred(index - 6).Text = Str(g_iLtRed(index))
    ...
    Private Sub PR_FigureRightAnkle()
        ...
        'Heel Tape
        If WHFIGURE1.fnDisplayToInches(Val(txtRight(5).Text)) <> g_nRtLastHeel Then GoTo RT_CALC

        'Ankle Tape
        If WHFIGURE1.fnDisplayToInches(Val(txtRight(g_iRtAnkle).Text)) <> g_nRtLastAnkle Then GoTo RT_CALC
...
RT_CALC:
        ...
        nHeelCir = WHFIGURE1.fnDisplayToInches(Val(txtRight(5).Text))
        g_nRtLastHeel = nHeelCir

        nAnkleCir = WHFIGURE1.fnDisplayToInches(Val(txtRight(g_iRtAnkle).Text))
        ...
        Else
            ...
            'grdRightDisplay.Col = 1
            'grdRightDisplay.Text = Str(iReduction)
            Rightgms(g_iRtAnkle - 6).Text = Str(iGrams)
            Rightred(g_iRtAnkle - 6).Text = Str(iReduction)
        ...
        If g_JOBSTEX_FL = True Then
            ...
            'As the last tape is at a 20 reduction we know that the stretch must
            'be 25%
            nLastTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtRight(nLastTape).Text))
            nLastTapeMMHg = ARMDIA1.round(FN_JOBSTEX_Pressure(nLastTapeCir, nHeelCir, 25, iZipper, iFabric))
            ...
            For ii = g_iRtAnkle + 1 To nLastTape - 1
                nTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtRight(ii).Text))
                nMMHg = 1000 'Force While
                While nMMHg > iPrevMMHg And nStretch > 1
                    nMMHg = ARMDIA1.round(FN_JOBSTEX_Pressure(nTapeCir, nHeelCir, nStretch, iZipper, iFabric))
                    If nMMHg > iPrevMMHg Then nStretch = nStretch - 1
    ...
    Private Sub PR_FigureRightTape(ByRef index As Short)
        ...
        nTapeCir = WHFIGURE1.fnDisplayToInches(Val(txtRight(index).Text))
        iZipper = chkRightZipper.CheckState
        iFabric = Val(VB.Left(VB6.GetItemString(cboFabric, cboFabric.SelectedIndex), 2))
        ...
        Rightgms(index - 6).Text = Str(g_iRtStretch(index))
        Rightred(index - 6).Text = Str(g_iRtRed(index))
    End Sub
    ...
    Private Sub PR_HeelChange(ByRef sLeg As String)
        ...
        If sLeg = "Left" Then
            If WHFIGURE1.fnDisplayToInches(Val(txtLeft(5).Text)) = g_nLtLastHeel Then Exit Sub
        Else
            If WHFIGURE1.fnDisplayToInches(Val(txtRight(5).Text)) = g_nRtLastHeel Then Exit Sub
        End If
    ...
    Private Sub PR_LoadFabricFromFile(ByRef sFileName As String)
        ...
        ii = 0
        Do Until EOF(fNum)
            Input(fNum, Project1.WHFIGURE1.POWERNET.Modulus(ii))
            Input(fNum, Project1.WHFIGURE1.POWERNET.Conversion_Renamed(ii))
            ii = ii + 1
        Loop
        FileClose(fNum)
    ...
    Private Sub txtLeft_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtLeft.Enter
        ...
        WHFIGURE1.Select_Text_In_Box(txtLeft(index))
    End Sub
    ...
    Private Sub txtLeft_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtLeft.Leave
        ...
        WHFIGURE1.Validate_And_Display_Text_In_Box(txtLeft(index), sInchVal)
        If sInchVal <> "" Then
    ...
    Private Sub txtRight_Enter(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtRight.Enter
        ...
        WHFIGURE1.Select_Text_In_Box(txtRight(index))
    End Sub
	
	Private Sub txtRight_Leave(ByVal eventSender As System.Object, ByVal eventArgs As System.EventArgs) Handles txtRight.Leave
        Dim index As Short = txtRight.GetIndex(eventSender)
        Dim sInchVal As String = ""
        WHFIGURE1.Validate_And_Display_Text_In_Box(txtRight(index), sInchVal)
        If sInchVal <> "" Then
            lblRight(index).Text = sInchVal
        End If
	...
	Private Sub Update_DDE_Text_Boxes()
		...
		For ii = 0 To 29
            ...
            If nValue <> 0 Then
                'If CheckBox1.Checked = True And ii >= 6 Then
                '    nValue = nValue + 0.3175
                'End If
                nValue = nValue * 10 'Shift decimal place to right
                nValue = WHFIGURE1.round(nValue)
		...
		For ii = 0 To 29
            ...
            If nValue <> 0 Then
                'If CheckBox2.Checked = True And ii >= 6 Then
                '    nValue = nValue + 0.3175
                'End If
                nValue = nValue * 10 'Shift decimal place to right
                nValue = WHFIGURE1.round(nValue)
    ...
    Private Function PR_GetBlkAttributeValues() As Boolean
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            ...
            If Not acBlkTbl.Has("WAISTBODY") Then
                'MsgBox("Can't find WAISTBODY symbol to update!", 16, "Waist Figure - Dialogue")
                Return False
            End If
            txtUidBody.Text = "WAISTBODY"
            Dim blkRecIdLeft As ObjectId = ObjectId.Null
            If Not acBlkTbl.Has("WAISTLEFTLEG") Then
                'MsgBox("Can't find WAISTLEFTLEG symbol to update!", 16, "Waist Figure - Dialogue")
                Return False
            End If
            blkRecIdLeft = acBlkTbl("WAISTLEFTLEG")
            txtUidLeftLeg.Text = "WAISTLEFTLEG"
            If blkRecIdLeft <> ObjectId.Null Then
                ...
                For Each objID As ObjectId In blkTblRec
                    ...
                    If TypeOf dbObj Is AttributeDefinition Then
                        ...
                        If acAttDef.Tag.ToUpper().Equals("TapeLengthsPt1", StringComparison.InvariantCultureIgnoreCase) Then
                            txtLeftLengths.Text = acAttDef.TextString
                        ElseIf acAttDef.Tag.ToUpper().Equals("TapeLengthsPt2", StringComparison.InvariantCultureIgnoreCase) Then
                            txtLeftLengths.Text = txtLeftLengths.Text & acAttDef.TextString
                        End If
                    End If
                Next
            ...
            If Not acBlkTbl.Has("WAISTRIGHTLEG") Then
                'MsgBox("Can't find WAISTRIGHTLEG symbol to update!", 16, "Waist Figure - Dialogue")
                Return False
            End If
            blkRecIdRight = acBlkTbl("WAISTRIGHTLEG")
            txtUidRightLeg.Text = "WAISTRIGHTLEG"
            If blkRecIdRight <> ObjectId.Null Then
                ...
                For Each objID As ObjectId In blkTblRec
                    ...
                    If TypeOf dbObj Is AttributeDefinition Then
                        Dim acAttDef As AttributeDefinition = dbObj
                        If acAttDef.Tag.ToUpper().Equals("TapeLengthsPt1", StringComparison.InvariantCultureIgnoreCase) Then
                            txtRightLengths.Text = acAttDef.TextString
                        ElseIf acAttDef.Tag.ToUpper().Equals("TapeLengthsPt2", StringComparison.InvariantCultureIgnoreCase) Then
                            txtRightLengths.Text = txtRightLengths.Text & acAttDef.TextString
                        End If
                    End If
                Next
            End If
        ...
        Return True
    End Function
    Private Sub PR_ReadLeftAndRightLegDWGInfo()
        Try
            ...
            If (resbuf IsNot Nothing) Then
                ...
                txtLeftLengths.Text = ""
                For ii = 0 To 29
                    ''txtLeft(ii).Text = arr(ii + 4).Value
                    nValue = Val(arr(ii + 4).Value)
                    If nValue <> 0 Then
                        nValue = nValue * 10 'Shift decimal place to right
                        nValue = WHFIGURE1.round(nValue)
                        sLeftString = New String(" ", 4)
                        sLeftString = RSet(Trim(Str(nValue)), Len(sLeftString))
                    Else
                        sLeftString = New String(" ", 4)
                    End If
                    'Tape values
                    txtLeftLengths.Text = txtLeftLengths.Text & sLeftString
                Next ii
                For ii = 0 To 29
                    lblLeft(ii).Text = arr(ii + 39).Value
                Next ii
            End If
            ...
            If (resbuf IsNot Nothing) Then
                ...
                txtRightLengths.Text = ""
                For ii = 0 To 29
                    ''txtRight(ii).Text = arr(ii + 4).Value
                    nValue = Val(arr(ii + 4).Value)
                    If nValue <> 0 Then
                        nValue = nValue * 10 'Shift decimal place to right
                        nValue = WHFIGURE1.round(nValue)
                        sRightString = New String(" ", 4)
                        sRightString = RSet(Trim(Str(nValue)), Len(sRightString))
                    Else
                        sRightString = New String(" ", 4)
                    End If
                    'Tape values
                    txtRightLengths.Text = txtRightLengths.Text & sRightString
                Next ii
                For ii = 0 To 29
                    lblRight(ii).Text = arr(ii + 39).Value
                Next ii
            End If
        Catch ex As Exception
    ...
    Private Sub PR_Update_BlkAttribute()
        ...
        Using acTrans As Transaction = acCurDb.TransactionManager.StartTransaction()
            Dim acBlkTbl As BlockTable
            acBlkTbl = acTrans.GetObject(acCurDb.BlockTableId, OpenMode.ForRead)
            If acBlkTbl.Has("WAISTBODY") Then
                ...
                For Each objID As ObjectId In acBlkTblRec
                    ...
                    If TypeOf dbObj Is BlockReference Then
                        ...
                        If blkRef.Name = "WAISTBODY" Then
                            Dim bIsContains As Boolean = False
            ...
            If acBlkTbl.Has("WAISTLEFTLEG") Then
                ...
                Next
            End If
            If acBlkTbl.Has("WAISTRIGHTLEG") Then
                Dim acBlkTblRec As BlockTableRecord
                acBlkTblRec = acTrans.GetObject(acBlkTbl(strLayout), OpenMode.ForWrite)
                For Each objID As ObjectId In acBlkTblRec
                    Dim dbObj As DBObject = acTrans.GetObject(objID, OpenMode.ForWrite)
                        ...
                        If blkRef.Name = "WAISTRIGHTLEG" Then
                            For Each attributeID As ObjectId In blkRef.AttributeCollection
                                ...
                                If TypeOf attRefObj Is AttributeReference Then
                                    Dim acAttRef As AttributeReference = attRefObj
                                    If acAttRef.Tag.ToUpper().Equals("TapeLengthsPt1", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightLengths.Text, 1, 60)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("TapeLengthsPt2", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightLengths.Text, 61)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("TapeMMs", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightMMs.Text, 1, 60)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("TapeMMs2", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightMMs.Text, 61)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Reduction", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightRed.Text, 1, 60)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Reduction2", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightRed.Text, 61)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Grams", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightStr.Text, 1, 60)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Grams2", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = Mid(txtRightStr.Text, 61)
                                    ElseIf acAttRef.Tag.ToUpper().Equals("AnkleTape", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = txtRightAnkle.Text
                                    ElseIf acAttRef.Tag.ToUpper().Equals("FirstTape", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = txtRightFirstTape.Text
                                    ElseIf acAttRef.Tag.ToUpper().Equals("LastTape", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = txtRightLastTape.Text
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Pressure", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = txtRightTemplate.Text
                                    ElseIf acAttRef.Tag.ToUpper().Equals("Fabric", StringComparison.InvariantCultureIgnoreCase) Then
                                        acAttRef.TextString = txtFabric.Text
                                    End If
                                End If
        ...
        End Using
    End Sub
    Private Sub PR_UpdateLegInfoToDWG()
        Try
            Dim _sClass As New SurroundingClass()
            Dim resbufLeft As New ResultBuffer
            resbufLeft = _sClass.GetXrecord("WaistLeftLeg", "WHLEFTDIC")
            If (resbufLeft IsNot Nothing) Then
                Dim arr() As TypedValue = resbufLeft.AsArray()
                ''To Remove Old Record
                _sClass.RemoveXrecord("WaistLeftLeg", "WHLEFTDIC")

                ''To Set New Record
                Dim resbufNewLeft As New ResultBuffer
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(0).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(1).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(2).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(3).Value))
                Dim ii As Double
                For ii = 0 To 29
                    resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), txtLeft(ii).Text))
                Next ii

                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(34).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(35).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), arr(36).Value))

                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Bool), arr(37).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Bool), arr(38).Value))
                For ii = 0 To 29
                    resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Text), lblLeft(ii).Text))
                Next ii
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Bool), arr(69).Value))
                resbufNewLeft.Add(New TypedValue(CInt(DxfCode.Bool), arr(70).Value))

                _sClass.SetXrecord(resbufNewLeft, "WaistLeftLeg", "WHLEFTDIC")
            End If

            Dim resbufRight As New ResultBuffer
            resbufRight = _sClass.GetXrecord("WaistRightLeg", "WHRIGHTDIC")
            If (resbufRight IsNot Nothing) Then
                Dim arr() As TypedValue = resbufRight.AsArray()
                ''To Remove Old Record
                _sClass.SetXrecord(resbufRight, "WaistRightLeg", "WHRIGHTDIC")

                ''To Set New Record
                Dim resbufNewRight As New ResultBuffer
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(0).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(1).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(2).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(3).Value))
                Dim ii As Double
                For ii = 0 To 29
                    resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), txtRight(ii).Text))
                Next ii

                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(34).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(35).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), arr(36).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Bool), arr(37).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Bool), arr(38).Value))
                For ii = 0 To 29
                    resbufNewRight.Add(New TypedValue(CInt(DxfCode.Text), lblRight(ii).Text))
                Next ii
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Bool), arr(69).Value))
                resbufNewRight.Add(New TypedValue(CInt(DxfCode.Bool), arr(70).Value))
                _sClass.SetXrecord(resbufNewRight, "WaistRightLeg", "WHRIGHTDIC")
            End If
        Catch ex As Exception
    ...
    Private Sub saveInfoToDWG()
        Try
            Dim _sClass As New SurroundingClass()
            If (_sClass.GetXrecord("WaistFigure", "WHFIGDIC") IsNot Nothing) Then
                _sClass.RemoveXrecord("WaistFigure", "WHFIGDIC")
            End If

            Dim resbuf As New ResultBuffer
            'Dim ii As Double
            'For ii = 1 To txtLeftMM.Count
            '    resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftMM(ii).Text))
            'Next
            'For ii = 1 To Leftgms.Count
            '    resbuf.Add(New TypedValue(CInt(DxfCode.Text), Leftgms(ii).Text))
            'Next
            'For ii = 1 To Leftred.Count
            '    resbuf.Add(New TypedValue(CInt(DxfCode.Text), Leftred(ii).Text))
            'Next
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtFabric.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftAnkle.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtRightAnkle.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftMMs.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtRightMMs.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftStr.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtRightStr.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftRed.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtRightRed.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtLeftTemplate.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Text), txtRightTemplate.Text))
            resbuf.Add(New TypedValue(CInt(DxfCode.Bool), CheckBox1.Checked))
            resbuf.Add(New TypedValue(CInt(DxfCode.Bool), CheckBox2.Checked))

            'resbuf.Add(New TypedValue(CInt(DxfCode.Bool), chkLeftZipper.Checked))
            'resbuf.Add(New TypedValue(CInt(DxfCode.Bool), g_POWERNET))
            'resbuf.Add(New TypedValue(CInt(DxfCode.Bool), g_JOBSTEX))
            'resbuf.Add(New TypedValue(CInt(DxfCode.Bool), g_JOBSTEX_FL))
            _sClass.SetXrecord(resbuf, "WaistFigure", "WHFIGDIC")
        Catch ex As Exception
        End Try
    End Sub
    Private Sub readDWGInfo()
        Try
            ...
            If (resbuf IsNot Nothing) Then
                Dim arr() As TypedValue = resbuf.AsArray()
                txtFabric.Text = arr(0).Value
                txtLeftAnkle.Text = arr(1).Value
                txtRightAnkle.Text = arr(2).Value
                txtLeftMMs.Text = arr(3).Value
                txtRightMMs.Text = arr(4).Value
                txtLeftStr.Text = arr(5).Value
                txtRightStr.Text = arr(6).Value
                txtLeftRed.Text = arr(7).Value
                txtRightRed.Text = arr(8).Value
                txtLeftTemplate.Text = arr(9).Value
                txtRightTemplate.Text = arr(10).Value
                CheckBox1.Checked = arr(11).Value
                CheckBox2.Checked = arr(12).Value
            End If
        Catch ex As Exception
        End Try
    End Sub
    Public Sub PR_LoadFigureFromMain()
        PR_GetBlkAttributeValues()
        readDWGInfo()
        PR_ReadLeftAndRightLegDWGInfo()
        Form_LinkClose()
