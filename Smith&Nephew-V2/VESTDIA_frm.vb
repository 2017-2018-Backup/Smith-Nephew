...
Friend Class vestdia
    ...
    Private Sub PR_CreateMacro_Draw()
        ...
        If FN_CirLinInt(xyStart, xyEnd, xyCen, nNeckActualCir, xyInt) Then
            ...
            End If
        Else
            MsgBox("Can't form Front neck with this data\nCheck JOB.LOG", 48, "Vest")
        End If
        ...
        While nAxilla > 0
            ...
            If sAxillaType.Contains("Regular") Then
                ...
                ElseIf FN_CirLinInt(xyWaistOFF, xyAxilla, xyAxilla, nRegAxillaFac_1, xyInt) Then
                    xyAxillaLow = xyInt
                Else
                    MsgBox("Can't form Axilla with this data\n", 48, "Vest Body - Dialog")
                End If
                xyCen = xyAxilla
                    ...
                    If FN_CirLinInt(xyPt1, xyPt2, xyCen, nRadius, xyInt) Then
                        ...
                        If nCount = 3 Then
                            ...
                            If FN_CalcLength(xyRaglanAxilla, xyFrontNeckOFF) < nInsertToNeckMinDist Then
                                ...
                                MsgBox("WARNING, Axilla Insert line is too close to Front Neck\nTry a smaller Axilla insert\nModule Cancelled", 48, "Vest Body - Dialog")
                                ''--------------Close("file", hCurve)
                                FileClose()
                                ''-----------------------EndPoly()
            ...
            If (sAxillaType.Equals("Open") Or sAxillaType.Equals("Lining")) Then
                ...
                Else
                    ...
                    PR_MakeXY(xyTextIns, xyAxilla.X - 0.25, xyAxilla.Y - 0.5)
                    PR_DrawText("LINING", xyTextIns, 0.125, 0, 0)
                End If
                If (sSide.Equals("Right")) Then
                    ARMDIA1.PR_SetLayer("TemplateRight")
                Else
            ...
            If (sAxillaType.Equals("Sleeveless")) Then
                ...
                aVestCurve = FN_CalcAngle(xyAxillaLow, xySleeveLess)   ''// V.I.P.	
        ...
        If BRAGiven Then
            ...
            Else
                ...
                MsgBox("Can't find BRADISK symbols to insert\nCheck your installation, that JOBST.SLB exists", 16, "Vest Body Dialog")
                Exit Sub
            End If
        ...
        If sNeckType.Equals("Measured Scoop") Then
            ...
            MsgBox("The FRONT Measured Scoop Neck has been drawn as a POLYLINE\nEdit this line and make it OPEN FITTED, this will then be a smooth line", 0, "Vest Body - Dialog")
        Else
        ...
        Else
            ...
            If SleeveLess = False Then
                ...
                MsgBox("WARNING!" & Chr(13) & "You might need to transfer the lowest axilla front neck" & Chr(13) & "to the sleeve raglan.\n
    ...
    Public Function FN_CurveAngle(ByRef xyStart As xy, ByRef nRadiusFromStart As Double) As Double
        ...
        If fileNum Then
            Do While Not EOF(fileNum)
                ...
                If sLine.Contains(Chr(32)) = False Then
                    MsgBox("Can't open VESTCURV.DAT\nCheck installation", 48, "CAD - Glove Dialogue")
                    FileClose()
                    Return 0
                End If
                FN_GetNumber(sLine, nLength, aAngle)
        ...
        If aCurveRotation = 100000 Then
            MsgBox("Can't Find Curve angle with this data in FN_CurveAngle ", 48, "CAD - Glove Dialogue")
        End If
    ...
    Public Sub PR_DataStamp(ByRef xyInsert As xy, ByRef nStampType As Double)
        ...
        If nStampType = 1 Then
            ...
            sText = txtFileNo.Text & Chr(10) & txtDiagnosis.Text & Chr(10) & txtAge.Text & Chr(10) & txtAge.Text
            PR_DrawMText(sText, xyInsert, False)
