...
EXIT_OK:
    ...
    Private Sub PR_CreateDrawMacro(ByRef sFileName As String, ByRef iInsertSize As Short)
        ...
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "LFS")
        MANGLOVE1.PR_AddDBValueToLast("ZipperLength", "%Length")
        'Only add this data base field if this is an editable curve
        If g_ExtendTo <> GLOVE_NORMAL Then MANGLOVE1.PR_AddDBValueToLast("curvetype", "LFS")
        ...
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "ETS")
        MANGLOVE1.PR_AddDBValueToLast("ZipperLength", "%Length")
        'Only add this data base field if this is an editable curve
        If g_ExtendTo <> GLOVE_NORMAL Then MANGLOVE1.PR_AddDBValueToLast("curvetype", "ETS")
        ...
        Else
            ...
            ARMDIA1.PR_SetLayer("Template" & MANGLOVE1.g_sSide)
            MANGLOVE1.PR_DrawLine(xyPt1, xyPt2)
            MANGLOVE1.PR_AddDBValueToLast("Zipper", "EOS")
            MANGLOVE1.PR_AddDBValueToLast("curvetype", "EOS")
        End If
        ...
        ARMDIA1.PR_SetTextData(HORIZ_CENTER, BOTTOM_, CURRENT, CURRENT, CURRENT)
        MANGLOVE1.PR_AddDBValueToLast("curvetype", "WRIST")
        ...
        If chkPalm.CheckState = 1 Or chkDorsal.CheckState = 1 Then
            ...
            If chkPalm.CheckState = 1 Then
                ...
                'Add REINFORCED text
                MANGLOVE1.PR_DrawText("REINFORCED\nPALM", xyPt3, 0.1, 0)
            End If
        ...
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "PALMER")
        ARMDIA1.PR_NamedHandle("hPalmer") 'Save a HANDLE for later use

        MANGLOVE1.PR_DrawMarker(xyLittle)
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "PALMER-WEB")
        If chkSlantedInserts.CheckState = 1 Then
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", Str(nInsert))
        Else
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", "0")
        End If

        MANGLOVE1.PR_DrawMarker(xyRing)
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "DORSAL-WEB")
        If chkSlantedInserts.CheckState = 1 Then
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", Str(nInsert))
        Else
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", "0")
        ...
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "DORSAL")

        MANGLOVE1.PR_DrawMarker(xyMiddle)
        MANGLOVE1.PR_AddDBValueToLast("Zipper", "OUTSIDE-WEB")
        If chkSlantedInserts.CheckState = 1 Then
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", Str(nInsert))
        Else
            MANGLOVE1.PR_AddDBValueToLast("ZipperLength", "0")
        ...
        For ii = 2 To 6
            ...
            MANGLOVE1.PR_AddDBValueToLast("Data", "PALM" & Trim(Str(ii)))
        Next ii
...
debugclose:
    ...
    Private Sub PR_DrawNormalThumb(ByRef nInsert As Double)
        ...
        MANGLOVE1.PR_AddDBValueToLast("curvetype", "ThumbPalmCurve")
        ...
        PR_DrawThumbCurve(ThumbBottCurve, g_ThumbTopCurve)
        MANGLOVE1.PR_AddDBValueToLast("curvetype", "ThumbPieceSeam")
        ...
        'Draw thumb curve
        PR_DrawThumbCurve(ThumbBottCurve, g_ThumbTopCurve)
        MANGLOVE1.PR_AddDBValueToLast("curvetype", "ThumbPieceOutsideEdge")
    ...
    Private Sub PR_DrawTapeNotes(ByRef iVertex As Short, ByRef xyStart As MANGLOVE1.XY, ByRef xyEnd As MANGLOVE1.XY, ByRef nOffSet As Double, ByRef sZipperID As String)
        ...
        If sZipperID <> "" Then
            If sZipperID <> "EOS" Then MANGLOVE1.PR_AddDBValueToLast("curvetype", "EOS")
            MANGLOVE1.PR_AddDBValueToLast("Zipper", sZipperID)
        End If
        ...
        MANGLOVE1.PR_AddDBValueToLast("Data", TapeNote(iVertex).sTapeText)

        'MMs
        MANGLOVE1.PR_AddDBValueToLast("MM", Trim(Str(TapeNote(iVertex).iMMs)) & "mm")

        'Grams
        MANGLOVE1.PR_AddDBValueToLast("Grams", Trim(Str(TapeNote(iVertex).iGms)) & "gm")

        'Reduction
        MANGLOVE1.PR_AddDBValueToLast("Reduction", Trim(Str(TapeNote(iVertex).iRed)))

        'Circumference  in Inches and Eighths format
        iInt = Int(TapeNote(iVertex).nCir)
        MANGLOVE1.PR_AddDBValueToLast("TapeLengths", Trim(Str(iInt)))

        nDec = (TapeNote(iVertex).nCir - iInt)
        If nDec > 0 Then
            iInt = ARMDIA1.round((nDec / EIGHTH) * 10) / 10
            If iInt <> 0 Then MANGLOVE1.PR_AddDBValueToLast("TapeLengths2", Trim(Str(iInt)))
        End If
